# Latoken Connector for StockSharp

This directory contains the **Latoken** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `LatokenMessageAdapter` message adapter, exposing Latoken market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Latoken`.

## Configuration

`LatokenMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new LatokenMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Latoken connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/latoken.html).
