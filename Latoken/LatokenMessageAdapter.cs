...
partial class LatokenMessageAdapter
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="LatokenMessageAdapter"/>.
	/// </summary>
	/// <param name="transactionIdGenerator">Transaction id generator.</param>
	public LatokenMessageAdapter(IdGenerator transactionIdGenerator)
	...
	protected override async ValueTask ConnectAsync(ConnectMessage message, CancellationToken cancellationToken)
	{
		...
		_httpClient = new HttpClient(RestEndpoint, _authenticator) { Parent = this };
		var userId = this.IsTransactional()
			? await _httpClient.GetUserIdAsync(cancellationToken)
			: null;

		_pusherClient = new PusherClient(WebSocketEndpoint, _authenticator, userId, ReConnectionSettings.WorkingTime) { Parent = this };
