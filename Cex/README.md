# Cex Connector for StockSharp

This directory contains the **Cex** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `CexMessageAdapter` message adapter, exposing Cex market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Cex`.

## Configuration

`CexMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new CexMessageAdapter(new IncrementalIdGenerator())
{
