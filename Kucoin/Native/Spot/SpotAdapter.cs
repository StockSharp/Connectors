...
class SpotAdapter : BaseNativeAdapter
{
	...
	private readonly SynchronizedDictionary<(SecurityId secId, TimeSpan tf), long> _candleTransIds = [];

	private readonly SynchronizedSet<long> _stopOrders = [];
	private long? _stopOrderSubId;

	protected string PortfolioName => nameof(Kucoin) + $"_{nameof(Spot)}_" + Adapter.Key.ToId();
	...
	public override async ValueTask ConnectAsync(CancellationToken cancellationToken)
	{
		...
		_httpClient = new(Adapter.IsDemo, Adapter.SpotAddress, new(Adapter.Key, Adapter.Secret, Adapter.Passphrase)) { Parent = Adapter };
	...
	public override async ValueTask RegisterOrderAsync(OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		switch (regMsg.OrderType)
		{
			...
			case OrderTypes.Conditional:
			{
				...
				var withdrawalId = await _httpClient.Withdraw(symbol, regMsg.Volume, condition.WithdrawInfo, cancellationToken);

				await SendOutMessageAsync(new ExecutionMessage
				{
					DataTypeEx = DataType.Transactions,
					ServerTime = CurrentTime,
					OrderStringId = withdrawalId,
		...
		{
			_stopOrders.Add(transId);

			orderId = await _httpClient.RegisterStopOrder(transId.To<string>(), regMsg.Side.ToNative(),
				symbol, regMsg.OrderType.ToNative(), regMsg.Comment, price, regMsg.Volume, condition?.StopPrice,
				"loss", regMsg.TimeInForce.ToNative(regMsg.TillDate, out var cancelAfter), cancelAfter,
				regMsg.PostOnly, regMsg.VisibleVolume, regMsg.MarginMode, cancellationToken);
		}
		else
		{
			orderId = await _httpClient.RegisterOrder(transId.To<string>(), regMsg.Side.ToNative(),
	...
	{
		var task = _stopOrders.Contains(cancelMsg.OriginalTransactionId)
			? _httpClient.CancelStopOrder(cancelMsg.OriginalTransactionId.To<string>(), cancellationToken)
			: _httpClient.CancelOrder(cancelMsg.OriginalTransactionId.To<string>(), cancellationToken);
	...
	public override async ValueTask OrderStatusAsync(OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (!statusMsg.IsSubscribe)
		{
			...
			if (_stopOrderSubId is not null)
			{
				await _privateClient.UnSubscribeStopOrders(_stopOrderSubId.Value, cancellationToken);
				_stopOrderSubId = null;
			}
		...
		if (!statusMsg.IsHistoryOnly())
		{
			...
			_stopOrderSubId = Adapter.TransactionIdGenerator.GetNextId();
			await _privateClient.SubscribeStopOrders(_stopOrderSubId.Value, cancellationToken);
		}
	}
	...
	public override async ValueTask PortfolioLookupAsync(PortfolioLookupMessage lookupMsg, CancellationToken cancellationToken)
	{
		...
		var positions = await _httpClient.GetPositions(default, default, cancellationToken);

		foreach (var position in positions)
		{
			await SendOutMessageAsync(new PositionChangeMessage
			{
				SecurityId = GetSecId(position.Currency),
				PortfolioName = PortfolioName + "_" + position.Type,
				ServerTime = CurrentTime,
				OriginalTransactionId = transId,
			}
			.TryAdd(PositionChangeTypes.CurrentValue, position.Balance?.ToDecimal(), true)
			.TryAdd(PositionChangeTypes.BlockedValue, position.Holds?.ToDecimal(), true), cancellationToken);
		}
	...
	{
		var symbols = await _httpClient.GetSymbols(cancellationToken);

		foreach (var symbol in symbols)
		{
			yield return new SecurityMessage
			{
				SecurityId = GetSecId(symbol.Code),
				OriginalTransactionId = lookupMsg.TransactionId,
				SecurityType = SecurityTypes.CryptoCurrency,
				PriceStep = symbol.PriceIncrement?.ToDecimal(),
				VolumeStep = symbol.BaseIncrement?.ToDecimal(),
				MinVolume = symbol.BaseMinSize?.ToDecimal()
	...
	public override async ValueTask TFCandlesSubscriptionAsync(MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is DateTime from)
		{
			...
			var candles = await _httpClient.GetCandles(symbol, tfName, (long)from.ToUnix(), (long)to.ToUnix(), cancellationToken);
	...
	private ValueTask SessionOnNewCandle(string symbol, TimeSpan tf, Ohlc candle, CancellationToken token)
	{
		...
		if (_candleTransIds.TryGetValue((secId, tf), out var transId))
			return ProcessCandleAsync(candle, transId, token);
	...
	private ValueTask SessionOnPusherBalanceChanged(Balance balance, CancellationToken cancellationToken)
	{
		return SendOutMessageAsync(new PositionChangeMessage
		{
			...
			PortfolioName = PortfolioName,
			ServerTime = balance.Time,
		}
		.TryAdd(PositionChangeTypes.CurrentValue, balance.Available?.ToDecimal(), true)
		.TryAdd(PositionChangeTypes.BlockedValue, balance.Hold?.ToDecimal(), true), cancellationToken);
	}
}
