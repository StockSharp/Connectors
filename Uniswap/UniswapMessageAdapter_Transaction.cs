...
public partial class UniswapMessageAdapter
{
        ...
        OrderRegisterMessage regMsg, CancellationToken cancellationToken)
    {
        ...
        if (regMsg.OrderType is not (null or OrderTypes.Market))
            throw new NotSupportedException(
                "Uniswap swaps are immediate AMM market orders.");
        if (regMsg.Condition is not null)
            throw new NotSupportedException(
                "Uniswap Trading API does not expose conditional orders.");
        ...
        if (!regMsg.UserOrderId.IsEmpty())
            throw new NotSupportedException(
                ...
                "client-order ID cannot be embedded by the Trading API.");
        var volume = regMsg.Volume.Abs();
        if (volume <= 0)
            throw new InvalidOperationException(
                "Uniswap swap volume must be positive.");
        var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
        if (baseUnits <= 0)
            throw new InvalidOperationException(
                "Uniswap swap volume rounds to zero base units.");

        var tradeType = regMsg.Side == Sides.Sell
            ? UniswapTradeTypes.ExactInput
            : UniswapTradeTypes.ExactOutput;
        var preliminary = await GetQuoteAsync(market, tradeType, baseUnits,
            false, cancellationToken);
        var inputToken = regMsg.Side == Sides.Sell
            ? market.BaseToken
            : market.QuoteToken;
        var approvalAmount = (preliminary.Quote.Input.MaximumAmount.IsEmpty()
                ? preliminary.Quote.Input.Amount
                : preliminary.Quote.Input.MaximumAmount).ParseInteger();
        if (!inputToken.Address.IsNativeToken())
            await EnsureApprovalAsync(inputToken, approvalAmount,
                cancellationToken);

        var quote = await GetQuoteAsync(market, tradeType, baseUnits, true,
            cancellationToken);
        var swap = await TradingClient.CreateSwapAsync(new()
        {
            Quote = quote.Quote,
            IsGasPriceRefreshed = true,
            IsTransactionSimulated = true,
            Deadline = DateTime.UtcNow.AddMinutes(2).ToUnixSeconds(),
        }, cancellationToken);
        if (swap?.Swap is null)
            throw new InvalidDataException(
                "Uniswap returned no swap transaction.");
        _ = await RpcClient.EstimateGasAsync(swap.Swap,
            cancellationToken);
        var hash = await RpcClient.SendTransactionAsync(swap.Swap,
            cancellationToken);
        var inputAmount = quote.Quote.Input.Amount.ParseInteger();
        var outputAmount = quote.Quote.Output.Amount.ParseInteger();
        var price = GetSwapPrice(market, regMsg.Side, volume, inputAmount,
            outputAmount);
        ...
        OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "A broadcast Uniswap transaction cannot be replaced through " +
            "the protocol API.");
    }
        ...
        OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "Uniswap has no cancellable order book. Pending EVM nonce " +
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "Uniswap has no open-order group to cancel.");
    }
        ...
        OrderStatusMessage statusMsg, CancellationToken cancellationToken)
    {
        ...
        if (statusMsg.OrderId is not null)
            throw new NotSupportedException(
                "Uniswap orders use EVM transaction hashes, not numeric " +
                "order identifiers.");
        if (!statusMsg.UserId.IsEmpty())
            throw new NotSupportedException(
                "Uniswap has no exchange-side user identifier.");
        if (statusMsg.SecurityIds.Length > 0)
            throw new NotSupportedException(
                "Use the primary security filter for Uniswap order status.");
    ...
    private async ValueTask EnsureApprovalAsync(UniswapToken token,
        BigInteger amount, CancellationToken cancellationToken)
    {
        if (amount <= 0)
            throw new InvalidOperationException(
                "Uniswap approval amount must be positive.");
        var response = await TradingClient.CheckApprovalAsync(new()
        {
            WalletAddress = RpcClient.WalletAddress,
            Token = token.Address,
            Amount = amount.ToString(CultureInfo.InvariantCulture),
            ChainId = (int)Chain,
            IsGasInfoIncluded = true,
        }, cancellationToken);
        if (response is null)
            throw new InvalidDataException(
                "Uniswap returned an empty approval response.");
        if (response.Cancel is not null)
            await BroadcastAndConfirmAsync(response.Cancel,
                "approval reset", cancellationToken);
        if (response.Approval is not null)
            await BroadcastAndConfirmAsync(response.Approval,
                "token approval", cancellationToken);
    }

    private async ValueTask BroadcastAndConfirmAsync(
        UniswapTransactionRequest transaction, string operation,
        CancellationToken cancellationToken)
    {
        _ = await RpcClient.EstimateGasAsync(transaction, cancellationToken);
        ...
        if (!IsSuccessful(receipt))
            throw new InvalidOperationException(
                $"Uniswap {operation} transaction '{hash}' reverted.");
    }
    ...
    private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
        UniswapRpcReceipt receipt, CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<
        (UniswapToken Token, BigInteger Amount)[]> LoadBalancesAsync(
        CancellationToken cancellationToken)
    {
        UniswapToken[] tokens;
        using (_sync.EnterScope())
        ...
        var result = new List<(UniswapToken, BigInteger)>();
    ...
    private async ValueTask SendPortfolioSnapshotAsync(long target,
        bool isForced,
        (UniswapToken Token, BigInteger Amount)[] balances,
        CancellationToken cancellationToken)
    {
    ...
    private ValueTask SendSwapOrderAsync(TrackedSwap swap, long target,
        ...
        => SendOutMessageAsync(new ExecutionMessage
        {
            ...
            CommissionCurrency = Chain == UniswapChains.Tempo
                ? null
                : Chain.NativeSymbol(),
        }, cancellationToken);
    ...
    private decimal? GetCommission(UniswapRpcReceipt receipt)
    {
        if (Chain == UniswapChains.Tempo)
            return null;
        ...
        return cost.FromBaseUnits(Chain.NativeDecimals());
    }

    private static decimal GetSwapPrice(UniswapMarket market, Sides side,
        decimal volume, BigInteger inputAmount, BigInteger outputAmount)
    {
        ...
        if (price <= 0)
            throw new InvalidDataException(
                "Uniswap returned a non-positive execution price.");
        return price;
    }
    ...
    private static bool IsSuccessful(UniswapRpcReceipt receipt)
