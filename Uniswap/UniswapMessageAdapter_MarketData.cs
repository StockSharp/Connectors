...
public partial class UniswapMessageAdapter
{
        ...
        MarketDataMessage mdMsg, CancellationToken cancellationToken)
    {
        ...
        if (mdMsg.From is not null)
            throw new NotSupportedException(
                "Uniswap quote probes do not expose historical Level1 " +
                "events.");
        ...
        MarketDataMessage mdMsg, CancellationToken cancellationToken)
    {
        ...
        if (!AllTimeFrames.Contains(timeFrame))
            throw new NotSupportedException(
                $"Uniswap does not support the {timeFrame} candle interval.");
        var now = DateTime.UtcNow;
        var to = (mdMsg.To ?? now).ToUniversalTime().Min(now);
        var maximum = GetSubscriptionMaximum(mdMsg.Count);
    ...
    private SecurityMessage CreateSecurity(UniswapMarket market,
    ...
    private async ValueTask SendLevel1Async(UniswapMarket market,
    ...
    private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
        UniswapMarket market, CancellationToken cancellationToken)
    {
        ...
        if (units <= 0)
            throw new InvalidOperationException(
                ...
                "units.");
        var bidQuote = await GetQuoteAsync(market,
            UniswapTradeTypes.ExactInput, units, false, cancellationToken);
        var askQuote = await GetQuoteAsync(market,
            UniswapTradeTypes.ExactOutput, units, false, cancellationToken);
        var bidOutput = bidQuote.Quote.Output.Amount.ParseInteger()
            .FromBaseUnits(market.QuoteToken.Decimals);
        var askInput = askQuote.Quote.Input.Amount.ParseInteger()
            .FromBaseUnits(market.QuoteToken.Decimals);
        var bid = bidOutput / ProbeVolume;
        var ask = askInput / ProbeVolume;
        if (bid <= 0 || ask <= 0)
            throw new InvalidDataException(
                "Uniswap returned a non-positive quote price.");
        return (bid, ask);
    }

    private ValueTask SendLevel1Async(UniswapMarket market, long target,
    ...
    private async ValueTask<UniswapQuoteResponse> GetQuoteAsync(
        UniswapMarket market, UniswapTradeTypes tradeType,
        BigInteger amount, bool isTrading,
        CancellationToken cancellationToken)
    {
        var isExactInput = tradeType == UniswapTradeTypes.ExactInput;
        var response = await TradingClient.GetQuoteAsync(new()
        {
            TradeType = tradeType,
            Amount = amount.ToString(CultureInfo.InvariantCulture),
            TokenInChainId = (int)Chain,
            TokenOutChainId = (int)Chain,
            TokenIn = (isExactInput
                ? market.BaseToken
                : market.QuoteToken).Address,
            TokenOut = (isExactInput
                ? market.QuoteToken
                : market.BaseToken).Address,
            Swapper = RpcClient.WalletAddress,
            SlippageTolerance = SlippageTolerance,
            RoutingPreference = UniswapRoutingPreferences.BestPrice,
            Protocols = [UniswapProtocols.V2, UniswapProtocols.V3,
                UniswapProtocols.V4],
        }, cancellationToken);
        if (response?.Quote is null || response.Routing !=
            UniswapRoutings.Classic)
            throw new InvalidDataException(
                $"Uniswap returned unsupported routing " +
                $"'{response?.Routing}'.");
        if (response.Quote.ChainId != (int)Chain ||
            response.Quote.Input is null || response.Quote.Output is null ||
            response.Quote.Input.Amount.IsEmpty() ||
            response.Quote.Output.Amount.IsEmpty())
            throw new InvalidDataException(
                "Uniswap returned an incomplete classic quote.");
        if (isTrading && response.PermitData is not null)
            throw new InvalidDataException(
                "Uniswap returned Permit2 data although direct approvals " +
                "were explicitly requested.");
        if (isTrading && response.Quote.TransactionFailureReasons is
            { Length: > 0 })
            throw new InvalidOperationException(
                "Uniswap quote simulation failed: " + string.Join(", ",
                    response.Quote.TransactionFailureReasons));
        return response;
    }

    private async ValueTask<UniswapTrade[]> LoadTradesAsync(
        UniswapMarket market, DateTime from, DateTime to, int maximum,
        CancellationToken cancellationToken)
    {
        var swaps = await GraphClient.GetSwapsAsync(market.PoolId, from,
    ...
    private static UniswapTrade ToTrade(UniswapMarket market,
        UniswapSwap swap)
    {
        if (swap?.Id.IsEmpty() != false || swap.Timestamp.IsEmpty())
            return null;
        var amount0 = swap.Amount0.ToDecimalInvariant();
        var amount1 = swap.Amount1.ToDecimalInvariant();
    ...
    private async ValueTask<bool> SendTradeAsync(UniswapMarket market,
        UniswapTrade trade, long target,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<UniswapCandle[]> LoadCandlesAsync(
        UniswapMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
    ...
    private ValueTask SendCandleAsync(UniswapMarket market,
        UniswapCandle candle, TimeSpan timeFrame, long target,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask PollLevel1Async(
        CancellationToken cancellationToken)
    {
        (UniswapMarket Market, long[] Targets)[] groups;
        using (_sync.EnterScope())
    ...
    private static DateTime FloorTime(DateTime value, TimeSpan interval)
    {
        ...
        return DateTime.UnixEpoch.AddTicks(ticks / interval.Ticks *
            interval.Ticks);
    }
