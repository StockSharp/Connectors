...
public partial class PionexMessageAdapter
{
	...
	{
		var mappings = new List<(string Symbol, PionexSections Section)>();
		if (IsSectionEnabled(PionexSections.Spot))
			mappings.AddRange((await RestClient.GetSpotSymbolsAsync(cancellationToken))
				...
				.Select(static item => (item.Symbol.ToUpperInvariant(), PionexSections.Spot)));
		if (IsSectionEnabled(PionexSections.Futures))
			mappings.AddRange((await RestClient.GetFuturesSymbolsAsync(cancellationToken))
				.Where(static item => item?.Symbol.IsEmpty() == false)
				.Select(static item => (item.Symbol.ToUpperInvariant(), PionexSections.Futures)));
		using (_sync.EnterScope())
		{
			_symbolSections.Clear();
			foreach (var mapping in mappings)
				_symbolSections[mapping.Symbol] = mapping.Section;
		}
	}
		...
		CancellationToken cancellationToken)
	{
			...
			(securityTypes.Count == 0 || securityTypes.Contains(SecurityTypes.CryptoCurrency)))
		{
			...
			foreach (var symbol in symbols)
			{
				...
				using (_sync.EnterScope())
					_symbolSections[code] = PionexSections.Spot;
				var security = new SecurityMessage
				{
					SecurityId = code.ToStockSharp(PionexSections.Spot),
		...
		if (left > 0 && IsSectionEnabled(PionexSections.Futures) &&
			(securityTypes.Count == 0 || securityTypes.Contains(SecurityTypes.Future)))
		{
			...
			foreach (var symbol in symbols)
			{
				...
				var code = symbol.Symbol.ToUpperInvariant();
				using (_sync.EnterScope())
					_symbolSections[code] = PionexSections.Futures;
				var security = new SecurityMessage
				{
					SecurityId = code.ToStockSharp(PionexSections.Futures),
					Name = symbol.Name.IsEmpty(code),
		...
		CancellationToken cancellationToken)
	{
		...
		using (_sync.EnterScope())
		{
			if (_level1Subscriptions.Remove(transactionId, out subscription))
			{
				...
				if (subscription.Section == PionexSections.Futures)
	...
	private async ValueTask OnTradesAsync(PionexWsTradeMessage message,
		CancellationToken cancellationToken)
	{
	...
	private CandleEmission[] UpdateCandles(string symbol, PionexSections section, DateTime time,
	...
	private async ValueTask OnDepthAsync(PionexWsDepthMessage message,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnIndexAsync(PionexWsIndexMessage message,
		CancellationToken cancellationToken)
	{
		foreach (var index in message?.Data ?? [])
		{
			...
			using (_sync.EnterScope())
				ids = [.. _level1Subscriptions
					.Where(pair => pair.Value.Section == PionexSections.Futures &&
			...
			foreach (var id in ids)
				await SendOutMessageAsync(new Level1ChangeMessage
				{
					SecurityId = symbol.ToStockSharp(PionexSections.Futures),
					ServerTime = time,
					OriginalTransactionId = id,
				}
	...
	private ValueTask SendLevel1SnapshotAsync(string symbol, PionexSections section,
		PionexTicker ticker, PionexBookTicker book, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendBestQuotesAsync(PionexBookLevel[] bids, PionexBookLevel[] asks,
		string symbol, PionexSections section, long transactionId, DateTime serverTime,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new Level1ChangeMessage
		{
			SecurityId = symbol.ToStockSharp(section),
	...
	private ValueTask SendBookAsync(PionexBookLevel[] bids, PionexBookLevel[] asks,
		string symbol, PionexSections section, long transactionId, DateTime serverTime, int depth,
		CancellationToken cancellationToken)
	...
	private ValueTask SendTradeAsync(string id, string price, string volume, DateTime time,
		Sides side, string symbol, PionexSections section, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(DateTime openTime, decimal open, decimal high, decimal low,
		decimal close, decimal volume, string symbol, PionexSections section, TimeSpan timeFrame,
	...
	private static QuoteChange[] ToQuotes(PionexBookLevel[] levels, int depth)
	...
	private static int GetCandleCount(MarketDataMessage message, TimeSpan timeFrame, DateTime to)
	{
		if (message.Count is long count)
			return count.Min(500).Max(1).To<int>();
		if (message.From is DateTime from && to > from)
			return ((to - from).Ticks / timeFrame.Ticks + 1).Min(500L).Max(1L).To<int>();
		return 500;
	}
}
