...
public partial class BtcTurkMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			_restClient = new(RestEndpoint, GraphEndpoint, Key, Secret)
			{
				Parent = this,
			};
			...
			if (exchangeInfo?.Symbols is not { Length: > 0 })
				throw new InvalidDataException(
					"BtcTurk returned no available markets.");
			RegisterMarkets(exchangeInfo.Symbols);

			_wsClient = new(WebSocketEndpoint,
		...
		CancellationToken cancellationToken)
	{
		...
		if (_portfolioSubscriptionId != 0 &&
			RestClient.IsCredentialsAvailable)
			await SendPortfolioSnapshotAsync(_portfolioSubscriptionId,
				cancellationToken);
		if (_orderStatusSubscriptionId != 0 &&
			RestClient.IsCredentialsAvailable)
