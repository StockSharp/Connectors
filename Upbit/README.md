# Upbit Connector for StockSharp

This directory contains the **Upbit** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `UpbitMessageAdapter` message adapter, exposing Upbit market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Upbit`.

## Configuration

`UpbitMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new UpbitMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Upbit connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/upbit.html).
