...
public partial class GateIOMessageAdapter
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="GateIOMessageAdapter"/>.
	/// </summary>
	/// <param name="transactionIdGenerator">Transaction id generator.</param>
	public GateIOMessageAdapter(IdGenerator transactionIdGenerator)
	...
	public override string[] AssociatedBoards
		=> [BoardCodes.GateIO, BoardCodes.GateIODelivery, BoardCodes.GateIOPerpetual, BoardCodes.GateIOOptions];
	...
	protected override ValueTask ConnectAsync(ConnectMessage connectMsg, CancellationToken cancellationToken)
	{
		...
		if (Sections.Contains(GateIOSections.Spot))
			addAdapter(new Native.Spot.SpotAdapter(this, _authenticator));

		if (Sections.Contains(GateIOSections.Perpetual))
			addAdapter(new Native.Futures.FuturesAdapter(this, _authenticator));

		if (Sections.Contains(GateIOSections.Delivery))
			addAdapter(new Native.Delivery.DeliveryAdapter(this, _authenticator));

		if (Sections.Contains(GateIOSections.Options))
			addAdapter(new Native.Options.OptionsAdapter(this, _authenticator));

		return _tracker.ConnectAsync(cancellationToken);
	}
