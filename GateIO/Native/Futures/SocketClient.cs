...
class SocketClient : BaseLogReceiver
{
	private static class Channels
	{
		...
		public const string OrderPlace = "futures.order_place";
		public const string CancelOrder = "futures.order_cancel";
		public const string CancelAllOrders = "futures.order_cancel_cp";
		public const string OrderAmend = "futures.order_amend";
		public const string Ping = "futures.ping";
		public const string Pong = "futures.pong";
		public const string Login = "futures.login";
	}
	...
	public override string Name => nameof(GateIO) + "_" + nameof(Futures) + nameof(SocketClient);
	...
	public SocketClient(GateIOMessageAdapter adapter, string coin, Authenticator authenticator, WorkingTime workingTime)
	{
		...
		_url = $"wss://{(adapter.IsDemo ? adapter.DemoFuturesWsDomain : adapter.FuturesWsDomain)}/{coin}";
		...
		switch (channel)
		{
			case Channels.Ticker:
				if (TickersReceived is { } tickerHandler)
					await tickerHandler.InvokeAsync(get<IEnumerable<Ticker>>(), cancellationToken);
				break;
	...
	public ValueTask OrderPlace(string reqId, string symbol, decimal amount, decimal price, string tif, decimal? iceberg, bool? reduceOnly, CancellationToken cancellationToken)
