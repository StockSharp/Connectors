...
class SocketClient : BaseLogReceiver
{
	...
	{
		public const string Ticker = "options.contract_tickers";
		public const string OrderBook = "options.order_book_update";
		public const string Trades = "options.trades";
		public const string BookTicker = "options.book_ticker";
		public const string Candles = "options.contract_candlesticks";
		public const string Orders = "options.orders";
		public const string UserTrades = "options.usertrades";
		public const string Positions = "options.positions";
		public const string Balances = "options.balances";
		public const string Ping = "options.ping";
		public const string Pong = "options.pong";
	}
	...
	public override string Name => nameof(GateIO) + "_" + nameof(Options) + nameof(SocketClient);
		...
		switch (channel)
		{
			...
			case Channels.Trades:
				if (TradesReceived is { } tradeHandler)
					await tradeHandler.InvokeAsync(get<IEnumerable<Trade>>(), cancellationToken);
				break;
			...
			case Channels.Candles:
				if (CandlesReceived is { } candleHandler)
					await candleHandler.InvokeAsync(get<IEnumerable<Candle>>(), cancellationToken);
				break;
			case Channels.Positions:
				if (PositionsReceived is { } posHandler)
					await posHandler.InvokeAsync(get<IEnumerable<Position>>(), cancellationToken);
				break;
	...
	public ValueTask SubscribeOrders(long transId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(transId, Channels.Orders, Events.Subscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask UnsubscribeOrders(long originTransId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(-originTransId, Channels.Orders, Events.Unsubscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask SubscribeUserTrades(long transId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(transId, Channels.UserTrades, Events.Subscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask UnsubscribeUserTrades(long originTransId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(-originTransId, Channels.UserTrades, Events.Unsubscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask SubscribePositions(long transId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(transId, Channels.Positions, Events.Subscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask UnsubscribePositions(long originTransId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(-originTransId, Channels.Positions, Events.Unsubscribe, new[] { userId, _all }, cancellationToken);

	public ValueTask SubscribeBalances(long transId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(transId, Channels.Balances, Events.Subscribe, new[] { userId }, cancellationToken);

	public ValueTask UnsubscribeBalances(long originTransId, string userId, CancellationToken cancellationToken)
		=> SendWithAuth(-originTransId, Channels.Balances, Events.Unsubscribe, new[] { userId }, cancellationToken);
