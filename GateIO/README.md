# GateIO Connector for StockSharp

This directory contains the **GateIO** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `GateIOMessageAdapter` message adapter, exposing GateIO market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `GateIO`.

## Configuration

`GateIOMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new GateIOMessageAdapter(new IncrementalIdGenerator())
{
...
See the [GateIO connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/gateio.html).
