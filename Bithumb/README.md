# Bithumb Connector for StockSharp

This directory contains the **Bithumb** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `BithumbMessageAdapter` message adapter, exposing Bithumb market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Bithumb`.

## Configuration

`BithumbMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new BithumbMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Bithumb connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/bithumb.html).
