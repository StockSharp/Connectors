...
public partial class VelodromeMessageAdapter
{
	...
	{
		public VelodromeMarket Market { get; init; }
	}
	...
	private sealed class TrackedSwap
	{
		...
		public VelodromeRpcReceipt Receipt { get; set; }
	}
	...
	private readonly Dictionary<string, VelodromeMarket> _markets =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, VelodromeMarket> _marketsByPool =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, VelodromeToken> _tokens =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private readonly Queue<VelodromeRpcLog> _realtimeLogs = [];
	private readonly Dictionary<BigInteger, DateTime> _blockTimes = [];
	private readonly Queue<BigInteger> _blockTimeOrder = [];
	private VelodromeRpcClient _rpcClient;
	private VelodromeSocketClient _socketClient;
	private DateTime _nextMarketPoll;
	private DateTime _nextPrivatePoll;

	/// <summary>Initializes the adapter.</summary>
	public VelodromeMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.Velodrome];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Velodrome) ||
			securityId.IsAssociated(BoardCodes.Velodrome);

	private VelodromeRpcClient RpcClient => _rpcClient ?? throw new
	...
	private void EnsureTradingReady()
	{
		...
		if (!RpcClient.IsSigningAvailable)
			throw new InvalidOperationException(
				"An EVM private key is required for Velodrome swaps.");
	}

	private VelodromeMarket GetMarket(SecurityId securityId)
	{
		if (!ValidateSecurityId(securityId))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not Velodrome.");
		...
		using (_sync.EnterScope())
			return _markets.TryGetValue(code, out var market)
				? market
				: throw new InvalidOperationException(
					$"Unknown Velodrome market '{code}'.");
	}

	private string GetPortfolioName()
		=> $"Velodrome_Optimism_{RpcClient.WalletAddress[2..10]}";
	...
	private void ValidatePortfolio(string portfolioName)
	{
		...
		if (!portfolioName.IsEmpty() &&
			...
			throw new InvalidOperationException(
				$"Unknown Velodrome portfolio '{portfolioName}'.");
	}

	private void OnSocketLog(VelodromeRpcLog log)
	{
		if (log is null)
			return;
		using (_sync.EnterScope())
