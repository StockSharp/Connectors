...
public partial class VelodromeMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			_ = await GetOrLoadTokenAsync(
				VelodromeExtensions.NativeTokenAddress, cancellationToken);
			VelodromeMarketDefinition[] definitions;
			try
			{
			...
			{
				throw new InvalidOperationException(
					"Configured Velodrome pool definitions are invalid.",
					error);
			}
			...
			foreach (var definition in definitions)
			{
					...
					!cancellationToken.IsCancellationRequested)
				{
					errors.Add(error);
					this.AddWarningLog(
						"Velodrome pool {0} loading failed: {1}",
			...
			if (markets.Length == 0)
				throw errors.Count == 1
					...
					: new AggregateException(
						"No Velodrome pools could be loaded.", errors);
			if (!WebSocketEndpoint.IsEmpty())
			{
				...
				{
					this.AddWarningLog(
						"Velodrome WebSocket is unavailable; JSON-RPC log " +
						"polling remains active: {0}", error.Message);
					DisposeSocket();
				}
			}
			connectMsg.SessionId = RpcClient.IsWalletConfigured
				? $"Velodrome Optimism {RpcClient.WalletAddress}"
				: "Velodrome Optimism public";
	...
	private async ValueTask RegisterDefinitionAsync(
		VelodromeMarketDefinition definition,
		CancellationToken cancellationToken)
	{
		var pool = await RpcClient.GetPoolAsync(definition.PoolId,
			cancellationToken);
		VelodromeToken baseToken;
		VelodromeToken quoteToken;
	...
	private async ValueTask<VelodromeToken> GetOrLoadTokenAsync(
	...
	private void RegisterMarket(VelodromeMarket market)
	{
		if (market?.PoolId.IsEmpty() != false || market.BaseToken is null ||
			...
			throw new InvalidDataException(
				"Velodrome pool metadata is incomplete.");
		using (_sync.EnterScope())
		{
			if (_marketsByPool.ContainsKey(market.PoolId))
				throw new InvalidOperationException(
					$"Velodrome pool '{market.PoolId}' is configured twice.");
			if (_markets.ContainsKey(market.SecurityCode))
				throw new InvalidOperationException(
					$"Velodrome security code '{market.SecurityCode}' is " +
					"configured twice.");
	...
	private VelodromeMarketDefinition[] ParseMarketDefinitions()
	{
		if (Pools.IsEmpty())
			throw new InvalidOperationException(
			...
			StringSplitOptions.TrimEntries))
		{
			...
			if (fields.Length is not 1 and not 3 and not 4)
				throw new FormatException(
					"Each Velodrome pool must use pool or " +
					"pool|base-token|quote-token|security-code format.");
			if (fields.Length >= 3 &&
				(fields[1].IsEmpty() || fields[2].IsEmpty()))
				throw new FormatException(
					"Both Velodrome base and quote token addresses are " +
	...
	private static (VelodromeToken BaseToken, VelodromeToken QuoteToken)
		OrientMarket(VelodromeToken token0, VelodromeToken token1)
	...
	private static string CreateSecurityCode(VelodromeToken baseToken,
		VelodromeToken quoteToken, VelodromePool pool)
	{
		var suffix = pool.PoolType switch
		{
			VelodromePoolTypes.Volatile => "VOLATILE",
			VelodromePoolTypes.Stable => "STABLE",
			VelodromePoolTypes.Slipstream => $"CL{pool.TickSpacing}",
			_ => throw new ArgumentOutOfRangeException(nameof(pool)),
		};
		return NormalizeSecurityCode(
			$"{baseToken.Symbol}-{quoteToken.Symbol}-{suffix}");
	}
	...
	private static string NormalizeSecurityCode(string value)
	{
		...
		if (value.Length > 64 || value.Any(static ch =>
			...
			throw new FormatException(
				$"Invalid Velodrome security code '{value}'.");
		return value;
	}
