...
sealed class VelodromeSocketClient : BaseLogReceiver
{
	...
	public VelodromeSocketClient(string endpoint)
	{
	...
	public override string Name => "Velodrome_WebSocket";

	public event Action<VelodromeRpcLog> LogReceived;

	public async ValueTask ConnectAsync(IEnumerable<VelodromeMarket> markets,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(markets);
		if (_socket is not null)
			throw new InvalidOperationException(
				"The Velodrome WebSocket is already connected.");
	...
	protected override void DisposeManaged()
	{
		...
		_socket?.Dispose();
		_lifetime?.Dispose();
		FailPending(new ObjectDisposedException(nameof(VelodromeSocketClient)));
		_sendGate.Dispose();
		_socket = null;
		_lifetime = null;
		_receiveTask = null;
	...
	private async ValueTask SubscribeAsync(VelodromeMarket market,
		CancellationToken cancellationToken)
	{
		...
		var payload = JsonConvert.SerializeObject(new VelodromeSocketRequest
		{
			Id = id,
			Parameters = new()
			{
		...
		try
		{
			...
			if (subscription.IsEmpty())
				throw new InvalidDataException(
					"Velodrome returned an empty WebSocket subscription id.");
		}
		catch
		{
			using (_sync.EnterScope())
		...
		CancellationToken cancellationToken)
	{
		var socket = _socket ?? throw new InvalidOperationException(
			"The Velodrome WebSocket is not connected.");
	...
	private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
	{
		try
		{
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				VelodromeSocketMessage message;
				try
				{
					message = JsonConvert.DeserializeObject<
						VelodromeSocketMessage>(payload, _jsonSettings);
				}
				catch (JsonException error)
				{
					throw new InvalidDataException(
						"Velodrome WebSocket returned an unexpected payload.",
						error);
				}
				if (message is null)
					throw new InvalidDataException(
						"Velodrome WebSocket returned an empty payload.");
				...
				if (message.Method.EqualsIgnoreCase("eth_subscription") &&
					message.Parameters?.Result is VelodromeRpcLog log)
				{
					try
					{
		...
		{
			this.AddWarningLog(
				"Velodrome WebSocket receive loop stopped: {0}",
				error.Message);
			FailPending(error);
		}
	}
		...
		while (true)
		{
			...
			if (result.MessageType == WebSocketMessageType.Close)
				throw new WebSocketException(
					$"Velodrome WebSocket closed with status " +
					$"'{socket.CloseStatus}'.");
			if (result.MessageType != WebSocketMessageType.Text)
				throw new InvalidDataException(
					"Velodrome WebSocket returned a non-text message.");
			if (target.Length + result.Count > _maximumMessageBytes)
				throw new InvalidDataException(
					"Velodrome WebSocket message exceeds 1 MiB.");
