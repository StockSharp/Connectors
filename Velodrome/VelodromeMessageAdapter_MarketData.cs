...
public partial class VelodromeMessageAdapter
{
	/// <inheritdoc />
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Velodrome quote probes do not expose historical Level1 " +
				"events.");
		...
		catch (Exception error)
		{
			throw new InvalidOperationException(
				$"Velodrome Level1 quote failed for '{market.SecurityCode}'.",
				error);
		}
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!AllTimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"Velodrome does not support the {timeFrame} candle interval.");
		var now = DateTime.UtcNow;
		var to = (mdMsg.To ?? now).ToUniversalTime().Min(now);
		var maximum = GetSubscriptionMaximum(mdMsg.Count);
	...
	private SecurityMessage CreateSecurity(VelodromeMarket market,
	...
	private async ValueTask SendLevel1Async(VelodromeMarket market,
	...
	private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
		VelodromeMarket market, CancellationToken cancellationToken)
	{
		...
		var bidQuote = await RpcClient.GetQuoteAsync(market,
			VelodromeTradeTypes.ExactInput, units, cancellationToken);
		var askQuote = await RpcClient.GetQuoteAsync(market,
			VelodromeTradeTypes.ExactOutput, units, cancellationToken);
		...
		if (bid <= 0 || ask <= 0)
			throw new InvalidDataException(
				"Velodrome returned a non-positive quote price.");
		return (bid, ask);
	}

	private ValueTask SendLevel1Async(VelodromeMarket market, long target,
	...
	private async ValueTask<VelodromeTrade[]> LoadTradesAsync(
		VelodromeMarket market, DateTime from, DateTime to, int maximum,
	...
	private async ValueTask<VelodromeTrade[]> LoadTradesByBlocksAsync(
		VelodromeMarket market, BigInteger fromBlock, BigInteger toBlock,
	...
	private async ValueTask<VelodromeTrade> ToTradeAsync(
		VelodromeMarket market, VelodromeRpcLog log,
		CancellationToken cancellationToken)
	{
		if (log is null || log.IsRemoved || log.Address.IsEmpty() ||
			...
			!topics[0].EqualsIgnoreCase(market.PoolType.GetSwapTopic()) ||
		...
		try
		{
			...
			{
				amount0 = VelodromeExtensions.ReadAbiSignedWord(log.Data, 0);
				amount1 = VelodromeExtensions.ReadAbiSignedWord(log.Data, 1);
			}
			else
			{
				amount0 = VelodromeExtensions.ReadAbiWord(log.Data, 0) -
					VelodromeExtensions.ReadAbiWord(log.Data, 2);
				amount1 = VelodromeExtensions.ReadAbiWord(log.Data, 1) -
					VelodromeExtensions.ReadAbiWord(log.Data, 3);
			}
		}
	...
	private async ValueTask<DateTime> GetLogTimeAsync(VelodromeRpcLog log,
		BigInteger blockNumber, CancellationToken cancellationToken)
	{
		// Optimism returns the block timestamp inside the log itself, which
		// spares a
	...
	private async ValueTask<bool> SendTradeAsync(VelodromeMarket market,
		VelodromeTrade trade, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask<VelodromeCandle[]> LoadCandlesAsync(
		VelodromeMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
	...
	private ValueTask SendCandleAsync(VelodromeMarket market,
		VelodromeCandle candle, TimeSpan timeFrame, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask PollLevel1Async(
		CancellationToken cancellationToken)
	{
		(VelodromeMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
