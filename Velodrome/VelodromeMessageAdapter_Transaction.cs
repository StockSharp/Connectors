...
public partial class VelodromeMessageAdapter
{
		...
		OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		if (regMsg.OrderType is not (null or OrderTypes.Market))
			throw new NotSupportedException(
				"Velodrome swaps are immediate AMM market orders.");
		if (regMsg.Condition is not null)
			throw new NotSupportedException(
				"Velodrome pool contracts do not expose conditional orders.");
		...
		if (volume <= 0)
			throw new InvalidOperationException(
				"Velodrome swap volume must be positive.");
		var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
		if (baseUnits <= 0)
			throw new InvalidOperationException(
				"Velodrome swap volume rounds to zero base units.");

		var tradeType = regMsg.Side == Sides.Sell
			? VelodromeTradeTypes.ExactInput
			: VelodromeTradeTypes.ExactOutput;
		...
		var approvalAmount = tradeType == VelodromeTradeTypes.ExactInput
		...
		OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"A broadcast Velodrome transaction cannot be replaced through " +
			"the protocol API.");
	}
		...
		OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"Velodrome has no cancellable order book. Pending EVM nonce " +
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"Velodrome has no open-order group to cancel.");
	}
		...
		OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (statusMsg.OrderId is not null)
			throw new NotSupportedException(
				"Velodrome orders use EVM transaction hashes, not numeric " +
				"order identifiers.");
		if (!statusMsg.UserId.IsEmpty())
			throw new NotSupportedException(
				"Velodrome has no exchange-side user identifier.");
		if (statusMsg.SecurityIds.Length > 0)
			throw new NotSupportedException(
				"Use the primary security filter for Velodrome order status.");
	...
	private async ValueTask EnsureApprovalAsync(VelodromeToken token,
		string spender, BigInteger amount,
		CancellationToken cancellationToken)
	{
		if (amount <= 0)
			throw new InvalidOperationException(
				"Velodrome approval amount must be positive.");
	...
	private async ValueTask BroadcastAndConfirmAsync(
		VelodromeTransaction transaction, string operation,
		CancellationToken cancellationToken)
	{
		var hash = await RpcClient.SendTransactionAsync(transaction,
			cancellationToken);
		...
		if (!IsSuccessful(receipt))
			throw new InvalidOperationException(
				$"Velodrome {operation} transaction '{hash}' reverted.");
	}
	...
	private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
		VelodromeRpcReceipt receipt, CancellationToken cancellationToken)
	{
	...
	private async ValueTask<
		(VelodromeToken Token, BigInteger Amount)[]> LoadBalancesAsync(
		CancellationToken cancellationToken)
	{
		VelodromeToken[] tokens;
		using (_sync.EnterScope())
			tokens = [.. _tokens.Values.GroupBy(static token => token.Address,
					StringComparer.OrdinalIgnoreCase)
				.Select(static group => group.First())];
		var result = new List<(VelodromeToken, BigInteger)>();
	...
	private async ValueTask SendPortfolioSnapshotAsync(long target,
		bool isForced,
		(VelodromeToken Token, BigInteger Amount)[] balances,
		CancellationToken cancellationToken)
	{
	...
	private static decimal? GetCommission(VelodromeRpcReceipt receipt)
	{
	...
	private static decimal GetSwapPrice(VelodromeMarket market, Sides side,
		decimal volume, BigInteger inputAmount, BigInteger outputAmount)
	{
		...
		if (price <= 0)
			throw new InvalidDataException(
				"Velodrome returned a non-positive execution price.");
		return price;
	}

	private static SwapExecution ReadSwapExecution(TrackedSwap swap,
		VelodromeRpcReceipt receipt)
	{
		...
		foreach (var log in receipt.Logs ?? [])
		{
			...
			if (!address.EqualsIgnoreCase(swap.Market.PoolId))
				continue;
			if (swap.Market.PoolType != VelodromePoolTypes.Slipstream)
			{
			...
			{
				amount0 += VelodromeExtensions.ReadAbiSignedWord(
					log.Data, 0);
				amount1 += VelodromeExtensions.ReadAbiSignedWord(
					log.Data, 1);
			}
			isFound = true;
		}
		...
		if (!isFound)
			throw new InvalidDataException(
				$"Successful Velodrome transaction " +
		...
		if (!isExpectedDirection)
			throw new InvalidDataException(
				$"Velodrome transaction '{swap.TransactionHash}' " +
	...
	private static bool IsSuccessful(VelodromeRpcReceipt receipt)
