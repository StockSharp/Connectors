...
public partial class WazirXMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (PrivatePollingInterval <= TimeSpan.Zero)
			throw new InvalidOperationException(
				"WazirX private polling interval must be " +
					"positive.");
		...
		try
		{
			...
			if (markets is not { Length: > 0 })
				throw new InvalidDataException(
					"WazirX returned no spot markets.");
			RegisterMarkets(markets, tickers);
			if (RestClient.IsCredentialsAvailable)
				await RestClient.SynchronizeTimeAsync(
					cancellationToken);
		...
		CancellationToken cancellationToken)
	{
		...
		if (_restClient?.IsCredentialsAvailable != true ||
			...
			return;
		if (CurrentTime >= _authRefreshAt)
		{
			try
			{
				await RefreshAuthKeyAsync(cancellationToken);
			}
		...
		if (CurrentTime - _lastPrivatePoll <
	...
	private async ValueTask RefreshAuthKeyAsync(
		CancellationToken cancellationToken)
	{
		EnsurePrivateReady();
		var token = await RestClient.CreateAuthTokenAsync(
			cancellationToken);
		WsClient.SetAuthKey(token.Key);
		var refresh = TimeSpan.FromTicks(
			token.Lifetime.Ticks / 2);
		if (refresh > TimeSpan.FromMinutes(10))
			refresh = TimeSpan.FromMinutes(10);
		if (refresh < TimeSpan.FromMinutes(1))
			refresh = TimeSpan.FromMinutes(1);
		_authRefreshAt = CurrentTime + refresh;
	}

	private async ValueTask EnsureAuthKeyAsync(
		CancellationToken cancellationToken)
	{
		if (_authRefreshAt == default ||
			CurrentTime >= _authRefreshAt)
			await RefreshAuthKeyAsync(cancellationToken);
	}

	private async ValueTask OnWebSocketMessageAsync(
		WazirXWsMessage message,
		CancellationToken cancellationToken)
	{
	...
	private void UnsubscribeClientEvents(WazirXWsClient client)
	{
