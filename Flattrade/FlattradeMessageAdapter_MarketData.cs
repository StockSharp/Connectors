...
public partial class FlattradeMessageAdapter
{
	...
	private readonly SynchronizedDictionary<string, FlattradeInstrument> _instruments = new(StringComparer.OrdinalIgnoreCase);
	private readonly SynchronizedDictionary<string, FlattradeMarketUpdate> _marketStates = new(StringComparer.OrdinalIgnoreCase);
	...
	protected override ValueTask OnMarketDepthSubscriptionAsync(MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (depth is < 1 or > 5)
			throw new ArgumentOutOfRangeException(nameof(mdMsg.MaxDepth), depth, "Flattrade provides five market-depth levels.");
	...
	protected override async ValueTask OnTFCandlesSubscriptionAsync(MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!mdMsg.IsSubscribe)
			return;
		if (!mdMsg.IsHistoryOnly())
			throw new NotSupportedException("Flattrade provides historical candles only; realtime candle subscriptions are not available.");
		...
		IEnumerable<FlattradeCandle> ordered = candles
	...
	private async Task<FlattradeInstrument> GetInstrument(string instrumentKey, CancellationToken cancellationToken)
	{
		...
		instrument = await _restClient.GetInstrument(instrumentKey, cancellationToken)
			?? throw new InvalidOperationException($"Flattrade instrument '{instrumentKey}' was not found in the official security master.");
		_instruments[instrumentKey] = instrument;
		return instrument;
	}

	private async ValueTask OnMarketDataReceived(FlattradeMarketUpdate update, CancellationToken cancellationToken)
	{
		...
		if (subscriptions.TryGetValue(DataType.Level1, out var level1Id))
		{
			...
			.TryAdd(Level1Fields.PriceStep, Positive(state.TickSize))
			.TryAdd(Level1Fields.VolumeStep, Positive(state.LotSize))
