...
static class Extensions
{
	...
	public static (string exchange, string token) ParseInstrumentKey(this string key)
	{
		...
		if (parts?.Length != 2 || parts[0].IsEmpty() || parts[1].IsEmpty())
			throw new FormatException($"Invalid Flattrade instrument key '{key}'.");
		parts[0].ToBoardCode();
	...
	public static string ToInstrumentKey(this SecurityId securityId)
	{
		...
		throw new InvalidOperationException("Flattrade token is missing. Select the security through Flattrade lookup so SecurityId.Native contains exchange|token.");
	}
	...
	public static string ToBoardCode(this string exchange)
		=> exchange?.ToUpperInvariant() switch
		{
			...
			"CDS" => "CDS",
			"MCX" => "MCX",
			_ => throw new ArgumentOutOfRangeException(nameof(exchange), exchange, "Unsupported Flattrade exchange segment."),
		};

	public static SecurityId ToSecurityId(this FlattradeInstrument instrument)
	...
	public static SecurityTypes ToSecurityType(this FlattradeInstrument instrument)
	{
	...
	public static string ToNative(this FlattradeProducts product)
		=> product switch
		{
			FlattradeProducts.Delivery => "C",
			FlattradeProducts.Intraday => "I",
			FlattradeProducts.Normal => "M",
			FlattradeProducts.MarginTrading => "F",
			FlattradeProducts.Cover => "H",
			FlattradeProducts.Bracket => "B",
			_ => throw new ArgumentOutOfRangeException(nameof(product), product, null),
		};

	public static FlattradeProducts ToProduct(this string product)
		=> product?.ToUpperInvariant() switch
		{
			"I" => FlattradeProducts.Intraday,
			"M" => FlattradeProducts.Normal,
			"F" => FlattradeProducts.MarginTrading,
			"H" => FlattradeProducts.Cover,
			"B" => FlattradeProducts.Bracket,
			_ => FlattradeProducts.Delivery,
		};
	...
	public static OrderTypes ToOrderType(this FlattradeOrder order)
	...
	public static DateTime? ToFlattradeTime(this string value)
	{
	...
	public static DateTime? GetCandleTime(this FlattradeCandle candle)
		=> candle.EpochTime.ToFlattradeTime() ?? candle.Time.ToFlattradeTime();

	public static DateTime GetMarketTime(this FlattradeMarketUpdate update)
		=> update.FeedTime.ToFlattradeTime() ?? update.LastTradeTime.ToFlattradeTime() ?? DateTime.UtcNow;

	public static void Apply(this FlattradeMarketUpdate state, FlattradeMarketUpdate update)
	{
	...
	public static FlattradeDepthLevel[] GetBids(this FlattradeMarketUpdate state)
		=> CreateDepth(
	...
	public static FlattradeDepthLevel[] GetAsks(this FlattradeMarketUpdate state)
		=> CreateDepth(
	...
	private static FlattradeDepthLevel[] CreateDepth(params (string price, string volume, string orders)[] values)
		=> [.. values
			.Select(v => new FlattradeDepthLevel
			{
