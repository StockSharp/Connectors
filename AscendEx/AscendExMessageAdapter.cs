...
public partial class AscendExMessageAdapter
{
	...
	private readonly Dictionary<string, AscendExSymbol> _marketsBySecurity =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, AscendExSymbol> _marketsByNative =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private AscendExRestClient _restClient;
	private AscendExWsClient _wsClient;
	...
	/// <see cref="AscendExMessageAdapter"/>.
	/// </summary>
	public AscendExMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.AscendEx];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.AscendEx) ||
			securityId.IsAssociated(BoardCodes.AscendEx);

	private AscendExRestClient RestClient
		=> _restClient ?? throw new InvalidOperationException(
			LocalizedStrings.ConnectionNotOk);

	private AscendExWsClient WsClient
	...
	private AscendExSymbol GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.AscendEx) &&
			!securityId.IsAssociated(BoardCodes.AscendEx))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not AscendEx.");
		...
		using (_sync.EnterScope())
		{
			...
			try
			{
				var normalized = requested.ToAscendExSecurityCode();
		...
		throw new InvalidOperationException(
			$"Unknown AscendEx market '{requested}'.");
	}

	private AscendExSymbol GetMarket(string nativeSymbol)
	{
		using (_sync.EnterScope())
	...
	private void RegisterMarkets(IEnumerable<AscendExSymbol> markets)
	{
		using (_sync.EnterScope())
		{
	...
	private static string CreatePublicTradeId(AscendExTrade trade)
		=> trade.Id > 0
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"AscendEx operation requires an exchange order ID.");
	}
