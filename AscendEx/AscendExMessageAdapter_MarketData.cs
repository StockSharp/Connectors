...
public partial class AscendExMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"AscendEx does not expose historical Level1 events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"AscendEx does not expose historical order-book events.");

		var market = GetMarket(mdMsg.SecurityId);
		var depth = (mdMsg.MaxDepth ?? 100).Min(500).Max(1);
		var streamDepth = AscendExRestClient.NormalizeDepth(depth);
		if (mdMsg.IsHistoryOnly())
		{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!AscendExExtensions.TimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"AscendEx does not support the {timeFrame} " +
					"candle interval.");

		var resolution = timeFrame.ToAscendExResolution();
		if (mdMsg.From is not null || mdMsg.To is not null ||
			mdMsg.Count is not null || mdMsg.IsHistoryOnly())
		{
			var to = (mdMsg.To ?? DateTime.UtcNow).ToUtc();
			...
			{
				this.AddWarningLog(
					"AscendEx REST candle history is " +
	...
	private SecurityMessage CreateSecurity(AscendExSymbol market,
	...
	private async ValueTask SendLevel1SnapshotAsync(
		AscendExSymbol market, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		if (ticker is null)
			throw new InvalidDataException(
				$"AscendEx returned no ticker for '{market.Pair}'.");
	...
	private Level1ChangeMessage CreateLevel1Message(
		AscendExSymbol market, AscendExTicker ticker, long transactionId)
		=> new Level1ChangeMessage
		{
			SecurityId = market.ToStockSharp(),
			ServerTime = ticker.Timestamp > 0
				? ticker.Timestamp.FromAscendExMilliseconds()
				: CurrentTime,
	...
	private async ValueTask OnWebSocketTickerAsync(
		AscendExTicker ticker, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketOrderBookAsync(
		AscendExOrderBook book, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketTradesAsync(
		AscendExTradePush push, CancellationToken cancellationToken)
	{
	...
	private ValueTask SendDepthAsync(string securityCode,
		AscendExOrderBook book, long transactionId, int depth,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new QuoteChangeMessage
		{
			SecurityId = securityCode.ToAscendExSecurityId(),
	...
	private ValueTask SendPublicTradeAsync(string securityCode,
		AscendExTrade trade, string tradeId, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(string securityCode,
		AscendExBar candle, TimeSpan timeFrame, long transactionId,
		CancellationToken cancellationToken)
	{
