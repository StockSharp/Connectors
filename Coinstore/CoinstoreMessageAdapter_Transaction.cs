...
public partial class CoinstoreMessageAdapter
{
	/// <inheritdoc />
		...
		CancellationToken cancellationToken)
	{
		...
		if (volume <= 0)
			throw new InvalidOperationException(
				"Coinstore order volume must be positive.");
		if (regMsg.VisibleVolume is > 0 &&
			regMsg.VisibleVolume != volume)
			throw new NotSupportedException(
				"Coinstore does not document iceberg orders.");
		if (regMsg.TillDate is not null)
			throw new NotSupportedException(
				"Coinstore does not document GTD orders.");
		if (regMsg.TimeInForce == TimeInForce.CancelBalance)
			throw new NotSupportedException(
				"Coinstore spot API does not consistently document " +
					"fill-or-kill orders.");
		...
		var condition = regMsg.Condition as CoinstoreOrderCondition;
		if (orderType == OrderTypes.Limit &&
			regMsg.Price <= 0)
			throw new InvalidOperationException(
				"Coinstore limit orders require a positive price.");
		var postOnly = regMsg.PostOnly == true ||
			condition?.PostOnly == true;
		if (postOnly && orderType != OrderTypes.Limit)
			throw new NotSupportedException(
				"Coinstore post-only execution is available only " +
					"for limit orders.");
		var immediateOrCancel =
			regMsg.TimeInForce == TimeInForce.MatchOrCancel ||
			condition?.TimeInForce ==
				CoinstoreTimeInForce.ImmediateOrCancel;
		if (postOnly && immediateOrCancel)
			throw new NotSupportedException(
				"Coinstore post-only and IOC cannot be combined.");

		var clientId = CoinstoreExtensions.CreateClientId(
			...
			regMsg.Side == Sides.Buy)
		{
			...
			if (ticker?.LastPrice is not > 0)
				throw new InvalidDataException(
					"Coinstore returned no price for a market buy.");
			quoteAmount = (volume * ticker.LastPrice.Value).ToWire();
		}
		var result = await RestClient.PlaceOrderAsync(
			market.Pair,
			new CoinstorePlaceOrderRequest
			{
				Market = market.Pair,
				Side = regMsg.Side.ToCoinstore(),
				Volume = orderType == OrderTypes.Market &&
					regMsg.Side == Sides.Buy
					? null
					: volume.ToWire(),
		...
		if (result?.OrderId.IsEmpty() != false)
			throw new InvalidDataException(
				"Coinstore accepted an order without returning " +
					"its identifier.");
		...
		CancellationToken cancellationToken)
	{
		...
		var market = cancelMsg.SecurityId.SecurityCode.IsEmpty()
			? tracked is null
				? throw new InvalidOperationException(
					"Coinstore cancellation requires a security ID " +
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"Coinstore does not provide an atomic order-replace " +
				"operation.");
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (cancelMsg.Mode.HasFlag(
			...
			throw new NotSupportedException(
				"Coinstore spot bulk cancellation cannot close positions.");
		var market = cancelMsg.SecurityId.SecurityCode.IsEmpty()
			? null
			: GetMarket(cancelMsg.SecurityId);
		...
		CancellationToken cancellationToken)
	{
		...
		if (requestedOrderId is not null)
		{
			if (markets.Length == 0)
			{
				...
				if (tracked is null)
					throw new InvalidOperationException(
						"Coinstore order lookup requires a security ID " +
							"for an untracked order.");
				markets =
				[
					GetMarket(new SecurityId
		...
		CancellationToken cancellationToken)
	{
		...
		CoinstoreSymbol[] trackedMarkets;
		using (_sync.EnterScope())
	...
	private ValueTask SendBalanceAsync(CoinstoreBalance balance,
		long originalTransactionId,
		CancellationToken cancellationToken)
	{
	...
	private ValueTask SendOrderAsync(CoinstoreOrder order,
		long originalTransactionId,
		CancellationToken cancellationToken)
	{
		...
		var execution = new ExecutionMessage
		{
			...
			Condition =
				order.Type.EqualsIgnoreCase("POST_ONLY") ||
				order.TimeInForce.EqualsIgnoreCase("IOC")
					? new CoinstoreOrderCondition
					{
						PostOnly = order.Type.EqualsIgnoreCase(
							"POST_ONLY"),
						TimeInForce = order.TimeInForce
							.EqualsIgnoreCase("IOC")
								? CoinstoreTimeInForce
									.ImmediateOrCancel
								: CoinstoreTimeInForce
									.GoodTillCanceled,
					}
		...
		CancellationToken cancellationToken)
	{
		var execution = new ExecutionMessage
		{
			DataTypeEx = DataType.Transactions,
			HasOrderInfo = true,
			SecurityId = tracked.SecurityCode.ToCoinstoreSecurityId(),
			ServerTime = CurrentTime,
			PortfolioName = GetPortfolioName(),
			Side = tracked.Side,
			...
			PostOnly = postOnly,
			Condition = postOnly
				? new CoinstoreOrderCondition { PostOnly = true }
				: null,
		};
	...
	private async ValueTask SendPrivateTradesAsync(
		CoinstoreSymbol market,
		IEnumerable<CoinstorePrivateTrade> trades,
		long originalTransactionId,
		bool onlyNew,
		CancellationToken cancellationToken)
	{
	...
	private CoinstoreSymbol[] GetStatusMarkets(OrderStatusMessage filter)
	{
	...
	private CoinstoreSymbol[] GetAllMarkets()
	{
		using (_sync.EnterScope())
			return [.. _marketsBySecurity.Values];
	}

	private async ValueTask<CoinstoreOrder[]> GetOpenOrdersAsync(
		IEnumerable<CoinstoreSymbol> markets,
		CancellationToken cancellationToken)
	{
		var orders = new List<CoinstoreOrder>();
		foreach (var market in (markets ?? [])
	...
	private bool MatchesOrder(
		CoinstoreOrder order, OrderStatusMessage filter)
	{
		if (order?.Id.IsEmpty() != false || order.Pair.IsEmpty())
			return false;
	...
	private DateTime GetOrderTime(CoinstoreOrder order)
	{
		var timestamp = GetOrderTimestamp(order);
		return timestamp > 0
			? timestamp.FromCoinstoreMilliseconds()
			: CurrentTime;
	}

	private static long GetOrderTimestamp(CoinstoreOrder order)
	...
	private DateTime GetTradeTime(CoinstorePrivateTrade trade)
	{
	...
	private static long GetTradeTimestamp(CoinstorePrivateTrade trade)
