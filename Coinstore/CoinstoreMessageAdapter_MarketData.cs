...
public partial class CoinstoreMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Coinstore does not expose historical Level1 events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Coinstore does not expose historical order-book events.");

		var market = GetMarket(mdMsg.SecurityId);
		var depth = (mdMsg.MaxDepth ?? 100).Min(100).Max(1);
		var streamDepth = CoinstoreRestClient.NormalizeDepth(depth);
		if (mdMsg.IsHistoryOnly())
		{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!CoinstoreExtensions.TimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"Coinstore does not support the {timeFrame} " +
					"candle interval.");

		var resolution = timeFrame.ToCoinstoreResolution();
		if (mdMsg.From is not null || mdMsg.To is not null ||
			mdMsg.Count is not null || mdMsg.IsHistoryOnly())
		{
			var to = (mdMsg.To ?? DateTime.UtcNow).ToUtc();
			var requested = mdMsg.Count?.Min(2000).Max(1).To<int>() ??
			...
			{
				this.AddWarningLog(
					"Coinstore REST candle history is " +
	...
	private SecurityMessage CreateSecurity(CoinstoreSymbol market,
		...
		=> new()
		{
			...
			PriceStep = CoinstoreExtensions.GetStep(
				market.QuotePrecision),
			VolumeStep = CoinstoreExtensions.GetStep(
	...
	private async ValueTask SendLevel1SnapshotAsync(
		CoinstoreSymbol market, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		if (ticker is null)
			throw new InvalidDataException(
				$"Coinstore returned no ticker for '{market.Pair}'.");
	...
	private Level1ChangeMessage CreateLevel1Message(
		CoinstoreSymbol market, CoinstoreTicker ticker, long transactionId)
		=> new Level1ChangeMessage
		{
			SecurityId = market.ToStockSharp(),
			ServerTime = ticker.Timestamp > 0
				? ticker.Timestamp.FromCoinstoreMilliseconds()
				: CurrentTime,
	...
	private async ValueTask OnWebSocketTickerAsync(
		CoinstoreTicker ticker, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketOrderBookAsync(
		CoinstoreOrderBook book, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketTradesAsync(
		CoinstoreTradePush push, CancellationToken cancellationToken)
	{
	...
	private ValueTask SendDepthAsync(string securityCode,
		CoinstoreOrderBook book, long transactionId, int depth,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new QuoteChangeMessage
		{
			SecurityId = securityCode.ToCoinstoreSecurityId(),
	...
	private ValueTask SendPublicTradeAsync(string securityCode,
		CoinstoreTrade trade, string tradeId, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(string securityCode,
		CoinstoreCandle candle, TimeSpan timeFrame, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		TimeSpan timeFrame, DateTime to)
	{
		...
		return count.Max(1).Min(2000).To<int>();
	}
	...
	private static DateTime ToTimestamp(long timestamp)
		=> timestamp > 100_000_000_000
			...
			: timestamp.FromCoinstoreSeconds();
