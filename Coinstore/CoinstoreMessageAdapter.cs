...
public partial class CoinstoreMessageAdapter
{
	...
	private readonly Dictionary<string, CoinstoreSymbol> _marketsBySecurity =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, CoinstoreSymbol> _marketsByNative =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private CoinstoreRestClient _restClient;
	private CoinstoreWsClient _wsClient;
	...
	/// <see cref="CoinstoreMessageAdapter"/>.
	/// </summary>
	public CoinstoreMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.Coinstore];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Coinstore) ||
			securityId.IsAssociated(BoardCodes.Coinstore);

	private CoinstoreRestClient RestClient
		=> _restClient ?? throw new InvalidOperationException(
			LocalizedStrings.ConnectionNotOk);

	private CoinstoreWsClient WsClient
	...
	private CoinstoreSymbol GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Coinstore) &&
			!securityId.IsAssociated(BoardCodes.Coinstore))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not Coinstore.");
		...
		using (_sync.EnterScope())
		{
			...
			try
			{
				var normalized = requested.ToCoinstoreSecurityCode();
		...
		throw new InvalidOperationException(
			$"Unknown Coinstore market '{requested}'.");
	}

	private CoinstoreSymbol GetMarket(string nativeSymbol)
	{
		using (_sync.EnterScope())
	...
	private void RegisterMarkets(IEnumerable<CoinstoreSymbol> markets)
	{
		using (_sync.EnterScope())
		{
	...
	private static string CreatePublicTradeId(CoinstoreTrade trade)
		=> trade.Id > 0
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"Coinstore operation requires an exchange order ID.");
	}
