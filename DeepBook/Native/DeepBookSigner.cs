...
sealed class DeepBookSigner : IDisposable
{
	...
	public DeepBookSigner(string walletAddress, SecureString privateKey)
	{
		...
		if (!keyText.IsEmpty())
		{
				...
				walletAddress.NormalizeSuiAddress() != derivedAddress)
			{
				...
				throw new ArgumentException(
					"The DeepBook wallet address does not match the private key.",
	...
	public byte[] SignTransaction(byte[] transactionBytes)
	{
		...
		if (!IsSigningAvailable || _publicKey is not { Length: 32 })
			throw new InvalidOperationException(
				"A Sui Ed25519 private key is required to sign a DeepBook swap.");
	...
	private static byte[] DecodePrivateKey(string value)
	{
		if (value.StartsWith("suiprivkey1", StringComparison.OrdinalIgnoreCase))
		{
			...
			try
			{
				if (decoded.Prefix != "suiprivkey" ||
					...
					throw new ArgumentException(
						"DeepBook requires a suiprivkey Ed25519 private key.",
						nameof(value));
		...
		if (value.Length != 64 || value.Any(static ch => !Uri.IsHexDigit(ch)))
			throw new ArgumentException(
				"DeepBook private key must be a suiprivkey Ed25519 key or 32 " +
