...
public partial class DeepBookMessageAdapter
{
		...
		OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		if (regMsg.OrderType is not (null or OrderTypes.Market))
			throw new NotSupportedException(
				"DeepBook swaps are immediate market operations.");
		if (regMsg.Condition is not null)
			throw new NotSupportedException(
				"Direct DeepBook swaps do not expose conditional orders.");
		if (regMsg.PostOnly == true)
			throw new NotSupportedException(
				"Post-only is not applicable to a DeepBook swap.");
		if (regMsg.TimeInForce is not null)
			throw new NotSupportedException(
				"Time-in-force is not applicable to a DeepBook swap.");
		if (!regMsg.UserOrderId.IsEmpty())
			throw new NotSupportedException(
				"The Sui transaction digest is the DeepBook order identifier.");
		var volume = regMsg.Volume.Abs();
		if (volume <= 0)
			throw new InvalidOperationException(
				"DeepBook order volume must be positive.");
		var baseAmount = volume.ToBaseUnits(market.BaseToken.Decimals);
		if (baseAmount == 0)
			throw new InvalidOperationException(
				"DeepBook order volume rounded to zero base units.");

		await _transactionGate.WaitAsync(cancellationToken);
		try
		{
			var quote = await ApiClient.GetQuoteAsync(market, regMsg.Side,
				volume, OrderBookDepth, cancellationToken);
			var slippageBasisPoints = decimal.Round(
				SlippageTolerance * 100m, 0,
				MidpointRounding.AwayFromZero).To<int>();
			var inputAmount = regMsg.Side == Sides.Sell
				? quote.InputAmount
				: quote.InputAmount.ApplyMaximumSlippage(
					slippageBasisPoints);
			var minimumOutput = quote.OutputAmount.ApplyMinimumSlippage(
				slippageBasisPoints);
			if (inputAmount == 0 || minimumOutput == 0)
				throw new InvalidOperationException(
					"The configured slippage makes the protected swap amount zero.");

			var pool = await SuiClient.GetSharedObjectAsync(market.PoolId, true,
				cancellationToken);
			market.PoolInitialVersion = pool.InitialVersion;
			var prepared = await SuiClient.PrepareSwapAsync(PackageId, market,
				quote, inputAmount, minimumOutput, pool, _clock,
				cancellationToken);
			var receipt = await SuiClient.ExecuteSwapAsync(prepared,
				cancellationToken);
			if (!receipt.IsSuccessful)
				throw new InvalidOperationException(
					$"DeepBook swap failed: {receipt.Error}");
		...
		OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"An executed DeepBook swap cannot be replaced.");
	}
		...
		OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"DeepBook swaps are final after Sui execution.");
	}
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"DeepBook has no cancellable order group.");
	}
		...
		OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (statusMsg.OrderId is not null)
			throw new NotSupportedException(
				"DeepBook orders use Sui transaction digests, not numeric IDs.");
		if (!statusMsg.UserId.IsEmpty())
			throw new NotSupportedException(
				"DeepBook has no exchange-side user identifier.");
		if (statusMsg.SecurityIds.Length > 0)
			throw new NotSupportedException(
				"Use the primary security filter for DeepBook order status.");
	...
	private static string GetTokenSecurityCode(DeepBookToken token,
		DeepBookToken[] tokens)
	{
