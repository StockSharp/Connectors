...
/// <summary>
/// QUIK DropCopy FIX protocol dialect.
/// </summary>
[MediaIcon(Media.MediaNames.quik)]
[Display(
	ResourceType = typeof(LocalizedStrings),
	Name = LocalizedStrings.QuikDropCopyKey,
	GroupName = LocalizedStrings.RussiaKey)]
public class QuikDropCopyFixDialect : BaseFixDialect
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="QuikDropCopyFixDialect"/>.
	/// </summary>
	...
	protected override async ValueTask<string> OnWriteAsync(IFixWriter writer, Message message, CancellationToken cancellationToken)
	{
		switch (message.Type)
		{
			case MessageTypes.OrderRegister:
			{
				...
				await writer.WriteAsync("QO-" + regMsg.TransactionId, cancellationToken);
				...
				await WriteSecurityIdAsync(writer, regMsg.SecurityId, false, cancellationToken);
			...
			case MessageTypes.OrderCancel:
			{
				...
				await writer.WriteAsync(FixTags.ClOrdID, cancellationToken);
				await writer.WriteAsync("QO-" + cancelMsg.TransactionId + ".CLX", cancellationToken);

				await writer.WriteAsync(FixTags.OrigClOrdID, cancellationToken);
				await writer.WriteAsync("QO-" + cancelMsg.OriginalTransactionId, cancellationToken);
				...
				//	await writer.WriteAsync(cancelMsg.Side.Value.ToFix(), cancellationToken);
				//}

				//if (!cancelMsg.ClientCode.IsEmpty())
				//{
				//	await writer.WriteAsync(FixTags.ClientID, cancellationToken);
				//	await writer.WriteAsync(cancelMsg.ClientCode, cancellationToken);
				//}

				//await WriteSecurityIdAsync(writer, cancelMsg.SecurityId, false, cancellationToken);

				//await WriteCurrencyAsync(writer, cancelMsg, cancellationToken);
			...
			case MessageTypes.PortfolioLookup:
			{
				...
				await writer.WriteAsync((int)SubscriptionRequestType.SnapshotPlusUpdates, cancellationToken);
			...
			case MessageTypes.MarketData:
			{
				...
				await WriteSecurityIdAsync(writer, securityId, true, cancellationToken);
			...
			case MessageTypes.SecurityLookup:
			{
				...
				else
				{
					...
					await WriteSecurityIdAsync(writer, lookupMsg.SecurityId, true, cancellationToken);
				}
	...
	private static async ValueTask WriteSecurityIdAsync(IFixWriter writer, SecurityId securityId, bool isDestination, CancellationToken cancellationToken)
	{
		...
		await writer.WriteAsync(isDestination ? FixTags.ExDestination : FixTags.SecurityExchange, cancellationToken);
	...
	protected override SecurityStates? FromSecurityTradingStatus(int? status)
	{
		...
		{
			null => null,
			17 => (SecurityStates?)SecurityStates.Trading,
