# ByBit Connector for StockSharp

This directory contains the **ByBit** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `ByBitMessageAdapter` message adapter, exposing ByBit market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `ByBit`.

## Configuration

`ByBitMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new ByBitMessageAdapter(new IncrementalIdGenerator())
{
...
See the [ByBit connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/bybit.html).
