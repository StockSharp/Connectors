...
public partial class TokocryptoMessageAdapter
{
	...
	private readonly Dictionary<string, TokocryptoSymbol> _marketsBySecurity =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, TokocryptoSymbol> _marketsByNative =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private TokocryptoRestClient _restClient;
	private TokocryptoWsClient _wsClient;
	...
	/// <see cref="TokocryptoMessageAdapter"/>.
	/// </summary>
	public TokocryptoMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.Tokocrypto];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Tokocrypto) ||
			securityId.IsAssociated(BoardCodes.Tokocrypto);

	private TokocryptoRestClient RestClient
		=> _restClient ?? throw new InvalidOperationException(
			LocalizedStrings.ConnectionNotOk);

	private TokocryptoWsClient WsClient
	...
	private TokocryptoSymbol GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Tokocrypto) &&
			!securityId.IsAssociated(BoardCodes.Tokocrypto))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not Tokocrypto.");
		...
		using (_sync.EnterScope())
		{
			...
			try
			{
				var normalized = requested.ToTokocryptoSecurityCode();
		...
		throw new InvalidOperationException(
			$"Unknown Tokocrypto market '{requested}'.");
	}

	private TokocryptoSymbol GetMarket(string nativeSymbol)
	{
		using (_sync.EnterScope())
	...
	private void RegisterMarkets(IEnumerable<TokocryptoSymbol> markets)
	{
		using (_sync.EnterScope())
		{
			...
			foreach (var market in markets ?? [])
			{
				if (market?.Pair.IsEmpty() != false ||
					...
					continue;
				market.Pair = market.Pair.Trim().ToUpperInvariant();
				var publicSymbol =
					(market.Base + market.Quote).ToUpperInvariant();
				_marketsBySecurity[market.SecurityCode] = market;
				_marketsByNative[market.Pair] = market;
				_marketsByNative[market.Pair.ToLowerInvariant()] = market;
				_marketsByNative[publicSymbol] = market;
				_marketsByNative[publicSymbol.ToLowerInvariant()] = market;
			}
		}
	}
	...
	private static string CreatePublicTradeId(TokocryptoTrade trade)
		=> trade.Id > 0
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"Tokocrypto operation requires an exchange order ID.");
	}
