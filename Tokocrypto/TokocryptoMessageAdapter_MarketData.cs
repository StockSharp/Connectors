...
public partial class TokocryptoMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Tokocrypto does not expose historical Level1 events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Tokocrypto does not expose historical order-book events.");

		var market = GetMarket(mdMsg.SecurityId);
		var depth = (mdMsg.MaxDepth ?? 20).Min(20).Max(1);
		var streamDepth = TokocryptoRestClient.NormalizeDepth(depth);
		if (mdMsg.IsHistoryOnly())
		{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!TokocryptoExtensions.TimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"Tokocrypto does not support the {timeFrame} " +
					"candle interval.");

		var resolution = timeFrame.ToTokocryptoResolution();
		if (mdMsg.From is not null || mdMsg.To is not null ||
			mdMsg.Count is not null || mdMsg.IsHistoryOnly())
		{
			var to = (mdMsg.To ?? DateTime.UtcNow).ToUtc();
			...
			{
				this.AddWarningLog(
					"Tokocrypto REST candle history is " +
	...
	private SecurityMessage CreateSecurity(TokocryptoSymbol market,
		...
		=> new()
		{
			...
			PriceStep = market.PriceStep is > 0
				? market.PriceStep
				: TokocryptoExtensions.GetStep(
					market.QuotePrecision),
			VolumeStep = market.VolumeStep is > 0
				? market.VolumeStep
				: TokocryptoExtensions.GetStep(
	...
	private async ValueTask SendLevel1SnapshotAsync(
		TokocryptoSymbol market, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		if (ticker is null)
			throw new InvalidDataException(
				$"Tokocrypto returned no ticker for '{market.Pair}'.");
	...
	private Level1ChangeMessage CreateLevel1Message(
		TokocryptoSymbol market, TokocryptoTicker ticker, long transactionId)
		=> new Level1ChangeMessage
		{
			SecurityId = market.ToStockSharp(),
			ServerTime = ticker.Timestamp > 0
				? ticker.Timestamp.FromTokocryptoMilliseconds()
				: CurrentTime,
	...
	private async ValueTask OnWebSocketTickerAsync(
		TokocryptoTicker ticker, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketOrderBookAsync(
		TokocryptoOrderBook book, CancellationToken cancellationToken)
	{
		...
		using (_sync.EnterScope())
			subscriptions = [.. _depthSubscriptions.Where(pair =>
				pair.Value.NativeSymbol.EqualsIgnoreCase(market.Pair) &&
				(book.Limit <= 0 ||
	...
	private async ValueTask OnWebSocketTradesAsync(
		TokocryptoTradePush push, CancellationToken cancellationToken)
	{
		if (push?.Pair.IsEmpty() != false)
			return;
		var market = GetMarket(push.Pair);
		if (market is null)
			return;
		KeyValuePair<long, MarketSubscription>[] subscriptions;
		using (_sync.EnterScope())
	...
	private async ValueTask OnWebSocketKlineAsync(
		TokocryptoKlineEvent push,
		CancellationToken cancellationToken)
	{
		if (push?.Market.IsEmpty() != false || push.Kline is null)
			return;
		var market = GetMarket(push.Market);
		if (market is null)
			return;
	...
	private ValueTask SendDepthAsync(string securityCode,
		TokocryptoOrderBook book, long transactionId, int depth,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new QuoteChangeMessage
		{
			SecurityId = securityCode.ToTokocryptoSecurityId(),
	...
	private ValueTask SendPublicTradeAsync(string securityCode,
		TokocryptoTrade trade, string tradeId, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(string securityCode,
		TokocryptoCandle candle, TimeSpan timeFrame, long transactionId,
		CancellationToken cancellationToken)
	{
	...
	private static DateTime ToTimestamp(long timestamp)
		=> timestamp > 100_000_000_000
			...
			: timestamp.FromTokocryptoSeconds();
