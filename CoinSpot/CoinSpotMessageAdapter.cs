...
public partial class CoinSpotMessageAdapter
{
	...
	private readonly Dictionary<string, CoinSpotMarket>
		_marketsBySecurity =
	...
	private CoinSpotRestClient _restClient;
	...
	/// <see cref="CoinSpotMessageAdapter"/>.
	/// </summary>
	public CoinSpotMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.CoinSpot];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.CoinSpot) ||
			securityId.IsAssociated(BoardCodes.CoinSpot);

	private CoinSpotRestClient RestClient
	...
	private CoinSpotMarket GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.CoinSpot) &&
			!securityId.IsAssociated(BoardCodes.CoinSpot))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not CoinSpot.");
		...
		throw new InvalidOperationException(
			$"Unknown CoinSpot market '{requested}'.");
	}

	private CoinSpotMarket GetMarket(string nativeSymbol)
	{
		...
		using (_sync.EnterScope())
		{
			if (_marketsByNative.TryGetValue(
				...
				_marketsBySecurity.TryGetValue(
					nativeSymbol, out market))
				return market;
			try
			{
				return _marketsByNative.TryGetValue(
					nativeSymbol.ToCoinSpotSymbol(), out market)
						? market
						: null;
			}
	...
	private CoinSpotMarket[] GetMarkets()
	{
		using (_sync.EnterScope())
			return [.. _marketsBySecurity.Values];
	}

	private void RegisterMarkets(IEnumerable<CoinSpotMarket> markets)
	{
		using (_sync.EnterScope())
		{
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"CoinSpot operation requires an exchange order ID.");
	}
