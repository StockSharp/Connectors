...
public partial class CoinSpotMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		if (PollingInterval <= TimeSpan.Zero)
			throw new InvalidOperationException(
				"CoinSpot polling interval must be positive.");
		PublicEndpoint = NormalizeEndpoint(
			PublicEndpoint, _defaultPublicEndpoint);
		TradingEndpoint = NormalizeEndpoint(
			TradingEndpoint, _defaultTradingEndpoint);
		ReadOnlyEndpoint = NormalizeEndpoint(
			ReadOnlyEndpoint, _defaultReadOnlyEndpoint);

		ClearState();
		var client = new CoinSpotRestClient(
			PublicEndpoint,
			TradingEndpoint,
			ReadOnlyEndpoint,
			Key,
		...
		try
		{
			...
			if (markets is not { Length: > 0 })
				throw new InvalidDataException(
					"CoinSpot returned no spot markets.");
