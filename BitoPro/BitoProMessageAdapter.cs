...
public partial class BitoProMessageAdapter
{
	...
	private readonly Dictionary<string, BitoProSymbol> _marketsBySecurity =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, BitoProSymbol> _marketsByNative =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private BitoProRestClient _restClient;
	private BitoProWsClient _wsClient;
	...
	/// <see cref="BitoProMessageAdapter"/>.
	/// </summary>
	public BitoProMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.BitoPro];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.BitoPro) ||
			securityId.IsAssociated(BoardCodes.BitoPro);

	private BitoProRestClient RestClient
		=> _restClient ?? throw new InvalidOperationException(
			LocalizedStrings.ConnectionNotOk);

	private BitoProWsClient WsClient
	...
	private BitoProSymbol GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.BitoPro) &&
			!securityId.IsAssociated(BoardCodes.BitoPro))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not BitoPro.");
		...
		using (_sync.EnterScope())
		{
			...
			try
			{
				var normalized = requested.ToBitoProSecurityCode();
		...
		throw new InvalidOperationException(
			$"Unknown BitoPro market '{requested}'.");
	}

	private BitoProSymbol GetMarket(string nativeSymbol)
	{
		using (_sync.EnterScope())
	...
	private void RegisterMarkets(IEnumerable<BitoProSymbol> markets)
	{
		using (_sync.EnterScope())
		{
	...
	private static string CreatePublicTradeId(BitoProTrade trade)
		=> string.Join("-",
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"BitoPro operation requires an exchange order ID.");
	}
