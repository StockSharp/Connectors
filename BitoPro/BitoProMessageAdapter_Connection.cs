...
public partial class BitoProMessageAdapter
{
		...
		ConnectMessage connectMsg, CancellationToken cancellationToken)
	{
		...
		if (configured.Any(static value => value) &&
			...
			throw new InvalidOperationException(
				"BitoPro email, API key and secret must be " +
					"configured together.");
		...
		try
		{
			_restClient = new(RestEndpoint, Email, Key, Secret)
			{
				Parent = this,
			};
			...
			if (markets is not { Length: > 0 })
				throw new InvalidDataException(
					"BitoPro returned no spot markets.");
	...
	private void UnsubscribeClientEvents(BitoProWsClient client)
	{
