...
public partial class NovaDaxMessageAdapter
{
		...
		ConnectMessage connectMsg, CancellationToken cancellationToken)
	{
		...
		if (configured.Any(static value => value) &&
			...
			throw new InvalidOperationException(
				"NovaDax API key and secret must be " +
					"configured together.");
		...
		{
			_restClient = new(
				RestEndpoint, Key, Secret, AccountId)
			{
				Parent = this,
			};
			_wsClient = new(
			...
			if (markets is not { Length: > 0 })
				throw new InvalidDataException(
					"NovaDax returned no spot markets.");
	...
	private void UnsubscribeClientEvents(NovaDaxWsClient client)
	{
