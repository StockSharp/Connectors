...
public partial class NovaDaxMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"NovaDax does not expose historical Level1 events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"NovaDax does not expose historical order-book events.");

		var market = GetMarket(mdMsg.SecurityId);
		var depth = (mdMsg.MaxDepth ?? 50).Min(50).Max(1);
		var streamDepth = NovaDaxRestClient.NormalizeDepth(depth);
		if (mdMsg.IsHistoryOnly())
		{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!NovaDaxExtensions.TimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"NovaDax does not support the {timeFrame} " +
					"candle interval.");

		if (!mdMsg.IsHistoryOnly())
			throw new NotSupportedException(
				"NovaDAX publishes candles through REST history only.");

		var resolution = timeFrame.ToNovaDaxResolution();
		var to = (mdMsg.To ?? DateTime.UtcNow).ToUtc();
		var requested = mdMsg.Count?.Min(3000).Max(1).To<int>() ??
	...
	private SecurityMessage CreateSecurity(NovaDaxSymbol market,
	...
	private async ValueTask SendLevel1SnapshotAsync(
		NovaDaxSymbol market, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		if (ticker is null)
			throw new InvalidDataException(
				$"NovaDax returned no ticker for '{market.Pair}'.");
	...
	private Level1ChangeMessage CreateLevel1Message(
		NovaDaxSymbol market, NovaDaxTicker ticker, long transactionId)
		=> new Level1ChangeMessage
		{
			SecurityId = market.ToStockSharp(),
			ServerTime = ticker.Timestamp > 0
				? ticker.Timestamp.FromNovaDaxMilliseconds()
				: CurrentTime,
	...
	private async ValueTask OnWebSocketTickerAsync(
		NovaDaxTicker ticker, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketOrderBookAsync(
		NovaDaxOrderBook book, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketTradesAsync(
		NovaDaxTradePush push, CancellationToken cancellationToken)
	{
	...
	private ValueTask SendDepthAsync(string securityCode,
		NovaDaxOrderBook book, long transactionId, int depth,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new QuoteChangeMessage
		{
			SecurityId = securityCode.ToNovaDaxSecurityId(),
	...
	private ValueTask SendPublicTradeAsync(string securityCode,
		NovaDaxTrade trade, string tradeId, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(string securityCode,
		NovaDaxCandle candle, TimeSpan timeFrame, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		TimeSpan timeFrame, DateTime to)
	{
		...
		return count.Max(1).Min(3000).To<int>();
	}
