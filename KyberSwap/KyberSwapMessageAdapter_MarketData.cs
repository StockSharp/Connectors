...
public partial class KyberSwapMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"KyberSwap executable quotes do not expose historical Level1 " +
				"events.");
	...
	private SecurityMessage CreateSecurity(KyberSwapMarket market,
	...
	private async ValueTask SendLevel1Async(KyberSwapMarket market,
	...
	private async ValueTask<(decimal Bid, decimal BidVolume, decimal Ask,
		decimal AskVolume)> LoadLevel1Async(KyberSwapMarket market,
		CancellationToken cancellationToken)
	{
		...
		if (bid <= 0 || ask <= 0 || askBaseVolume <= 0)
			throw new InvalidDataException(
				"KyberSwap returned a non-positive executable quote.");
	...
	private async ValueTask PollMarketAsync(
		CancellationToken cancellationToken)
	{
		(KyberSwapMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
