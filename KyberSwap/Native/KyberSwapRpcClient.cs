...
sealed class KyberSwapRpcClient : BaseLogReceiver
{
	...
	public KyberSwapRpcClient(string endpoint, KyberSwapChains chain,
		string walletAddress, SecureString privateKey)
	{
		...
		else
		{
			WalletAddress = walletAddress.IsEmpty()
				? KyberSwapExtensions.ProbeAddress
				: walletAddress.NormalizeAddress();
		}
	...
	public override string Name => "KyberSwap_JSON-RPC";

	public KyberSwapChains Chain { get; }
	...
	public bool IsWalletConfigured =>
		!WalletAddress.EqualsIgnoreCase(KyberSwapExtensions.ProbeAddress);
	...
	{
		var chainId = (await SendAsync<KyberSwapRpcEmptyParameters, string>(
			"eth_chainId", new(), true, cancellationToken)).ParseInteger();
		if (chainId != new BigInteger((int)Chain))
			throw new InvalidOperationException(
				$"JSON-RPC is connected to chain {chainId}, but KyberSwap " +
				$"chain {(int)Chain} is configured.");
	}

	public async ValueTask<KyberSwapToken> GetTokenAsync(string address,
	...
	public async ValueTask<BigInteger> GetBalanceAsync(KyberSwapToken token,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
			return (await SendAsync<KyberSwapRpcAddressTagParameters, string>(
				"eth_getBalance", new()
				{
					Address = WalletAddress,
					BlockTag = "latest",
				}, true, cancellationToken)).ParseInteger();
		var data = KyberSwapExtensions.EncodeStaticCall(
			"balanceOf(address)",
			KyberSwapExtensions.AbiAddress(WalletAddress));
	...
	public async ValueTask<BigInteger> GetAllowanceAsync(
		KyberSwapToken token, string spender,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		var data = KyberSwapExtensions.EncodeStaticCall(
			...
			KyberSwapExtensions.AbiAddress(WalletAddress),
			KyberSwapExtensions.AbiAddress(spender));
		return (await CallContractAsync(token.Address, data,
			cancellationToken)).ParseInteger();
	}

	public KyberSwapTransaction CreateApprovalTransaction(
		KyberSwapToken token, string spender, BigInteger amount)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		return new()
		{
			...
			Data = KyberSwapExtensions.EncodeStaticCall(
				"approve(address,uint256)",
				KyberSwapExtensions.AbiAddress(spender),
				KyberSwapExtensions.AbiWord(amount)),
			Value = BigInteger.Zero,
		};
	}

	public async ValueTask<BigInteger> EstimateGasAsync(
		KyberSwapTransaction transaction,
		CancellationToken cancellationToken)
	{
		ValidateTransaction(transaction);
		var call = ToRpcCall(transaction);
		return (await SendAsync<KyberSwapRpcCallOnlyParameters, string>(
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			var nonce = (await SendAsync<KyberSwapRpcAddressTagParameters,
			...
			else
			{
				var gasPrice = (await SendAsync<
					KyberSwapRpcEmptyParameters, string>("eth_gasPrice",
			...
			var hash = await SendAsync<KyberSwapRpcValueParameters, string>(
	...
	public async ValueTask<KyberSwapRpcReceipt> GetReceiptAsync(string hash,
		CancellationToken cancellationToken)
	{
		hash = hash.NormalizeHash();
		var receipt = await SendAsync<KyberSwapRpcValueParameters,
			KyberSwapRpcReceipt>("eth_getTransactionReceipt", new()
			{
				Value = hash,
	...
	public async ValueTask<KyberSwapRpcReceipt> WaitForReceiptAsync(
	...
	public async ValueTask<KyberSwapRpcBlock> GetBlockAsync(
		BigInteger blockNumber, CancellationToken cancellationToken)
	{
		if (blockNumber < 0)
			throw new ArgumentOutOfRangeException(nameof(blockNumber));
		var block = await SendAsync<KyberSwapRpcTagBooleanParameters,
			KyberSwapRpcBlock>("eth_getBlockByNumber", new()
			{
	...
	public async ValueTask VerifyContractAsync(string address, string name,
		CancellationToken cancellationToken)
	{
		var code = await SendAsync<KyberSwapRpcAddressTagParameters, string>(
			"eth_getCode", new()
			{
				Address = address.NormalizeAddress(),
				BlockTag = "latest",
		...
		if (code.IsEmpty() || code.EqualsIgnoreCase("0x") ||
			...
			throw new InvalidDataException(
				$"KyberSwap {name} '{address}' is not a contract on " +
				$"chain {(int)Chain}.");
	}
		...
		TryGetEip1559FeesAsync(CancellationToken cancellationToken)
	{
		try
		{
			var block = await SendAsync<
				KyberSwapRpcTagBooleanParameters, KyberSwapRpcBlock>(
			...
			var priority = (await SendAsync<
				KyberSwapRpcEmptyParameters, string>(
	...
	private async ValueTask<string> CallContractAsync(string address,
		...
		=> await SendAsync<KyberSwapRpcCallParameters, string>("eth_call",
			new()
			{
				Call = new()
				{
		...
		where TParameters : KyberSwapRpcParameters
	{
		...
		var payload = JsonConvert.SerializeObject(
			new KyberSwapRpcRequest<TParameters>
			{
				Id = requestId,
				Method = method,
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			if (!response.IsSuccessStatusCode)
				throw new InvalidOperationException(
					...
					Truncate(body));
			KyberSwapRpcResponse<TResult> rpc;
			try
			{
				rpc = JsonConvert.DeserializeObject<
					KyberSwapRpcResponse<TResult>>(body, _jsonSettings);
			}
	...
	private void ValidateTransaction(KyberSwapTransaction transaction)
	{
	...
	private KyberSwapRpcCall ToRpcCall(KyberSwapTransaction transaction)
		=> new()
		{
			From = WalletAddress,
