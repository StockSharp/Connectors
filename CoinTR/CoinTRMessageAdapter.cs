...
public partial class CoinTRMessageAdapter
{
	...
	private readonly Dictionary<string, CoinTRSymbol> _marketsBySecurity =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, CoinTRSymbol> _marketsByNative =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private CoinTRRestClient _restClient;
	private CoinTRWsClient _wsClient;
	private long _portfolioSubscriptionId;
	private long _orderStatusSubscriptionId;

	/// <summary>
	/// Initializes a new instance of the
	/// <see cref="CoinTRMessageAdapter"/>.
	/// </summary>
	public CoinTRMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.CoinTR];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.CoinTR) ||
			securityId.IsAssociated(BoardCodes.CoinTR);

	private CoinTRRestClient RestClient
		=> _restClient ?? throw new InvalidOperationException(
			LocalizedStrings.ConnectionNotOk);

	private CoinTRWsClient WsClient
	...
	private CoinTRSymbol GetMarket(SecurityId securityId)
	{
		if (!securityId.BoardCode.IsEmpty() &&
			!securityId.BoardCode.EqualsIgnoreCase(BoardCodes.CoinTR) &&
			!securityId.IsAssociated(BoardCodes.CoinTR))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not CoinTR.");
		...
		throw new InvalidOperationException(
			$"Unknown CoinTR market '{requested}'.");
	}

	private CoinTRSymbol GetMarket(string nativeSymbol)
	{
		using (_sync.EnterScope())
	...
	private void RegisterMarkets(IEnumerable<CoinTRSymbol> markets)
	{
		using (_sync.EnterScope())
		{
		...
		string stringOrderId)
	{
		...
		throw new InvalidOperationException(
			"CoinTR operation requires an exchange order ID.");
	}
