# Digifinex Connector for StockSharp

This directory contains the **Digifinex** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `DigifinexMessageAdapter` message adapter, exposing Digifinex market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Digifinex`.

## Configuration

`DigifinexMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new DigifinexMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Digifinex connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/digifinex.html).
