...
sealed class CapitalFuturesStaDispatcher : IDisposable
{
	...
	public CapitalFuturesStaDispatcher()
	{
		if (!OperatingSystem.IsWindows())
			throw new PlatformNotSupportedException("Capital Futures API requires Windows and a registered SKCOM component.");

		_thread = new(Run)
		{
			IsBackground = true,
			Name = "Capital Futures SKCOM STA",
		};
	...
	public Task<T> InvokeAsync<T>(Func<T> action, CancellationToken cancellationToken)
	{
		...
		{
			if (Interlocked.CompareExchange(ref state, 2, 0) == 0)
				completion.TrySetException(new ObjectDisposedException(nameof(CapitalFuturesStaDispatcher)));
		}
