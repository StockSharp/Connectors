namespace StockSharp.Glassnode.Native;

sealed class GlassnodeApiException : InvalidOperationException
{
	public GlassnodeApiException(HttpStatusCode statusCode, int? errorCode,
		string message)
		: base(message)
	{
...
sealed class GlassnodeRestClient : BaseLogReceiver
{
	...
	public GlassnodeRestClient(string endpoint, SecureString apiKey,
		TimeSpan requestInterval)
	{
		...
		if (_apiKey.IsEmpty() || _apiKey.Length > 4096 ||
			...
			throw new ArgumentException(
				"Glassnode API key is empty or invalid.", nameof(apiKey));
		...
		_client.DefaultRequestHeaders.UserAgent.ParseAdd("StockSharp-Glassnode/1.0");
		_client.DefaultRequestHeaders.TryAddWithoutValidation("X-Api-Key", _apiKey);
	}

	public override string Name => "Glassnode_REST";

	public async ValueTask<GlassnodeAsset[]> GetAssetsAsync(
		CancellationToken cancellationToken)
		=> (await GetAsync<GlassnodeAssetsResponse>("metadata/assets",
			cancellationToken))?.Data ?? [];

	public ValueTask<GlassnodeValuePoint[]> GetClosePricesAsync(string assetId,
		GlassnodeIntervals interval, DateTime from, DateTime to,
		CancellationToken cancellationToken)
		=> GetAsync<GlassnodeValuePoint[]>(BuildMetricPath(
			"market/price_usd_close_v2", assetId, interval, from, to),
			cancellationToken);

	public ValueTask<GlassnodeOhlcPoint[]> GetOhlcAsync(string assetId,
		GlassnodeIntervals interval, DateTime from, DateTime to,
		CancellationToken cancellationToken)
		=> GetAsync<GlassnodeOhlcPoint[]>(BuildMetricPath(
			"market/price_usd_ohlc_v2", assetId, interval, from, to),
			cancellationToken);

	private static string BuildMetricPath(string metric, string assetId,
		GlassnodeIntervals interval, DateTime from, DateTime to)
	{
		assetId = GlassnodeExtensions.NormalizeAssetId(assetId);
		from = from.EnsureUtc();
		to = to.EnsureUtc();
		ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(from, to,
			nameof(from));
		return "metrics/" + metric + "?a=" + Escape(assetId) +
			"&s=" + Format(from.ToUnixSeconds()) +
			"&u=" + Format(to.ToUnixSeconds()) +
			"&i=" + Escape(interval.ToWire());
	}
		...
		try
		{
			for (var attempt = 0; ; attempt++)
			{
					...
					error is HttpRequestException or TaskCanceledException)
				{
					...
					throw new IOException("Glassnode REST transport failed: " +
					...
					if (response.IsSuccessStatusCode)
					{
						...
						catch (JsonException error)
						{
							throw new InvalidDataException(
								"Glassnode returned invalid JSON.", error);
						}
					}
	...
	private static TimeSpan? GetRetryDelay(HttpResponseMessage response)
	{
		...
		if (response.Headers.TryGetValues("x-rate-limit-reset", out var values) &&
			double.TryParse(values.FirstOrDefault(), NumberStyles.Float,
	...
	private GlassnodeApiException CreateException(HttpStatusCode statusCode,
		byte[] body)
	{
		GlassnodeErrorResponse error = null;
		if (body.Length > 0)
		{
			try
			{
				error = JsonConvert.DeserializeObject<GlassnodeErrorResponse>(
		...
		var detail = error?.Message.IsEmpty(error?.Detail)
			.IsEmpty(error?.Error).IsEmpty(error?.Status?.ErrorMessage);
		if (detail.IsEmpty())
			detail = statusCode.ToString();
		return new(statusCode, error?.Status?.ErrorCode,
			$"Glassnode REST request failed ({(int)statusCode}): {Redact(detail)}");
	}
		...
		HttpResponseMessage response, CancellationToken cancellationToken)
	{
		if (response.Content.Headers.ContentLength is long contentLength &&
			...
			throw new InvalidDataException("Glassnode response is too large.");
	...
	private static Uri ValidateEndpoint(string endpoint)
	{
		if (!Uri.TryCreate(endpoint?.Trim().TrimEnd('/') + "/",
			...
			throw new ArgumentException(
				"Glassnode endpoint must be an HTTPS API v1 root without credentials, query, or fragment.",
				nameof(endpoint));
		return uri;
	}
