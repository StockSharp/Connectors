# Paradex Connector for StockSharp

This directory contains the **Paradex** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `ParadexMessageAdapter` message adapter, exposing Paradex market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `ParadexSpot`.

## Configuration

`ParadexMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new ParadexMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Paradex connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/paradex.html).
