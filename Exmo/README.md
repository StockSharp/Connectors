# Exmo Connector for StockSharp

This directory contains the **Exmo** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `ExmoMessageAdapter` message adapter, exposing Exmo market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Exmo`.

## Configuration

`ExmoMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new ExmoMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Exmo connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/exmo.html).
