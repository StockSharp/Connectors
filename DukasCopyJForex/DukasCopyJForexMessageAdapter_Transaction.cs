...
public partial class DukasCopyJForexMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		var condition = regMsg.Condition as DukasCopyJForexOrderCondition ?? new();
		...
		CancellationToken cancellationToken)
	{
		...
		var condition = replaceMsg.Condition as DukasCopyJForexOrderCondition ?? tracker?.Condition ?? new();
	...
	private ValueTask ProcessOrderUpdate(DukasCopyJForexOrder order, CancellationToken cancellationToken)
		=> order == null ? default : ProcessOrderUpdateCore(order, cancellationToken);

	private async ValueTask ProcessOrderUpdateCore(DukasCopyJForexOrder order,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask SendOrder(DukasCopyJForexOrder order, long lookupTransactionId,
		CancellationToken cancellationToken)
	{
		...
		var condition = tracker?.Condition ?? new DukasCopyJForexOrderCondition
		{
	...
	private ValueTask ProcessAccountUpdate(DukasCopyJForexAccount account,
		CancellationToken cancellationToken)
		=> account == null || _portfolioSubscriptionId == 0
			? default
			: SendAccount(account, _portfolioSubscriptionId, false, cancellationToken);

	private async ValueTask SendAccount(DukasCopyJForexAccount account, long originalTransactionId,
		bool sendPortfolio, CancellationToken cancellationToken)
	{
		...
		if (sendPortfolio)
		{
			await SendOutMessageAsync(new PortfolioMessage
			{
				...
				BoardCode = DukasCopyJForexExtensions.BoardCode,
	...
	private ValueTask SendPosition(string symbol, IEnumerable<DukasCopyJForexOrder> orders, long originalTransactionId,
		CancellationToken cancellationToken)
	{
