...
class SpotAdapter : NativeAdapter
{
	...
	protected override string PortfolioName => base.PortfolioName + $"_{nameof(Spot)}";
	...
	public override async ValueTask RegisterOrder(OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		switch (regMsg.OrderType)
		{
			...
			{
				var condition = (CoinExOrderCondition)regMsg.Condition;
		...
		var order = await _httpClient.RegisterOrder(regMsg.TransactionId, symbol, regMsg.MarginMode is not null, regMsg.Side.ToNative(), regMsg.ToOrderType(), price, regMsg.Volume, regMsg.VisibleVolume is not null, cancellationToken);
	...
	public override async ValueTask OrderStatus(OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		//foreach (var order in await _httpClient.GetOpenOrders(true, cancellationToken))
		//{
		//	if (!long.TryParse(order.ClientId, out var transId))
		//		continue;

		//	await ProcessOrderAsync(transId, statusMsg.TransactionId, order, cancellationToken);
		//}

		foreach (var order in await _httpClient.GetOpenOrders(false, 100, cancellationToken))
		{
			if (!long.TryParse(order.ClientId, out var transId))
				continue;
	...
	private ValueTask ProcessOrderAsync(long transId, long originTransId, Order order, OrderStates state, CancellationToken cancellationToken)
	{
		return SendOutMessageAsync(new ExecutionMessage
		{
			...
			PortfolioName = PortfolioName,
			OrderState = state,
			Commission = order.BaseFee?.ToDecimal(),
		}, cancellationToken);
	}
