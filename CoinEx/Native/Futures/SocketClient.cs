...
class SocketClient : BaseLogReceiver
{
	...
	public override string Name => nameof(CoinEx) + "_" + nameof(Futures) + nameof(SocketClient);
	...
	public event Func<Position, CancellationToken, ValueTask> PositionReceived;
	...
	private int OnPreProcess(ReadOnlyMemory<byte> source, Memory<byte> destination)
		=> source.Span.UnGZip(destination.Span);
	...
	private async ValueTask OnProcess(WebSocketMessage msg, CancellationToken cancellationToken)
	{
		...
		else if (method.ContainsIgnoreCase(Channel.Position))
		{
			if (PositionReceived is { } handler)
				await handler(((JToken)data.position).DeserializeObject<Position>(), cancellationToken);
		}
		else
		{
			if (obj.message == "OK")
	...
	public ValueTask SubscribePosition(long id, CancellationToken cancellationToken)
	{
		return Send(Channel.Position, Commands.Subscribe, id, id, new { market_list = Array.Empty<string>() }, cancellationToken);
	}

	public ValueTask UnSubscribePosition(long id, long originTransId, CancellationToken cancellationToken)
	{
		return Send(Channel.Position, Commands.Unsubscribe, id, -originTransId, new { market_list = Array.Empty<string>() }, cancellationToken);
	}
