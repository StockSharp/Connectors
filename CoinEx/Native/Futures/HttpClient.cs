...
class HttpClient : BaseLogReceiver
{
	...
	public override string Name => nameof(CoinEx) + "_" + nameof(Futures) + nameof(HttpClient);

	public Task<IEnumerable<Symbol>> GetSymbols(CancellationToken cancellationToken)
	{
		return MakeRequest<IEnumerable<Symbol>>(CreateUrl("futures/market"), CreateRequest(Method.Get), cancellationToken);
	}

	public Task<IEnumerable<Ohlc>> GetCandles(string symbol, string period, int limit, CancellationToken cancellationToken)
	{
		return MakeRequest<IEnumerable<Ohlc>>(CreateUrl($"futures/kline?market={symbol}&period={period}&limit={limit}"), CreateRequest(Method.Get), cancellationToken);
	}

	public Task<Order> RegisterOrder(long clientId, string symbol, string side, string type, decimal? price, decimal volume, bool isHide, CancellationToken cancellationToken)
	{
		var url = CreateUrl("futures/order");
		var request = CreateRequest(Method.Post);

		IDictionary<string, object> body = new ExpandoObject();

		body.Add("market", symbol);
		body.Add("market_type", "FUTURES");
		body.Add("side", side);
		body.Add("type", type);
	...
	{
		var url = CreateUrl("futures/modify-order");
		var request = CreateRequest(Method.Post);
	...
	{
		var url = CreateUrl("futures/cancel-order");
	...
	public Task<IEnumerable<Order>> GetOpenOrders(int limit, CancellationToken cancellationToken)
	{
		var url = CreateUrl("futures/pending-order");
		var request = CreateRequest(Method.Get);

		request.AddParameter("market_type", "FUTURES");
	...
	{
		var url = CreateUrl("assets/futures/balance");
		var request = CreateRequest(Method.Get);

		return MakeRequest<IEnumerable<Balance>>(url, ApplySecret(request, url), cancellationToken);
	}

	public Task<IEnumerable<Position>> GetPositions(int limit, CancellationToken cancellationToken)
	{
		var url = CreateUrl("futures/pending-position");
		var request = CreateRequest(Method.Get);

		request
			.AddParameter("market_type", "FUTURES")
