...
public partial class NinjaTraderMessageAdapter
{
	private readonly SynchronizedDictionary<long, NinjaTraderAccount> _accounts = [];
	private readonly SynchronizedDictionary<long, NinjaTraderOrder> _orders = [];
	private readonly SynchronizedDictionary<long, NinjaTraderOrderVersion> _orderVersions = [];
	...
	private async Task<NinjaTraderAccount[]> EnsureAccounts(CancellationToken cancellationToken)
	{
	...
	private async Task<NinjaTraderAccount> GetAccount(string portfolioName, CancellationToken cancellationToken)
	{
		...
		return (await EnsureAccounts(cancellationToken)).FirstOrDefault(a => a.Name.EqualsIgnoreCase(portfolioName))
			?? throw new InvalidOperationException($"NinjaTrader account '{portfolioName}' was not found.");
	}
	...
	protected override async ValueTask RegisterOrderAsync(OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		var stopPrice = (regMsg.Condition as NinjaTraderOrderCondition)?.StopPrice;
		...
		var orderId = await _httpClient.PlaceOrder(new PlaceOrderRequest
		{
			...
			OrderType = orderType,
			Price = orderType is NinjaTraderOrderTypes.Market or NinjaTraderOrderTypes.Stop ? null : regMsg.Price,
			StopPrice = stopPrice,
	...
	protected override async ValueTask ReplaceOrderAsync(OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		var stopPrice = (replaceMsg.Condition as NinjaTraderOrderCondition)?.StopPrice;
		...
		await _httpClient.ModifyOrder(new ModifyOrderRequest
		{
			...
			OrderType = orderType,
			Price = orderType is NinjaTraderOrderTypes.Market or NinjaTraderOrderTypes.Stop ? null : replaceMsg.Price,
			StopPrice = stopPrice,
	...
	private async ValueTask OnEntityReceived(string entityType, NinjaTraderEntity entity, CancellationToken cancellationToken)
	{
		switch (entityType)
		{
			case "order":
	...
	private async ValueTask ProcessOrder(NinjaTraderOrder order, long originalTransactionId, CancellationToken cancellationToken)
	{
		...
		await SendOutMessageAsync(new ExecutionMessage
		{
			...
			Condition = version?.StopPrice is decimal stopPrice ? new NinjaTraderOrderCondition { StopPrice = stopPrice } : null,
			Error = state == OrderStates.Failed ? new InvalidOperationException(version?.Text ?? order.OrdStatus.ToString()) : null,
		}, cancellationToken);
	}

	private async ValueTask ProcessFill(NinjaTraderFill fill, long originalTransactionId, CancellationToken cancellationToken)
	{
	...
	private async ValueTask ProcessPosition(NinjaTraderPosition position, CancellationToken cancellationToken)
	{
	...
	private ValueTask ProcessCashBalance(NinjaTraderCashBalance balance, CancellationToken cancellationToken)
