...
{
	private const string _boardCode = "NINJATRADER";
	...
	private async Task<(NinjaTraderContract contract, SecurityId securityId)> EnsureContract(string symbol, CancellationToken cancellationToken)
	{
		if (_contractIds.TryGetValue(symbol, out var cachedId) && _contracts.TryGetValue(cachedId, out var cachedSecurityId))
			return (new NinjaTraderContract { Id = cachedId, Name = symbol }, cachedSecurityId);

		var contract = await _httpClient.FindContract(symbol, cancellationToken)
			?? throw new InvalidOperationException($"NinjaTrader contract '{symbol}' was not found.");
	...
	private async Task<SecurityId> GetSecurityId(long contractId, CancellationToken cancellationToken)
	{
		...
		var contract = await _httpClient.GetContract(contractId, cancellationToken)
			?? throw new InvalidOperationException($"NinjaTrader contract '{contractId}' was not found.");
	...
	protected override async ValueTask OnTFCandlesSubscriptionAsync(MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		var request = new NinjaTraderChartRequest
		{
			Symbol = contract.Name,
			ChartDescription = new()
			{
				UnderlyingType = isDaily ? NinjaTraderChartTypes.DailyBar : NinjaTraderChartTypes.MinuteBar,
				ElementSize = isDaily ? (int)timeFrame.TotalDays : (int)timeFrame.TotalMinutes,
				ElementSizeUnit = NinjaTraderChartUnits.UnderlyingUnits,
	...
	private async ValueTask OnQuoteReceived(NinjaTraderQuote quote, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnDomReceived(NinjaTraderDom dom, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnChartReceived(NinjaTraderChart chart, CancellationToken cancellationToken)
	{
