...
public partial class QuickSwapMessageAdapter
{
	...
	{
		public QuickSwapMarket Market { get; init; }
	}
	...
	private sealed class TrackedSwap
	{
		...
		public QuickSwapRpcReceipt Receipt { get; set; }
	}
	...
	private readonly Dictionary<string, QuickSwapMarket> _markets =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, QuickSwapToken> _tokens =
		new(StringComparer.OrdinalIgnoreCase);
	...
	private readonly Dictionary<QuickSwapPoolVersions,
		QuickSwapGraphClient> _graphClients = [];
	private QuickSwapRpcClient _rpcClient;
	private DateTime _nextMarketPoll;
	private DateTime _nextPrivatePoll;

	/// <summary>Initializes the adapter.</summary>
	public QuickSwapMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.QuickSwap];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.QuickSwap) ||
			securityId.IsAssociated(BoardCodes.QuickSwap);

	private QuickSwapRpcClient RpcClient => _rpcClient ?? throw new
		InvalidOperationException(LocalizedStrings.ConnectionNotOk);

	private QuickSwapGraphClient GetGraphClient(
		...
		using (_sync.EnterScope())
			return _graphClients.TryGetValue(poolVersion, out var client)
				...
				: throw new InvalidOperationException(
					$"A compatible QuickSwap {poolVersion} subgraph is " +
	...
	private void EnsureTradingReady()
	{
		...
		if (!RpcClient.IsSigningAvailable)
			throw new InvalidOperationException(
				"An EVM private key is required for QuickSwap swaps.");
	}

	private QuickSwapMarket GetMarket(SecurityId securityId)
	{
		if (!ValidateSecurityId(securityId))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not QuickSwap.");
		...
		using (_sync.EnterScope())
			return _markets.TryGetValue(code, out var market)
				? market
				: throw new InvalidOperationException(
					$"Unknown QuickSwap market '{code}'.");
	}

	private string GetPortfolioName()
		=> $"QuickSwap_{Chain}_{RpcClient.WalletAddress[2..10]}";
	...
	private void ValidatePortfolio(string portfolioName)
	{
		if (!portfolioName.IsEmpty() &&
			...
			throw new InvalidOperationException(
				$"Unknown QuickSwap portfolio '{portfolioName}'.");
	}
