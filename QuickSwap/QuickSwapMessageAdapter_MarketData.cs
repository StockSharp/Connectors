...
public partial class QuickSwapMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"QuickSwap quote probes do not expose historical Level1 " +
				"events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!AllTimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"QuickSwap does not support the {timeFrame} candle interval.");
		var now = DateTime.UtcNow;
		var to = (mdMsg.To ?? now).ToUniversalTime().Min(now);
		var maximum = GetSubscriptionMaximum(mdMsg.Count);
	...
	private SecurityMessage CreateSecurity(QuickSwapMarket market,
	...
	private async ValueTask SendLevel1Async(QuickSwapMarket market,
	...
	private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
		QuickSwapMarket market, CancellationToken cancellationToken)
	{
		...
		var bidQuote = await RpcClient.GetQuoteAsync(market,
			QuickSwapTradeTypes.ExactInput, units, cancellationToken);
		var askQuote = await RpcClient.GetQuoteAsync(market,
			QuickSwapTradeTypes.ExactOutput, units, cancellationToken);
		...
		if (bid <= 0 || ask <= 0)
			throw new InvalidDataException(
				"QuickSwap returned a non-positive quote price.");
		return (bid, ask);
	}

	private ValueTask SendLevel1Async(QuickSwapMarket market, long target,
	...
	private async ValueTask<QuickSwapTrade[]> LoadTradesAsync(
		QuickSwapMarket market, DateTime from, DateTime to, int maximum,
	...
	private static QuickSwapTrade ToTrade(QuickSwapMarket market,
		QuickSwapSwap swap)
	{
		...
		decimal? amount0;
		decimal? amount1;
		if (market.PoolVersion == QuickSwapPoolVersions.V3)
		{
	...
	private async ValueTask<bool> SendTradeAsync(QuickSwapMarket market,
		QuickSwapTrade trade, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask<QuickSwapCandle[]> LoadCandlesAsync(
		QuickSwapMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
	...
	private ValueTask SendCandleAsync(QuickSwapMarket market,
		QuickSwapCandle candle, TimeSpan timeFrame, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask PollLevel1Async(
		CancellationToken cancellationToken)
	{
		(QuickSwapMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
