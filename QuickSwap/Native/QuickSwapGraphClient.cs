...
sealed class QuickSwapGraphClient : BaseLogReceiver
{
	...
	private readonly QuickSwapPoolVersions _poolVersion;
		...
		QuickSwapPoolVersions poolVersion)
	{
		...
		if (source.IsEmpty())
			throw new ArgumentException(
				"A QuickSwap subgraph source is required.", nameof(source));
	...
	public override string Name => $"QuickSwap_{_poolVersion}_Subgraph";
	...
	public async ValueTask<QuickSwapPool[]> GetPoolsAsync(int maximum,
		CancellationToken cancellationToken)
	{
		var response = await SendAsync<QuickSwapPoolVariables,
			QuickSwapPoolData>(new()
			{
				Query = _poolVersion == QuickSwapPoolVersions.V3
				? _v3PoolsQuery
				: _v2PoolsQuery,
				Variables = new() { First = maximum.Min(1000).Max(1) },
			}, cancellationToken);
		return _poolVersion == QuickSwapPoolVersions.V3
			? response.Pools ?? []
			: response.Pairs ?? [];
	}

	public async ValueTask<QuickSwapSwap[]> GetSwapsAsync(string poolId,
		DateTime from, int maximum, CancellationToken cancellationToken)
	{
		var response = await SendAsync<QuickSwapSwapVariables,
			QuickSwapSwapData>(new()
			{
				Query = _poolVersion == QuickSwapPoolVersions.V3
				? _v3SwapsQuery
				: _v2SwapsQuery,
	...
	private async ValueTask<TData> SendAsync<TVariables, TData>(
		QuickSwapGraphRequest<TVariables> payload,
		CancellationToken cancellationToken)
		...
		where TData : class
	{
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			if (!response.IsSuccessStatusCode)
				throw new QuickSwapApiException(response.StatusCode,
					$"The Graph HTTP {(int)response.StatusCode}: " +
					Truncate(responseBody));
			QuickSwapGraphResponse<TData> graph;
			try
			{
				graph = JsonConvert.DeserializeObject<
					QuickSwapGraphResponse<TData>>(responseBody,
					_jsonSettings);
			}
