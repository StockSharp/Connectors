...
public partial class XtMessageAdapter
{
	...
	{
		var mappings = new List<(string Symbol, XtSections Section)>();
		if (IsSectionEnabled(XtSections.Spot))
			mappings.AddRange((await RestClient.GetSpotSymbolsAsync(cancellationToken))
				...
				.Select(static item => (item.Symbol.ToUpperInvariant(), XtSections.Spot)));
		if (IsSectionEnabled(XtSections.Futures))
			mappings.AddRange((await RestClient.GetFuturesSymbolsAsync(cancellationToken))
				.Where(static item => item?.Symbol.IsEmpty() == false)
				.Select(static item => (item.Symbol.ToUpperInvariant(), XtSections.Futures)));
		...
		CancellationToken cancellationToken)
	{
			...
			(securityTypes.Count == 0 || securityTypes.Contains(SecurityTypes.CryptoCurrency)))
		{
			...
			foreach (var symbol in symbols)
			{
				...
				using (_sync.EnterScope())
					_knownSymbols.Add((XtSections.Spot, code));
				var security = new SecurityMessage
				{
					SecurityId = code.ToStockSharp(XtSections.Spot),
		...
		if (left > 0 && IsSectionEnabled(XtSections.Futures) &&
			(securityTypes.Count == 0 || securityTypes.Contains(SecurityTypes.Future)))
		{
			...
			foreach (var symbol in symbols)
			{
				...
				var code = symbol.Symbol.ToUpperInvariant();
				using (_sync.EnterScope())
					_knownSymbols.Add((XtSections.Futures, code));
				var security = new SecurityMessage
				{
					SecurityId = code.ToStockSharp(XtSections.Futures),
					Name = symbol.Name.IsEmpty(code),
					...
					Multiplier = symbol.ContractSize.ToDecimal() ?? 1m,
				}.TryFillUnderlyingId(symbol.BaseCurrency?.ToUpperInvariant());
				if (!security.IsMatch(lookupMsg, securityTypes))
					continue;
				await SendOutMessageAsync(security, cancellationToken);
		...
		CancellationToken cancellationToken)
	{
		...
		if (subscribeTrade)
			await SubscribePublicAsync(section, "TRADE", symbol, cancellationToken);
		if (subscribeDepth)
			await SubscribePublicAsync(section, "DEPTH", symbol, cancellationToken);
		if (subscribeIndex)
			await SubscribePublicAsync(section, "INDEX", symbol, cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		var book = await RestClient.GetDepthAsync(section, symbol, depth, cancellationToken);
	...
	private ValueTask SubscribePublicAsync(XtSections section, string topic, string symbol,
		CancellationToken cancellationToken)
		=> GetMarketClient(section).SubscribeAsync(topic, symbol,
			topic.EqualsIgnoreCase("DEPTH") ? 50 : null, cancellationToken);

	private ValueTask UnsubscribePublicAsync(XtSections section, string topic, string symbol,
		CancellationToken cancellationToken)
		=> GetMarketClient(section).UnsubscribeAsync(topic, symbol,
			topic.EqualsIgnoreCase("DEPTH") ? 50 : null, cancellationToken);

	private XtWsClient GetMarketClient(XtSections section)
		=> section == XtSections.Spot
			? _spotMarketClient ?? throw new InvalidOperationException("XT.COM Spot stream is not connected.")
			: _futuresMarketClient ?? throw new InvalidOperationException("XT.COM Futures stream is not connected.");
		...
		CancellationToken cancellationToken)
	{
		...
		using (_sync.EnterScope())
		{
			if (_level1Subscriptions.Remove(transactionId, out subscription))
			{
				...
				if (subscription.Section == XtSections.Futures)
		...
		if (subscription is null)
			return;
		if (trade)
			await UnsubscribePublicAsync(subscription.Section, "TRADE", subscription.Symbol, cancellationToken);
		if (depth)
			await UnsubscribePublicAsync(subscription.Section, "DEPTH", subscription.Symbol, cancellationToken);
		if (index)
			await UnsubscribePublicAsync(subscription.Section, "INDEX", subscription.Symbol, cancellationToken);
	}
	...
	private async ValueTask OnTradesAsync(XtSections section, XtWsTradeMessage message,
		CancellationToken cancellationToken)
	{
	...
	private CandleEmission[] UpdateCandles(string symbol, XtSections section, DateTime time,
	...
	private async ValueTask OnDepthAsync(XtSections section, XtWsDepthMessage message,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnIndexAsync(XtSections section, XtWsIndexMessage message,
		CancellationToken cancellationToken)
	{
		if (section != XtSections.Futures)
			return;
		foreach (var index in message?.Data ?? [])
		{
			...
			using (_sync.EnterScope())
				ids = [.. _level1Subscriptions
					.Where(pair => pair.Value.Section == XtSections.Futures &&
			...
			foreach (var id in ids)
				await SendOutMessageAsync(new Level1ChangeMessage
				{
					SecurityId = symbol.ToStockSharp(XtSections.Futures),
					ServerTime = time,
					OriginalTransactionId = id,
				}
	...
	private ValueTask SendLevel1SnapshotAsync(string symbol, XtSections section,
		XtTicker ticker, XtBookTicker book, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendBestQuotesAsync(XtBookLevel[] bids, XtBookLevel[] asks,
		string symbol, XtSections section, long transactionId, DateTime serverTime,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new Level1ChangeMessage
		{
			SecurityId = symbol.ToStockSharp(section),
	...
	private ValueTask SendBookAsync(XtBookLevel[] bids, XtBookLevel[] asks,
		string symbol, XtSections section, long transactionId, DateTime serverTime, int depth,
		CancellationToken cancellationToken)
	...
	private ValueTask SendTradeAsync(string id, string price, string volume, DateTime time,
		Sides side, string symbol, XtSections section, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(DateTime openTime, decimal open, decimal high, decimal low,
		decimal close, decimal volume, string symbol, XtSections section, TimeSpan timeFrame,
	...
	private static QuoteChange[] ToQuotes(XtBookLevel[] levels, int depth)
