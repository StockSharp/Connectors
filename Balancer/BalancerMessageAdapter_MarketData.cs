...
public partial class BalancerMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Balancer quote probes do not expose historical Level1 " +
				"events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		var from = mdMsg.From?.EnsureUtc() ?? DateTime.UnixEpoch;
		...
		using (_sync.EnterScope())
			...
			{
				Market = market,
				From = mdMsg.From?.EnsureUtc(),
				To = mdMsg.To?.EnsureUtc(),
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!AllTimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"Balancer does not support the {timeFrame} candle interval.");
		var now = DateTime.UtcNow;
		var to = (mdMsg.To ?? now).EnsureUtc().Min(now);
		var maximum = GetSubscriptionMaximum(mdMsg.Count);
	...
	private SecurityMessage CreateSecurity(BalancerMarket market,
		long originalTransactionId)
		=> new SecurityMessage
		{
			SecurityId = market.ToStockSharp(),
			Name = $"{market.BaseToken.Symbol}/{market.QuoteToken.Symbol} " +
				$"{market.Pool.Type} V{market.Pool.ProtocolVersion}",
	...
	private async ValueTask SendLevel1Async(BalancerMarket market,
	...
	private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
		BalancerMarket market, CancellationToken cancellationToken)
	{
		var bidQuote = await ApiClient.GetQuoteAsync(_deployment, market,
			BalancerSwapTypes.ExactIn, ProbeVolume, cancellationToken);
		var askQuote = await ApiClient.GetQuoteAsync(_deployment, market,
			BalancerSwapTypes.ExactOut, ProbeVolume, cancellationToken);
		var bid = bidQuote.Price;
		var ask = askQuote.Price;
		if (bid <= 0 || ask <= 0)
			throw new InvalidDataException(
				"Balancer returned a non-positive quote price.");
		return (bid, ask);
	}

	private ValueTask SendLevel1Async(BalancerMarket market, long target,
	...
	private async ValueTask<BalancerTrade[]> LoadTradesAsync(
		BalancerMarket market, DateTime from, DateTime to, int maximum,
		CancellationToken cancellationToken)
		=> await ApiClient.GetTradesAsync(_deployment, market, from, to,
			maximum.Min(HistoryMaximum).Max(1), cancellationToken);

	private async ValueTask<BalancerTrade> ToTradeAsync(
		BalancerMarket market, BalancerRpcLog log,
		CancellationToken cancellationToken)
	{
		BalancerRawSwap raw;
		try
		{
			raw = BalancerExtensions.DecodeSwap(log, _deployment);
		}
		...
		if (raw is null || !raw.PoolId.EqualsIgnoreCase(market.Pool.Id))
			return null;
		var time = await GetBlockTimeAsync(raw.BlockNumber, cancellationToken);
		if (!raw.TryCreateTrade(market, time, out var trade))
			return null;
		return trade;
	}
	...
	private async ValueTask<bool> SendTradeAsync(BalancerMarket market,
		BalancerTrade trade, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask<BalancerCandle[]> LoadCandlesAsync(
		BalancerMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
	...
	private ValueTask SendCandleAsync(BalancerMarket market,
		BalancerCandle candle, TimeSpan timeFrame, long target,
		CancellationToken cancellationToken)
	{
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (var log in logs)
		{
			...
			try
			{
				var pool = BalancerExtensions.DecodeSwap(log, _deployment)?.PoolId;
				if (pool.IsEmpty())
					continue;
				using (_sync.EnterScope())
	...
	private async ValueTask PollLevel1Async(
		CancellationToken cancellationToken)
	{
		(BalancerMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
		...
		CancellationToken cancellationToken)
	{
		...
		{
			var now = DateTime.UtcNow;
			var to = (item.Subscription.To ?? now).EnsureUtc().Min(now);
		...
		TimeSpan timeFrame, DateTime to, int maximum)
	{
		...
		if (message.From is DateTime from && to > from)
			return ((to - from.EnsureUtc()).Ticks /
