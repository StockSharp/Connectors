...
public partial class SunIoMessageAdapter
{
	...
	{
		public SunIoMarket Market { get; init; }
	}
	...
	private sealed class TrackedSwap
	{
		...
		public SunIoRouterTransaction Transaction { get; set; }
	}
	...
	private readonly record struct Level1Fingerprint(decimal Bid,
		decimal Ask, decimal BidVolume, decimal AskVolume);
	...
	private readonly Dictionary<string, SunIoMarket> _markets =
		new(StringComparer.OrdinalIgnoreCase);
	private readonly Dictionary<string, SunIoMarket> _marketsByAddress =
	...
	private SunIoApiClient _apiClient;
	private SunIoSigner _signer;
	private SunIoToken _trxToken;
	private DateTime _nextMarketPoll;
	private DateTime _nextPrivatePoll;

	/// <summary>Initializes the adapter.</summary>
	public SunIoMessageAdapter(IdGenerator transactionIdGenerator)
	...
	/// <inheritdoc />
	public override string[] AssociatedBoards => [BoardCodes.SunIo];

	/// <inheritdoc />
	protected override bool ValidateSecurityId(SecurityId securityId)
		=> securityId.BoardCode.IsEmpty() ||
			securityId.BoardCode.EqualsIgnoreCase(BoardCodes.SunIo) ||
			securityId.IsAssociated(BoardCodes.SunIo);

	private SunIoApiClient ApiClient => _apiClient ?? throw new
		InvalidOperationException(LocalizedStrings.ConnectionNotOk);

	private SunIoSigner Signer => _signer ?? throw new
		InvalidOperationException(LocalizedStrings.ConnectionNotOk);

	private void EnsureConnected()
	{
		if (_apiClient is null || _signer is null || _trxToken is null)
	...
	private void EnsureTradingReady()
	{
		...
		if (!Signer.IsSigningAvailable)
			throw new InvalidOperationException(
				"A TRON private key is required for SUN.io swaps.");
	}

	private SunIoMarket GetMarket(SecurityId securityId)
	{
		if (!ValidateSecurityId(securityId))
			throw new InvalidOperationException(
				$"Security board '{securityId.BoardCode}' is not SUN.io.");
		...
		using (_sync.EnterScope())
			return _markets.TryGetValue(code, out var market)
				? market
				: throw new InvalidOperationException(
					$"Unknown SUN.io market '{code}'.");
	}

	private string GetPortfolioName()
		=> $"SUN.io_{Signer.WalletAddress[..8]}";

	private void ValidatePortfolio(string portfolioName)
	{
		if (!Signer.IsWalletAvailable)
			throw new InvalidOperationException(
				"A TRON wallet address is required for portfolio data.");
		if (!portfolioName.IsEmpty() &&
			!portfolioName.EqualsIgnoreCase(GetPortfolioName()))
			throw new InvalidOperationException(
				$"Unknown SUN.io portfolio '{portfolioName}'.");
	}
