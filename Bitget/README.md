# Bitget Connector for StockSharp

This directory contains the **Bitget** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `BitgetMessageAdapter` message adapter, exposing Bitget market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Bitget`.

## Configuration

`BitgetMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new BitgetMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Bitget connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/bitget.html).
