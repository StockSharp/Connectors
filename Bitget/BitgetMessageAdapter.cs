...
public partial class BitgetMessageAdapter
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="BitgetMessageAdapter"/>.
	/// </summary>
	/// <param name="transactionIdGenerator">Transaction id generator.</param>
	public BitgetMessageAdapter(IdGenerator transactionIdGenerator)
	...
	protected override async ValueTask ConnectAsync(ConnectMessage connectMsg, CancellationToken cancellationToken)
	{
		if (this.IsTransactional())
		{
			...
			if (Passphrase.IsEmpty())
				throw new InvalidOperationException(LocalizedStrings.Passphrase);
		}
		...
		if (Sections.Contains(BitgetSections.Spot))
			addAdapter(new Native.Spot.SpotAdapter(this, _authenticator));

		if (Sections.Contains(BitgetSections.Futures))
		{
			var prefix = IsDemo ? "S" : string.Empty;

			addAdapter(new Native.Futures.MixAdapter(this, _authenticator, $"{prefix}COIN-FUTURES", BoardCodes.BitgetFutCoin, SecurityTypes.Future));
			addAdapter(new Native.Futures.MixAdapter(this, _authenticator, $"{prefix}USDC-FUTURES", BoardCodes.BitgetFutUsdc, SecurityTypes.Future));
			addAdapter(new Native.Futures.MixAdapter(this, _authenticator, $"{prefix}USDT-FUTURES", BoardCodes.BitgetFutUsdt, SecurityTypes.Future));
		}
