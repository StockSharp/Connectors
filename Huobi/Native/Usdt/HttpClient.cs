...
class HttpClient(Authenticator authenticator, string domain) : BaseLogReceiver
{
    ...
    public override string Name => nameof(Huobi) + "_" + nameof(Usdt) + "_" + nameof(HttpClient);

	public Task<IEnumerable<Contract>> GetContractsAsync(CancellationToken cancellationToken)
	{
		var url = CreateUrl("swap_contract_info");
		var request = CreateRequest(Method.Get);

		return MakeRequestAsync<IEnumerable<Contract>>(url, request, cancellationToken);
	}
	...
	{
		var url = CreateUrl("swap_account_info");
	...
	{
		var url = CreateUrl("swap_position_info");
		var request = CreateRequest(Method.Post);

		return MakeRequestAsync<IEnumerable<Position>>(url, ApplySecret(request, url), cancellationToken);
	}

	public async Task<IEnumerable<Order>> GetOpenOrdersAsync(string symbol, int? size = null, CancellationToken cancellationToken = default)
	{
		var url = CreateUrl("swap_openorders");
		var request = CreateRequest(Method.Post);
	...
	public Task RegisterOrderAsync(long transactionId, string contractCode, bool reduceOnly, string type,
		string direction, decimal? price, decimal volume, int levelRate, string offset, CancellationToken cancellationToken)
	{
		var url = CreateUrl("swap_order");
		var request = CreateRequest(Method.Post);
		...
		body.reduce_only = reduceOnly ? 1 : 0;
	...
	public Task CancelOrderAsync(long transactionId, string symbol, CancellationToken cancellationToken)
	{
		var url = CreateUrl("swap_cancel");
		var request = CreateRequest(Method.Post);
	...
	public Task BatchCancelOrderAsync(string underlyingAsset, string contractCode, string direction, CancellationToken cancellationToken)
	{
		var url = CreateUrl("swap_cancelall");
		var request = CreateRequest(Method.Post);
	...
	{
		var url = CreateUrl("swap_trigger_order");
		var request = CreateRequest(Method.Post);
	...
	{
		var url = CreateUrl("swap_trigger_cancel");
		var request = CreateRequest(Method.Post);
	...
	{
		var url = CreateUrl("swap_trigger_cancelall");
		var request = CreateRequest(Method.Post);
	...
	{
		var url = CreateUrl("swap_trigger_openorders");
		var request = CreateRequest(Method.Post);
	...
	private Url CreateUrl(string methodName, string version = "v1/", string api = "linear-swap-api")
	{
