...
class FuturesAdapter(HuobiMessageAdapter parent, Authenticator authenticator, string domain) : NativeAdapter(parent, authenticator, domain)
{
	private enum ContractTypes
	{
		Week,
		NextWeek,
		Quarter,
    ...
    private static ContractTypes ToContractType(string type)
	{
		return type switch
		{
			"this_week"		=> ContractTypes.Week,
			"next_week"		=> ContractTypes.NextWeek,
			"quarter"		=> ContractTypes.Quarter,
			"next_quarter"	=> ContractTypes.NextQuarter,

			_ => throw new ArgumentOutOfRangeException(nameof(type), type, LocalizedStrings.InvalidValue),
		};
	}

	private async ValueTask<string> Format(SecurityId securityId, CancellationToken cancellationToken)
	{
		var (asset, type) = await EnsureGetContractInfo(securityId, cancellationToken);

		var typeStr = type switch
		{
			ContractTypes.Week			=> "CW",
			ContractTypes.NextWeek		=> "NW",
			ContractTypes.Quarter		=> "CQ",
			ContractTypes.NextQuarter	=> "NQ",

			_ => throw new ArgumentOutOfRangeException(type.ToString()),
		};

		return $"{asset}_{typeStr}";
	}

	private async ValueTask<(string, ContractTypes)> EnsureGetContractInfo(SecurityId securityId, CancellationToken cancellationToken)
	{
		if (_contractInfo.Count == 0)
		{
		...
		var contractCode = securityId.ToSymbol();

		if (_contractInfo.TryGetValue(contractCode, out var tuple))
			return tuple;

		throw new ArgumentOutOfRangeException(contractCode);
	}
	...
	public override async ValueTask SecurityLookupAsync(SecurityLookupMessage lookupMsg, CancellationToken cancellationToken)
	{
		...
		foreach (var contract in await _httpClient.GetContractsAsync(cancellationToken))
		{
			var secMsg = new SecurityMessage
			{
				...
				SettlementDate = contract.SettlementTime,
	...
	public override async ValueTask MarketDataAsync(MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.DataType2 == DataType.Level1)
		{
			...
			var symbol = await Format(securityId, cancellationToken);

			if (mdMsg.IsSubscribe)
			{
				await _pusherClient.SubscribeTicker(symbol, securityId, transId, cancellationToken);
				await _pusherClient.SubscribeBest(symbol, securityId, AddExtraRequest(), cancellationToken);

				await SendSubscriptionResultAsync(mdMsg, cancellationToken);
			}
			else
			{
				await _pusherClient.UnSubscribeTicker(symbol, securityId, transId, cancellationToken);
				await _pusherClient.UnSubscribeBest(symbol, securityId, AddExtraRequest(), cancellationToken);
			}
		}
		else if (mdMsg.DataType2 == DataType.MarketDepth)
		{
			...
			if (mdMsg.IsSubscribe)
			{
				await _pusherClient.SubscribeOrderBook(symbol, securityId, depth, transId, cancellationToken);

				await SendSubscriptionResultAsync(mdMsg, cancellationToken);
			}
			else
				await _pusherClient.UnSubscribeOrderBook(symbol, securityId, depth, transId, cancellationToken);
		}
		else if (mdMsg.DataType2 == DataType.Ticks)
		{
			...
			if (mdMsg.IsSubscribe)
			{
				if (mdMsg.To != null)
				{
					var trades = await _httpClient.GetTradesAsync(symbol, (int?)mdMsg.Count ?? maxCount, cancellationToken);
					...
					await _pusherClient.SubscribeTrades(symbol, securityId, transId, cancellationToken);

				await SendSubscriptionResultAsync(mdMsg, cancellationToken);
			}
			else
				await _pusherClient.UnSubscribeTrades(symbol, securityId, transId, cancellationToken);
		}
		else if (mdMsg.DataType2.IsTFCandles)
		{
			...
			{
				if (mdMsg.To != null)
					await _pusherClient.RequestCandles(symbol, securityId, tf, transId, (long?)mdMsg.From?.ToUnix(), (long?)mdMsg.To?.ToUnix(), cancellationToken);
				else
				{
					await _pusherClient.SubscribeCandles(symbol, securityId, tf, transId, cancellationToken);

					await SendSubscriptionResultAsync(mdMsg, cancellationToken);
				}
			}
			else
				await _pusherClient.UnSubscribeCandles(symbol, securityId, tf, transId, cancellationToken);
		}
		else
		{
	...
	public override async ValueTask RegisterOrderAsync(OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		await _httpClient.RegisterOrderAsync(regMsg.TransactionId, symbol, regMsg.TimeInForce.ToFutureNative(condition?.Opponent, regMsg.PostOnly, condition?.Optimal), regMsg.Side.ToNative(), price, regMsg.Volume, regMsg.Leverage ?? 10, condition?.Offset?.ToOffset() ?? "open", cancellationToken);
	}

	public override async ValueTask CancelOrderAsync(OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		var asset = cancelMsg.GetUnderlyingCode();

		if (IsAlgo(cancelMsg.OriginalTransactionId))
			await _httpClient.CancelTriggerOrderAsync(asset, cancelMsg.OrderStringId, cancellationToken);
		else
			await _httpClient.CancelOrderAsync(cancelMsg.OriginalTransactionId, asset, cancellationToken);
	}
	...
	{
		await _httpClient.BatchCancelOrderAsync(cancelMsg.GetUnderlyingCode(), cancelMsg.SecurityId.SecurityCode, null, cancelMsg.Side?.ToNative(), cancellationToken);
	}
	...
	public override async ValueTask OrderStatusAsync(OrderStatusMessage message, CancellationToken cancellationToken)
	{
		...
		{
			var orders = await _httpClient.GetOpenOrdersAsync(coin, size: 50, cancellationToken: cancellationToken);
			...
			var algoOrders = await _httpClient.GetOpenTriggerOrdersAsync(coin, size: 500, cancellationToken: cancellationToken);
