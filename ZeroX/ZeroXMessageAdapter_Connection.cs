...
public partial class ZeroXMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			_ = await GetOrLoadTokenAsync(
				ZeroXExtensions.NativeTokenAddress, cancellationToken);

			ZeroXMarketDefinition[] definitions;
			try
			{
			...
			{
				throw new InvalidOperationException(
					"Configured 0x market definitions are invalid.", error);
			}
			...
			foreach (var definition in definitions)
			{
					...
					!cancellationToken.IsCancellationRequested)
				{
					errors.Add(error);
					this.AddWarningLog(
						"0x market {0}/{1} loading failed: {2}",
			...
			if (markets.Length == 0)
				throw errors.Count == 1
					...
					: new AggregateException(
						"No 0x markets could be loaded.", errors);

			connectMsg.SessionId = RpcClient.IsWalletConfigured
				? $"0x {Chain} {RpcClient.WalletAddress}"
				: $"0x {Chain} public";
	...
	private async ValueTask RegisterDefinitionAsync(
		ZeroXMarketDefinition definition,
		CancellationToken cancellationToken)
	{
		...
		if (probe <= 0)
			throw new InvalidOperationException(
				"The configured 0x quote probe volume rounds to zero.");
	...
	private async ValueTask<ZeroXToken> GetOrLoadTokenAsync(string address,
		CancellationToken cancellationToken)
	{
	...
	private void RegisterMarket(ZeroXMarket market)
	{
		...
		using (_sync.EnterScope())
		{
			...
			if (_markets.Values.Any(item =>
				...
				throw new InvalidOperationException(
					"The same 0x token pair is configured twice.");
	...
	private ZeroXMarketDefinition[] ParseMarketDefinitions()
	{
		var configured = Markets.IsEmpty()
			? Chain.GetDefaultMarkets()
			: Markets;
		var result = new List<ZeroXMarketDefinition>();
		...
		if (result.Count == 0)
			throw new InvalidOperationException(
				"At least one 0x market must be configured.");
		return [.. result];
	}
	...
	private static string NormalizeSecurityCode(string value)
	{
		...
		if (value.Length > 64 || value.Any(static ch =>
			...
			throw new FormatException($"Invalid 0x security code '{value}'.");
		return value;
	}
