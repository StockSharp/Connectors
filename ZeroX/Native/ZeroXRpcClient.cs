...
sealed class ZeroXRpcClient : BaseLogReceiver
{
	...
	public ZeroXRpcClient(string endpoint, ZeroXChains chain,
	...
	public override string Name => "ZeroX_JSON-RPC";

	public ZeroXChains Chain { get; }
	...
	public bool IsWalletConfigured =>
		!WalletAddress.EqualsIgnoreCase(ZeroXExtensions.ProbeAddress);
	...
	{
		var chainId = (await SendAsync<ZeroXRpcEmptyParameters, string>(
			"eth_chainId", new(), true, cancellationToken)).ParseInteger();
		if (chainId != new BigInteger((int)Chain))
			throw new InvalidOperationException(
				$"JSON-RPC is connected to chain {chainId}, but 0x " +
				$"chain {(int)Chain} is configured.");
	}

	public async ValueTask<ZeroXToken> GetTokenAsync(string address,
	...
	public async ValueTask<BigInteger> GetBalanceAsync(ZeroXToken token,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
			return (await SendAsync<ZeroXRpcAddressTagParameters, string>(
				"eth_getBalance", new()
				{
					Address = WalletAddress,
					BlockTag = "latest",
				}, true, cancellationToken)).ParseInteger();
		var data = ZeroXExtensions.EncodeStaticCall(
			"balanceOf(address)",
			ZeroXExtensions.AbiAddress(WalletAddress));
	...
	public async ValueTask<BigInteger> GetAllowanceAsync(
		ZeroXToken token, string spender,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		var data = ZeroXExtensions.EncodeStaticCall(
			...
			ZeroXExtensions.AbiAddress(WalletAddress),
			ZeroXExtensions.AbiAddress(spender));
		return (await CallContractAsync(token.Address, data,
			cancellationToken)).ParseInteger();
	}

	public ZeroXTransaction CreateApprovalTransaction(
		ZeroXToken token, string spender, BigInteger amount)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		return new()
		{
			...
			Data = ZeroXExtensions.EncodeStaticCall(
				"approve(address,uint256)",
				ZeroXExtensions.AbiAddress(spender),
				ZeroXExtensions.AbiWord(amount)),
			Value = BigInteger.Zero,
		};
	}

	public async ValueTask<BigInteger> EstimateGasAsync(
		ZeroXTransaction transaction,
		CancellationToken cancellationToken)
	{
		ValidateTransaction(transaction);
		var call = ToRpcCall(transaction);
		return (await SendAsync<ZeroXRpcCallOnlyParameters, string>(
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			var nonce = (await SendAsync<ZeroXRpcAddressTagParameters,
			...
			else
			{
				var gasPrice = (await SendAsync<
					ZeroXRpcEmptyParameters, string>("eth_gasPrice",
			...
			var hash = await SendAsync<ZeroXRpcValueParameters, string>(
	...
	public async ValueTask<ZeroXRpcReceipt> GetReceiptAsync(string hash,
		CancellationToken cancellationToken)
	{
		hash = hash.NormalizeHash();
		var receipt = await SendAsync<ZeroXRpcValueParameters,
			ZeroXRpcReceipt>("eth_getTransactionReceipt", new()
			{
				Value = hash,
	...
	public async ValueTask<ZeroXRpcReceipt> WaitForReceiptAsync(
	...
	public async ValueTask<ZeroXRpcBlock> GetBlockAsync(
		BigInteger blockNumber, CancellationToken cancellationToken)
	{
		if (blockNumber < 0)
			throw new ArgumentOutOfRangeException(nameof(blockNumber));
		var block = await SendAsync<ZeroXRpcTagBooleanParameters,
			ZeroXRpcBlock>("eth_getBlockByNumber", new()
			{
	...
	public async ValueTask VerifyContractAsync(string address, string name,
		CancellationToken cancellationToken)
	{
		var code = await SendAsync<ZeroXRpcAddressTagParameters, string>(
			"eth_getCode", new()
			{
				Address = address.NormalizeAddress(),
				BlockTag = "latest",
		...
		if (code.IsEmpty() || code.EqualsIgnoreCase("0x") ||
			...
			throw new InvalidDataException(
				$"0x {name} '{address}' is not a contract on " +
				$"chain {(int)Chain}.");
	}
		...
		TryGetEip1559FeesAsync(CancellationToken cancellationToken)
	{
		try
		{
			var block = await SendAsync<
				ZeroXRpcTagBooleanParameters, ZeroXRpcBlock>(
			...
			var priority = (await SendAsync<
				ZeroXRpcEmptyParameters, string>(
	...
	private async ValueTask<string> CallContractAsync(string address,
		...
		=> await SendAsync<ZeroXRpcCallParameters, string>("eth_call",
			new()
			{
				Call = new()
				{
		...
		where TParameters : ZeroXRpcParameters
	{
		...
		var payload = JsonConvert.SerializeObject(
			new ZeroXRpcRequest<TParameters>
			{
				Id = requestId,
				Method = method,
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			if (!response.IsSuccessStatusCode)
				throw new InvalidOperationException(
					...
					Truncate(body));
			ZeroXRpcResponse<TResult> rpc;
			try
			{
				rpc = JsonConvert.DeserializeObject<
					ZeroXRpcResponse<TResult>>(body, _jsonSettings);
			}
	...
	private void ValidateTransaction(ZeroXTransaction transaction)
	{
	...
	private ZeroXRpcCall ToRpcCall(ZeroXTransaction transaction)
		=> new()
		{
			From = WalletAddress,
