...
public partial class ZeroXMessageAdapter
{
		...
		OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		if (regMsg.OrderType is not (null or OrderTypes.Market))
			throw new NotSupportedException(
				"0x Swap API v2 supports immediate market swaps only.");
		...
		if (regMsg.Condition is not null)
			throw new NotSupportedException(
				"0x Swap API v2 does not expose conditional orders.");
		...
		if (regMsg.TillDate is not null)
			throw new NotSupportedException(
				"An expiry cannot be attached to a 0x Swap API v2 request.");
		...
		var response = await HttpClient.GetQuoteAsync(sourceToken.Address,
			destinationToken.Address, quote.InputAmount,
			RpcClient.WalletAddress, SlippageTolerance, cancellationToken);
		ValidateQuote(response, sourceToken, destinationToken,
			quote.InputAmount, true);
		ValidateBalanceIssue(response, sourceToken);
		var spender = GetAllowanceTarget(response);
		await RpcClient.VerifyContractAsync(spender, "AllowanceHolder",
			cancellationToken);
		...
		if (wasApproved)
		{
			quote = await GetOrderQuoteAsync(market, regMsg.Side,
				requestedBaseAmount, cancellationToken);
			response = await HttpClient.GetQuoteAsync(sourceToken.Address,
				destinationToken.Address, quote.InputAmount,
				RpcClient.WalletAddress, SlippageTolerance,
				cancellationToken);
			ValidateQuote(response, sourceToken, destinationToken,
				quote.InputAmount, true);
			ValidateBalanceIssue(response, sourceToken);
			var refreshedSpender = GetAllowanceTarget(response);
			await RpcClient.VerifyContractAsync(refreshedSpender,
				"AllowanceHolder", cancellationToken);
			_ = await EnsureApprovalAsync(sourceToken, refreshedSpender,
				quote.InputAmount, cancellationToken);
		}

		var destinationAmount = ValidateSwapResponse(response, sourceToken,
			destinationToken, quote.InputAmount);
		var transaction = ToTransaction(response.Transaction);
		await RpcClient.VerifyContractAsync(transaction.To,
			"0x transaction target", cancellationToken);
		...
		OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"A broadcast 0x transaction cannot be replaced through the " +
			"Swap API v2.");
	}
		...
		OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"0x Swap API v2 has no cancellable order book. Pending EVM " +
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"0x Swap API v2 has no open-order group to cancel.");
	}
		...
		OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (statusMsg.OrderId is not null)
			throw new NotSupportedException(
				"0x swaps use EVM transaction hashes, not numeric order " +
				"identifiers.");
		if (!statusMsg.UserId.IsEmpty())
			throw new NotSupportedException(
				"0x Swap API v2 has no exchange-side user identifier.");
		if (statusMsg.SecurityIds.Length > 0)
			throw new NotSupportedException(
				"Use the primary security filter for 0x order status.");
	...
	private async ValueTask<ZeroXQuote> GetOrderQuoteAsync(
		ZeroXMarket market, Sides side, BigInteger requestedBaseAmount,
		CancellationToken cancellationToken)
	{
		if (side == Sides.Sell)
		...
		if (adjustedInput <= 0)
			throw new InvalidDataException(
				"0x returned an invalid market-buy input estimate.");
	...
	private static BigInteger ValidateSwapResponse(ZeroXQuoteResponse response,
		ZeroXToken sourceToken, ZeroXToken destinationToken,
		BigInteger requestedAmount)
	{
		ValidateQuote(response, sourceToken, destinationToken, requestedAmount,
			true);
		var destinationAmount = response.BuyAmount.ParseInteger();
		if (destinationAmount <= 0)
			throw new InvalidDataException(
				"0x API returned a non-positive swap output amount.");
		return destinationAmount;
	}

	private static string GetAllowanceTarget(ZeroXQuoteResponse response)
	{
		ArgumentNullException.ThrowIfNull(response);
		var target = response.AllowanceTarget;
		var issueTarget = response.Issues?.Allowance?.Spender;
		if (target.IsEmpty())
			target = issueTarget;
		else if (!issueTarget.IsEmpty() &&
			!target.NormalizeAddress().EqualsIgnoreCase(
				issueTarget.NormalizeAddress()))
			throw new InvalidDataException(
				"0x API returned conflicting allowance targets.");
		if (target.IsEmpty())
			throw new InvalidDataException(
				"0x API returned no AllowanceHolder target for an ERC-20 " +
				"swap.");
		return target.NormalizeAddress();
	}

	private static void ValidateBalanceIssue(ZeroXQuoteResponse response,
		ZeroXToken sourceToken)
	{
		var balance = response?.Issues?.Balance;
		if (balance is null)
			return;
		if (!balance.Token.NormalizeAddress().EqualsIgnoreCase(
				sourceToken.Address))
			throw new InvalidDataException(
				"0x API returned a balance issue for an unexpected token.");
		var actual = balance.Actual.ParseInteger();
		var expected = balance.Expected.ParseInteger();
		if (actual < expected)
			throw new InvalidOperationException(
				$"Insufficient {sourceToken.Symbol} balance: {actual} base " +
				$"units available, {expected} required.");
	}

	private ZeroXTransaction ToTransaction(ZeroXTransactionData data)
	{
		ArgumentNullException.ThrowIfNull(data);
		var value = data.Value.ParseInteger();
		if (value != BigInteger.Zero)
			throw new InvalidDataException(
				"A wrapped-token 0x swap must not transfer native value.");
		var gas = data.Gas.ParseInteger();
		if (gas <= 0)
			throw new InvalidDataException(
				"0x API returned a non-positive transaction gas limit.");
		if (!data.GasPrice.IsEmpty() && data.GasPrice.ParseInteger() < 0)
			throw new InvalidDataException(
				"0x API returned a negative gas price.");
		return new()
		{
	...
	private async ValueTask<bool> EnsureApprovalAsync(ZeroXToken token,
		string spender, BigInteger amount,
		CancellationToken cancellationToken)
	{
		if (amount <= 0)
			throw new InvalidOperationException(
				"0x approval amount must be positive.");
		...
		if (!IsAutoApprove)
			throw new InvalidOperationException(
				$"Token '{token.Symbol}' allowance for the 0x router is " +
	...
	private async ValueTask BroadcastAndConfirmAsync(
		ZeroXTransaction transaction, string operation,
		CancellationToken cancellationToken)
	{
		var hash = await RpcClient.SendTransactionAsync(transaction,
			cancellationToken);
		...
		if (!IsSuccessful(receipt))
			throw new InvalidOperationException(
				$"0x {operation} transaction '{hash}' reverted.");
	}
	...
	private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
		ZeroXRpcReceipt receipt, CancellationToken cancellationToken)
	{
	...
	private async ValueTask<
		(ZeroXToken Token, BigInteger Amount)[]> LoadBalancesAsync(
		CancellationToken cancellationToken)
	{
		ZeroXToken[] tokens;
		using (_sync.EnterScope())
	...
	private async ValueTask SendPortfolioSnapshotAsync(long target,
		bool isForced,
		(ZeroXToken Token, BigInteger Amount)[] balances,
		CancellationToken cancellationToken)
	{
	...
	private static decimal? GetCommission(ZeroXRpcReceipt receipt)
	{
	...
	private static decimal GetSwapPrice(ZeroXMarket market, Sides side,
		BigInteger sourceAmount, BigInteger destinationAmount)
	{
		...
		if (volume <= 0 || quote <= 0)
			throw new InvalidDataException(
				"0x returned non-positive swap amounts.");
		return quote / volume;
	}

	private ZeroXSwapExecution ReadSwapExecution(TrackedSwap swap,
		ZeroXRpcReceipt receipt)
	{
		...
		foreach (var log in receipt.Logs ?? [])
		{
			if (log?.IsRemoved != false || log.Address.IsEmpty() ||
				...
				!topics[0].EqualsIgnoreCase(ZeroXExtensions.TransferTopic))
				continue;
			string tokenAddress;
			try
			{
			...
			if (!isSource && !isDestination)
				continue;
			var from = ZeroXExtensions.ReadTopicAddress(topics[1]);
			var to = ZeroXExtensions.ReadTopicAddress(topics[2]);
			var amount = log.Data.ParseInteger();
			if (amount < 0)
				throw new InvalidDataException(
					"A 0x Transfer event contains a negative amount.");
			if (isSource)
			{
		...
		if (sourceAmount != swap.SourceAmount)
			throw new InvalidDataException(
				$"0x transaction '{swap.TransactionHash}' spent an " +
	...
	private static bool IsSuccessful(ZeroXRpcReceipt receipt)
