...
public partial class ZeroXMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"0x executable quotes do not expose historical Level1 " +
				"events.");
	...
	private SecurityMessage CreateSecurity(ZeroXMarket market,
	...
	private async ValueTask SendLevel1Async(ZeroXMarket market,
	...
	private async ValueTask<(decimal Bid, decimal BidVolume, decimal Ask,
		decimal AskVolume)> LoadLevel1Async(ZeroXMarket market,
		CancellationToken cancellationToken)
	{
		...
		if (bid <= 0 || ask <= 0 || askBaseVolume <= 0)
			throw new InvalidDataException(
				"0x returned a non-positive executable quote.");
	...
	private async ValueTask PollMarketAsync(
		CancellationToken cancellationToken)
	{
		(ZeroXMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
