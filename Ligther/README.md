# Ligther Connector for StockSharp

This directory contains the **Ligther** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `LigtherMessageAdapter` message adapter, exposing Ligther market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `LigtherSpot`.

## Configuration

`LigtherMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new LigtherMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Ligther connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/ligther.html).
