...
abstract class BaseNativeAdapter(LigtherMessageAdapter owner, string boardCode) : BaseLogReceiver, INativeAdapter
{
	protected LigtherMessageAdapter Owner { get; } = owner ?? throw new ArgumentNullException(nameof(owner));
	public string BoardCode { get; } = boardCode.ThrowIfEmpty(nameof(boardCode));

	public abstract LigtherSections Section { get; }
