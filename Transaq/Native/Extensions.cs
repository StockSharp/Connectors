...
static class Extensions
{
	...
	public static DateTime ToDate(this string s, DateTime now)
	{
		...
		if (DateTime.TryParseExact(s, _dateFormat, null, DateTimeStyles.None, out var date))
		{
			return date + now.TimeOfDay;
		}

		if (TimeSpan.TryParseExact(s, _timeFormatMls, null, out var time) || TimeSpan.TryParseExact(s, _timeFormat, null, out time))
		{
	...
	public static Unit GetElementValueToUnit(this XElement elem, string name)
	{
		...
		return elem.Element(name)?.Value.ToUnit(false);
	}
}
