...
public partial class JainamMessageAdapter
{
    ...
    private readonly SynchronizedDictionary<string, SecurityId> _securityIds =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly SynchronizedDictionary<string, JainamInstrument> _instruments =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly SynchronizedDictionary<string, JainamMarketUpdate> _marketStates =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly SynchronizedDictionary<
        string,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var instrument in await _restClient.GetInstruments(cancellationToken))
        {
            ...
            var instrumentKey =
                instrument.Exchange.ToInstrumentKey(instrument.Token);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (depth is < 1 or > 5)
        {
            throw new ArgumentOutOfRangeException(
                ...
                depth,
                "Jainam provides five market-depth levels.");
        }
    ...
    private async Task<JainamInstrument> GetInstrument(
        ...
        CancellationToken cancellationToken)
    {
        ...
        instrument = await _restClient.GetInstrument(
            ...
            throw new InvalidOperationException(
                $"Jainam instrument '{instrumentKey}' was not found in the official contract master.");
        _instruments[instrumentKey] = instrument;
        return instrument;
    }

    private async ValueTask OnMarketDataReceived(
        JainamMarketUpdate update,
        CancellationToken cancellationToken)
    {
        var instrumentKey =
            update.Exchange.ToInstrumentKey(update.Token);
        ...
        if (subscriptions.TryGetValue(DataType.Level1, out var level1Id))
        {
            ...
            .TryAdd(
                ...
                Positive(state.LastPrice) != null ? serverTime : null)
                ...
                previous != trade)
            {
                ...
                await SendOutMessageAsync(
                    new ExecutionMessage
                    {
                        ...
                        SecurityId = securityId,
                        TradeStringId =
                            $"{instrumentKey}:{serverTime.Ticks}:{lastPrice.ToString(CultureInfo.InvariantCulture)}",
