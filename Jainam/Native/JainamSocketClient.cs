...
sealed class JainamSocketClient : BaseLogReceiver
{
    ...
    private readonly SynchronizedDictionary<string, bool> _subscriptions =
        new(StringComparer.OrdinalIgnoreCase);
    private TaskCompletionSource<bool> _loginCompletion;

    public JainamSocketClient(
        string endpoint,
        string userId,
        string sessionToken,
    ...
    {
        var id = userId.ThrowIfEmpty(nameof(userId));
        _userId = id.EndsWith("_API", StringComparison.OrdinalIgnoreCase)
            ? id
            : id + "_API";
        var token = sessionToken.ThrowIfEmpty(nameof(sessionToken));
    ...
    public override string Name => nameof(Jainam) + "_" + nameof(JainamSocketClient);

    public event Func<JainamMarketUpdate, CancellationToken, ValueTask> MarketDataReceived;
    ...
    public ValueTask SendHeartbeat(CancellationToken cancellationToken)
        => Send(new JainamSocketHeartbeat(), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var envelope = JsonConvert.DeserializeObject<JainamSocketEnvelope>(
            content,
            _jsonSettings) ??
            throw new InvalidDataException(
                "Jainam returned an empty WebSocket message.");
        ...
        switch (envelope.Type?.ToLowerInvariant())
        {
            case "cf":
                {
                    var acknowledgement =
                        JsonConvert.DeserializeObject<JainamSocketAcknowledgement>(
                            content,
                            _jsonSettings) ??
                        throw new InvalidDataException(
                            "Jainam returned an invalid WebSocket acknowledgement.");
                    var status = acknowledgement.Status.IsEmpty(acknowledgement.Key);
                    if (!status.EqualsIgnoreCase("OK"))
                    {
                        var error = new InvalidOperationException(
                            $"Jainam WebSocket login failed: {status.IsEmpty("unknown status")}.");
            ...
            case "ck":
                {
                    var acknowledgement =
                        JsonConvert.DeserializeObject<JainamSocketAcknowledgement>(
                            ...
                            _jsonSettings);
                    var error = new InvalidOperationException(
                        $"Jainam WebSocket login failed: {acknowledgement?.Status.IsEmpty("NOT_OK")}.");
                    _loginCompletion?.TrySetException(error);
                    throw error;
                }
            ...
            case "df":
                {
                    var update = JsonConvert.DeserializeObject<JainamMarketUpdate>(
                        content,
                        _jsonSettings) ??
                        throw new InvalidDataException(
                            "Jainam returned an invalid market-data update.");
            ...
            default:
                this.AddVerboseLog(
                    "Ignored Jainam WebSocket message type {0}.",
                    envelope.Type);
                break;
        }
    }
    ...
    private ValueTask SendSubscription(
        ...
        CancellationToken cancellationToken)
        => Send(
            new JainamSocketSubscriptionRequest
            {
                Type = subscribe
                    ? isDepth ? "d" : "t"
                    : isDepth ? "ud" : "u",
    ...
    private ValueTask Send<T>(T request, CancellationToken cancellationToken)
        where T : class
        => _client.SendAsync(
            JsonConvert.SerializeObject(request, Formatting.None, _jsonSettings),
            cancellationToken);
