...
public partial class JainamMessageAdapter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (regMsg.TimeInForce == TimeInForce.MatchOrCancel)
            throw new NotSupportedException("Jainam does not expose fill-or-kill orders.");

        var condition = regMsg.Condition as JainamOrderCondition;
        var triggerPrice = condition?.TriggerPrice;
        if (orderType == OrderTypes.Conditional && triggerPrice is not > 0)
        {
            throw new InvalidOperationException(
                "A positive trigger price is required for a Jainam stop order.");
        }
        ...
        var complexity =
            condition?.Complexity ?? JainamOrderComplexities.Regular;
        var orderId = await _restClient.PlaceOrder(
            new JainamOrderRequest
            {
                ...
                Price = FormatPrice(
                    orderType is OrderTypes.Market ? 0 : regMsg.Price),
                Validity = regMsg.TimeInForce.ToValidity(),
                StopLossTriggerPrice = FormatOptional(triggerPrice),
                TrailingStopLoss =
                    FormatOptional(condition?.TrailingStopLoss),
                MarketProtectionPercent =
                    FormatOptional(condition?.MarketProtectionPercent),
                DisclosedQuantity =
                    disclosedQuantity > 0 ? disclosedQuantity : null,
        ...
        await SendOutMessageAsync(
            new ExecutionMessage
            {
                ...
                Condition = CreateCondition(
                    ...
                    condition?.TrailingStopLoss,
                    condition?.MarketProtectionPercent,
                    condition?.AlgoId,
                    condition?.OrderTag),
            },
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var current = await ResolveOrder(
            ...
            cancellationToken);
        var orderType =
            replaceMsg.OrderType ?? current.OrderType.ToOrderType();
        ValidateOrderType(orderType);
        var condition = replaceMsg.Condition as JainamOrderCondition;
        var triggerPrice =
            condition?.TriggerPrice ?? Positive(current.StopLossTriggerPrice);
        if (orderType == OrderTypes.Conditional && triggerPrice is not > 0)
        {
            throw new InvalidOperationException(
                "A positive trigger price is required for a Jainam stop order.");
        ...
        var quantity = ToQuantity(
            ...
            false);
        var disclosedQuantity = ToQuantity(
            condition?.DisclosedVolume ?? current.DisclosedQuantity,
            nameof(condition.DisclosedVolume),
            true);
        if (disclosedQuantity > quantity)
        {
        ...
        await _restClient.ModifyOrder(
            new JainamModifyOrderRequest
            {
                OrderId = current.OrderId,
                Quantity = quantity,
                OrderType = orderType.ToNative(replaceMsg.Price),
                Price = FormatPrice(
                    orderType is OrderTypes.Market ? 0 : replaceMsg.Price),
                StopLossTriggerPrice = FormatOptional(triggerPrice),
                Validity =
                    ((TimeInForce?)(
                ...
                DisclosedQuantity =
                    disclosedQuantity > 0 ? disclosedQuantity : null,
                MarketProtectionPercent = FormatOptional(
                    condition?.MarketProtectionPercent ??
                    Positive(current.MarketProtectionPercent)),
                TrailingStopLoss = FormatOptional(
                    condition?.TrailingStopLoss ??
                    Positive(current.TrailingStopLoss)),
            },
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await SendOutMessageAsync(
            ...
            .TryAdd(
                ...
                limits.UtilizedMargin + limits.BlockedForPayout,
                true),
            cancellationToken);
        ...
        foreach (var position in await _restClient.GetPositions(cancellationToken))
        {
            ...
            await SendOutMessageAsync(
                ...
                .TryAdd(
                    ...
                    Positive(position.PreviousDayClose),
                    true)
                .TryAdd(
        ...
        foreach (var holding in await _restClient.GetHoldings(cancellationToken))
        {
            if (holding == null)
                continue;
            var exchange =
                !holding.NseInstrumentId.IsEmpty() ? "NSE" : "BSE";
            var instrumentId = exchange == "NSE"
                ? holding.NseInstrumentId
                : holding.BseInstrumentId;
            var tradingSymbol = exchange == "NSE"
                ? holding.NseTradingSymbol
                : holding.BseTradingSymbol;
            ...
            await SendOutMessageAsync(
                new PositionChangeMessage
                {
                    OriginalTransactionId = originalTransactionId,
                    PortfolioName = _resolvedPortfolioName,
                    SecurityId = exchange.ToSecurityId(
                        instrumentId,
                        tradingSymbol),
                    ServerTime = CurrentTime,
                }
                .TryAdd(
                    ...
                    holding.TotalQuantity != 0
                        ...
                        : holding.DpQuantity + holding.T1Quantity,
                    true)
                .TryAdd(
                    PositionChangeTypes.BlockedValue,
                ...
                .TryAdd(
                    PositionChangeTypes.CurrentPrice,
                    Positive(holding.PreviousDayClose),
                    true),
                cancellationToken);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var balance = order.PendingQuantity >= 0
        ...
        await SendOutMessageAsync(
            new ExecutionMessage
            {
                ...
                SecurityId = await GetSecurityId(
                    ...
                    cancellationToken),
                PortfolioName =
                    order.ClientId.IsEmpty(_resolvedPortfolioName),
                ...
                Condition = CreateCondition(
                    ...
                    order.OrderComplexity.ToComplexity(),
                    Positive(order.StopLossTriggerPrice),
                    Positive(order.DisclosedQuantity),
                    Positive(order.TrailingStopLoss),
                    Positive(order.MarketProtectionPercent),
                    order.AlgoId,
                    order.OrderTag),
                Error = state == OrderStates.Failed
                    ? new InvalidOperationException(
                        order.RejectionReason.IsEmpty(
                            $"Jainam order status: {order.OrderStatus}."))
                    : null,
            },
            cancellationToken);
    }
        ...
        await SendOutMessageAsync(
            new ExecutionMessage
            {
                ...
                SecurityId = await GetSecurityId(
                    ...
                    cancellationToken),
                PortfolioName =
                    trade.ClientId.IsEmpty(_resolvedPortfolioName),
                Side = trade.TransactionType.ToSide(),
                TradePrice = trade.TradedPrice,
                TradeVolume = trade.FilledQuantity,
                ServerTime =
                    trade.FillTime.ToJainamTime() ??
                    trade.OrderTime.ToJainamTime() ??
                    CurrentTime,
            },
            cancellationToken);
    }

    private async Task<JainamOrder> ResolveOrder(
        string orderId,
        long originalTransactionId,
        CancellationToken cancellationToken)
    {
        ...
        bool allowZero)
    {
            ...
            value > long.MaxValue)
        {
            throw new ArgumentOutOfRangeException(
                parameterName,
                value,
                "Jainam quantities must be non-negative whole numbers within Int64 range.");
        }
    ...
    private static JainamOrderCondition CreateCondition(
        JainamProducts product,
        JainamOrderComplexities complexity,
        decimal? triggerPrice,
        decimal? disclosedVolume,
        decimal? trailingStopLoss,
        decimal? marketProtectionPercent,
        string algoId,
        string orderTag)
        => new()
        {
    ...
    private DateTime GetOrderTime(JainamOrder order)
        => order.ExchangeTimestamp.ToJainamTime() ??
            order.ExchangeUpdateTime.ToJainamTime() ??
            order.BrokerUpdateTime.ToJainamTime() ??
            order.OrderTime.ToJainamTime() ??
            CurrentTime;
}
