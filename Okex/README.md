# Okex Connector for StockSharp

This directory contains the **Okex** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `OkexMessageAdapter` message adapter, exposing Okex market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
`OkexMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new OkexMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Okex connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/okex.html).
