# Mexc Connector for StockSharp

This directory contains the **Mexc** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `MexcMessageAdapter` message adapter, exposing Mexc market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Mexc`.

## Configuration

`MexcMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new MexcMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Mexc connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/mexc.html).
