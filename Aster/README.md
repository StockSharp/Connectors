# Aster Connector for StockSharp

This directory contains the **Aster** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `AsterMessageAdapter` message adapter, exposing Aster market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `AsterSpot`.

## Configuration

`AsterMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new AsterMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Aster connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/aster.html).
