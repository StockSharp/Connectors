...
sealed class MeteoraRpcClient : BaseLogReceiver
{
	...
	public MeteoraRpcClient(string endpoint, MeteoraClusters cluster,
		string walletAddress, SecureString privateKey)
	{
		...
		if (!walletAddress.IsEmpty())
		{
				...
				!walletAddress.Equals(derivedAddress, StringComparison.Ordinal))
			{
				...
				throw new ArgumentException(
					"The configured Meteora wallet does not match the private key.",
	...
	public override string Name => "Meteora_JSON-RPC";
	...
	public async ValueTask VerifyAsync(CancellationToken cancellationToken)
	{
		var health = await SendAsync<string>("getHealth",
			new MeteoraRpcEmptyParameters(), cancellationToken);
		if (!health.EqualsIgnoreCase("ok"))
			throw new InvalidDataException(
				$"Solana RPC health check returned '{health}'.");
		var program = await GetAccountAsync(MeteoraExtensions.ProgramAddress,
			cancellationToken);
	...
	public async ValueTask<MeteoraRpcAccount> GetAccountAsync(string address,
		CancellationToken cancellationToken)
		=> (await SendAsync<MeteoraRpcContextValue<MeteoraRpcAccount>>(
			"getAccountInfo", new MeteoraRpcAddressAccountParameters
			{
				Address = address.NormalizePublicKey(),
				Config = new(),
			}, cancellationToken)).Value;

	public async ValueTask<MeteoraRpcAccount[]> GetAccountsAsync(
		IEnumerable<string> addresses, CancellationToken cancellationToken)
	{
		...
		return (await SendAsync<MeteoraRpcContextValue<MeteoraRpcAccount[]>>(
			"getMultipleAccounts", new MeteoraRpcAddressesAccountParameters
			{
	...
	{
		EnsureWallet();
		return (await SendAsync<MeteoraRpcContextValue<ulong>>("getBalance",
			new MeteoraRpcAddressCommitmentParameters
			{
	...
	public ValueTask<MeteoraRpcSignatureInfo[]> GetSignaturesAsync(
		string address, string before, int limit,
		CancellationToken cancellationToken)
	{
		if (limit is < 1 or > 1000)
			throw new ArgumentOutOfRangeException(nameof(limit));
		return SendAsync<MeteoraRpcSignatureInfo[]>("getSignaturesForAddress",
			new MeteoraRpcSignaturesParameters
			{
				Address = address.NormalizePublicKey(),
				Config = new()
				{
	...
	public ValueTask<MeteoraRpcTransaction> GetTransactionAsync(
		string signature, CancellationToken cancellationToken)
		=> SendAsync<MeteoraRpcTransaction>("getTransaction",
			new MeteoraRpcTransactionParameters
			{
		...
		CancellationToken cancellationToken)
	{
		var accounts = (writableAccounts ?? []).Select(static address =>
			...
			.Take(128).ToArray();
		var fees = await SendAsync<MeteoraRpcRecentFee[]>(
			"getRecentPrioritizationFees", new MeteoraRpcRecentFeesParameters
			{
				Addresses = accounts,
			}, cancellationToken) ?? [];
	...
	public async ValueTask<string> SendTransactionAsync(
		...
		CancellationToken cancellationToken, Account additionalSigner)
	{
		EnsureSigning();
		...
		var latest = await SendAsync<
			MeteoraRpcContextValue<MeteoraRpcLatestBlockhash>>(
			"getLatestBlockhash", new MeteoraRpcLatestBlockhashParameters
			{
				Config = new(),
			}, cancellationToken);
		...
		else
		{
			try
			{
				transaction = builder.Build([_account, additionalSigner]);
			}
			finally
			{
				if (additionalSigner.PrivateKey.KeyBytes is { } key)
					CryptographicOperations.ZeroMemory(key);
			}
		}
		return await SendAsync<string>("sendTransaction",
			new MeteoraRpcSendTransactionParameters
			{
	...
	public async ValueTask<MeteoraTransactionReceipt> GetReceiptAsync(
		string signature, CancellationToken cancellationToken)
	{
		...
		MeteoraRpcParameters parameters, CancellationToken cancellationToken)
	{
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			var request = new MeteoraRpcRequest<MeteoraRpcParameters>
			{
			...
			MeteoraRpcResponse<TResult> envelope;
			try
			{
				envelope = JsonConvert.DeserializeObject<
					MeteoraRpcResponse<TResult>>(body, _serializerSettings);
			}
	...
	private void EnsureSigning()
	{
		...
		if (!IsSigningAvailable)
			throw new InvalidOperationException(
				"A Solana private key is required to sign Meteora transactions.");
	}
