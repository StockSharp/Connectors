...
public partial class AerodromeMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			_ = await GetOrLoadTokenAsync(
				AerodromeExtensions.NativeTokenAddress, cancellationToken);
			AerodromeMarketDefinition[] definitions;
			try
			{
			...
			{
				throw new InvalidOperationException(
					"Configured Aerodrome pool definitions are invalid.",
					error);
			}
			...
			foreach (var definition in definitions)
			{
					...
					!cancellationToken.IsCancellationRequested)
				{
					errors.Add(error);
					this.AddWarningLog(
						"Aerodrome pool {0} loading failed: {1}",
			...
			if (markets.Length == 0)
				throw errors.Count == 1
					...
					: new AggregateException(
						"No Aerodrome pools could be loaded.", errors);
			if (!WebSocketEndpoint.IsEmpty())
			{
				...
				{
					this.AddWarningLog(
						"Aerodrome WebSocket is unavailable; JSON-RPC log " +
						"polling remains active: {0}", error.Message);
					DisposeSocket();
				}
			}
			connectMsg.SessionId = RpcClient.IsWalletConfigured
				? $"Aerodrome Base {RpcClient.WalletAddress}"
				: "Aerodrome Base public";
	...
	private async ValueTask RegisterDefinitionAsync(
		AerodromeMarketDefinition definition,
		CancellationToken cancellationToken)
	{
		var pool = await RpcClient.GetPoolAsync(definition.PoolId,
			cancellationToken);
		AerodromeToken baseToken;
		AerodromeToken quoteToken;
	...
	private async ValueTask<AerodromeToken> GetOrLoadTokenAsync(
	...
	private void RegisterMarket(AerodromeMarket market)
	{
		if (market?.PoolId.IsEmpty() != false || market.BaseToken is null ||
			...
			throw new InvalidDataException(
				"Aerodrome pool metadata is incomplete.");
		using (_sync.EnterScope())
		{
			if (_marketsByPool.ContainsKey(market.PoolId))
				throw new InvalidOperationException(
					$"Aerodrome pool '{market.PoolId}' is configured twice.");
			if (_markets.ContainsKey(market.SecurityCode))
				throw new InvalidOperationException(
					$"Aerodrome security code '{market.SecurityCode}' is " +
					"configured twice.");
	...
	private AerodromeMarketDefinition[] ParseMarketDefinitions()
	{
		if (Pools.IsEmpty())
			throw new InvalidOperationException(
			...
			StringSplitOptions.TrimEntries))
		{
			...
			if (fields.Length is not 1 and not 3 and not 4)
				throw new FormatException(
					"Each Aerodrome pool must use pool or " +
					"pool|base-token|quote-token|security-code format.");
			if (fields.Length >= 3 &&
				(fields[1].IsEmpty() || fields[2].IsEmpty()))
				throw new FormatException(
					"Both Aerodrome base and quote token addresses are " +
	...
	private static (AerodromeToken BaseToken, AerodromeToken QuoteToken)
		OrientMarket(AerodromeToken token0, AerodromeToken token1)
	...
	private static string CreateSecurityCode(AerodromeToken baseToken,
		AerodromeToken quoteToken, AerodromePool pool)
	{
		var suffix = pool.PoolType switch
		{
			AerodromePoolTypes.Volatile => "VOLATILE",
			AerodromePoolTypes.Stable => "STABLE",
			AerodromePoolTypes.Slipstream => $"CL{pool.TickSpacing}",
	...
	private static string NormalizeSecurityCode(string value)
	{
		...
		if (value.Length > 64 || value.Any(static ch =>
			...
			throw new FormatException(
				$"Invalid Aerodrome security code '{value}'.");
		return value;
	}
