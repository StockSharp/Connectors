...
public partial class AerodromeMessageAdapter
{
    ...
    {
        public AerodromeMarket Market { get; init; }
    }
    ...
    private sealed class TrackedSwap
    {
        ...
        public AerodromeRpcReceipt Receipt { get; set; }
    }
    ...
    private readonly Dictionary<string, AerodromeMarket> _markets =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, AerodromeMarket> _marketsByPool =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, AerodromeToken> _tokens =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    private readonly Queue<AerodromeRpcLog> _realtimeLogs = [];
    private readonly Dictionary<BigInteger, DateTime> _blockTimes = [];
    private readonly Queue<BigInteger> _blockTimeOrder = [];
    private AerodromeRpcClient _rpcClient;
    private AerodromeSocketClient _socketClient;
    private DateTime _nextMarketPoll;
    private DateTime _nextPrivatePoll;

    /// <summary>Initializes the adapter.</summary>
    public AerodromeMessageAdapter(IdGenerator transactionIdGenerator)
    ...
    /// <inheritdoc />
    public override string[] AssociatedBoards => [BoardCodes.Aerodrome];

    /// <inheritdoc />
    protected override bool ValidateSecurityId(SecurityId securityId)
        => securityId.BoardCode.IsEmpty() ||
            securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Aerodrome) ||
            securityId.IsAssociated(BoardCodes.Aerodrome);

    private AerodromeRpcClient RpcClient => _rpcClient ?? throw new
    ...
    private void EnsureTradingReady()
    {
        ...
        if (!RpcClient.IsSigningAvailable)
            throw new InvalidOperationException(
                "An EVM private key is required for Aerodrome swaps.");
    }

    private AerodromeMarket GetMarket(SecurityId securityId)
    {
        if (!ValidateSecurityId(securityId))
            throw new InvalidOperationException(
                $"Security board '{securityId.BoardCode}' is not Aerodrome.");
        ...
        using (_sync.EnterScope())
            return _markets.TryGetValue(code, out var market)
                ? market
                : throw new InvalidOperationException(
                    $"Unknown Aerodrome market '{code}'.");
    }

    private string GetPortfolioName()
        => $"Aerodrome_Base_{RpcClient.WalletAddress[2..10]}";
    ...
    private void ValidatePortfolio(string portfolioName)
    {
        ...
        if (!portfolioName.IsEmpty() &&
            ...
            throw new InvalidOperationException(
                $"Unknown Aerodrome portfolio '{portfolioName}'.");
    }

    private void OnSocketLog(AerodromeRpcLog log)
    {
        if (log is null)
            return;
        using (_sync.EnterScope())
