...
public partial class AerodromeMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Aerodrome quote probes do not expose historical Level1 " +
				"events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!AllTimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"Aerodrome does not support the {timeFrame} candle interval.");
		var now = DateTime.UtcNow;
		var to = (mdMsg.To ?? now).ToUniversalTime().Min(now);
		var maximum = GetSubscriptionMaximum(mdMsg.Count);
	...
	private SecurityMessage CreateSecurity(AerodromeMarket market,
	...
	private async ValueTask SendLevel1Async(AerodromeMarket market,
	...
	private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
		AerodromeMarket market, CancellationToken cancellationToken)
	{
		...
		var bidQuote = await RpcClient.GetQuoteAsync(market,
			AerodromeTradeTypes.ExactInput, units, cancellationToken);
		var askQuote = await RpcClient.GetQuoteAsync(market,
			AerodromeTradeTypes.ExactOutput, units, cancellationToken);
		...
		if (bid <= 0 || ask <= 0)
			throw new InvalidDataException(
				"Aerodrome returned a non-positive quote price.");
		return (bid, ask);
	}

	private ValueTask SendLevel1Async(AerodromeMarket market, long target,
	...
	private async ValueTask<AerodromeTrade[]> LoadTradesAsync(
		AerodromeMarket market, DateTime from, DateTime to, int maximum,
	...
	private async ValueTask<AerodromeTrade[]> LoadTradesByBlocksAsync(
		AerodromeMarket market, BigInteger fromBlock, BigInteger toBlock,
	...
	private async ValueTask<AerodromeTrade> ToTradeAsync(
		AerodromeMarket market, AerodromeRpcLog log,
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			{
				amount0 = AerodromeExtensions.ReadAbiSignedWord(log.Data, 0);
				amount1 = AerodromeExtensions.ReadAbiSignedWord(log.Data, 1);
			}
			else
			{
	...
	private async ValueTask<bool> SendTradeAsync(AerodromeMarket market,
		AerodromeTrade trade, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask<AerodromeCandle[]> LoadCandlesAsync(
		AerodromeMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
	...
	private ValueTask SendCandleAsync(AerodromeMarket market,
		AerodromeCandle candle, TimeSpan timeFrame, long target,
		CancellationToken cancellationToken)
	{
	...
	private async ValueTask PollLevel1Async(
		CancellationToken cancellationToken)
	{
		(AerodromeMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
