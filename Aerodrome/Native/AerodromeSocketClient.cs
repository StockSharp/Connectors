...
sealed class AerodromeSocketClient : BaseLogReceiver
{
	...
	public AerodromeSocketClient(string endpoint)
	{
	...
	public override string Name => "Aerodrome_WebSocket";

	public event Action<AerodromeRpcLog> LogReceived;

	public async ValueTask ConnectAsync(IEnumerable<AerodromeMarket> markets,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(markets);
		if (_socket is not null)
			throw new InvalidOperationException(
				"The Aerodrome WebSocket is already connected.");
	...
	protected override void DisposeManaged()
	{
		...
		_socket?.Dispose();
		_lifetime?.Dispose();
		FailPending(new ObjectDisposedException(nameof(AerodromeSocketClient)));
		_sendGate.Dispose();
		_socket = null;
		_lifetime = null;
		_receiveTask = null;
	...
	private async ValueTask SubscribeAsync(AerodromeMarket market,
		CancellationToken cancellationToken)
	{
		...
		var payload = JsonConvert.SerializeObject(new AerodromeSocketRequest
		{
			Id = id,
			Parameters = new()
			{
		...
		try
		{
			...
			if (subscription.IsEmpty())
				throw new InvalidDataException(
					"Aerodrome returned an empty WebSocket subscription id.");
		}
		catch
		{
			using (_sync.EnterScope())
		...
		CancellationToken cancellationToken)
	{
		var socket = _socket ?? throw new InvalidOperationException(
			"The Aerodrome WebSocket is not connected.");
	...
	private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
	{
		try
		{
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				AerodromeSocketMessage message;
				try
				{
					message = JsonConvert.DeserializeObject<
						AerodromeSocketMessage>(payload, _jsonSettings);
				}
				catch (JsonException error)
				{
					throw new InvalidDataException(
						"Aerodrome WebSocket returned an unexpected payload.",
						error);
				}
				if (message is null)
					throw new InvalidDataException(
						"Aerodrome WebSocket returned an empty payload.");
				...
				if (message.Method.EqualsIgnoreCase("eth_subscription") &&
					message.Parameters?.Result is AerodromeRpcLog log)
				{
					try
					{
		...
		{
			this.AddWarningLog(
				"Aerodrome WebSocket receive loop stopped: {0}",
				error.Message);
			FailPending(error);
		}
	}
		...
		while (true)
		{
			...
			if (result.MessageType == WebSocketMessageType.Close)
				throw new WebSocketException(
					$"Aerodrome WebSocket closed with status " +
					$"'{socket.CloseStatus}'.");
			if (result.MessageType != WebSocketMessageType.Text)
				throw new InvalidDataException(
					"Aerodrome WebSocket returned a non-text message.");
			if (target.Length + result.Count > _maximumMessageBytes)
				throw new InvalidDataException(
					"Aerodrome WebSocket message exceeds 1 MiB.");
