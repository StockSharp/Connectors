...
sealed class ManifestTradeDataReader
{
	private static readonly UTF8Encoding _utf8 = new(false, true);
	private readonly byte[] _data;
	private int _offset;

	public ManifestTradeDataReader(byte[] data)
	...
	public void ReadDiscriminator(byte[] expected, string typeName)
	{
		...
		if (!actual.SequenceEqual(expected))
			throw new InvalidDataException(
				$"Account or event data is not a Manifest Trade {typeName}.");
	}
	...
	private void EnsureAvailable(int length)
	{
		if (length < 0 || length > Remaining)
			throw new InvalidDataException(
				"Manifest Trade account or event data is truncated.");
	}
}
