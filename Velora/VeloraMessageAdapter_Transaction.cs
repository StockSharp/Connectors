...
public partial class VeloraMessageAdapter
{
		...
		OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		if (regMsg.OrderType is not (null or OrderTypes.Market))
			throw new NotSupportedException(
				"Velora Market API supports immediate market swaps only.");
		...
		if (regMsg.Condition is not null)
			throw new NotSupportedException(
				"Velora Market API does not expose conditional orders.");
		...
		if (regMsg.TillDate is not null)
			throw new NotSupportedException(
				"An expiry cannot be attached to a Velora Market API request.");
		...
		await ValidateBalanceAsync(sourceToken, quote.InputAmount,
			cancellationToken);
		var route = await HttpClient.GetPriceAsync(sourceToken.Address,
			sourceToken.Decimals, destinationToken.Address,
			destinationToken.Decimals, quote.InputAmount,
			RpcClient.WalletAddress, cancellationToken);
		var destinationAmount = ValidatePriceRoute(route, sourceToken,
			destinationToken, quote.InputAmount, Chain);
		var router = GetRouteTarget(route);
		await RpcClient.VerifyContractAsync(router, "Augustus v6.2 router",
			cancellationToken);
		...
		if (wasApproved)
		{
			...
			await ValidateBalanceAsync(sourceToken, quote.InputAmount,
				cancellationToken);
			route = await HttpClient.GetPriceAsync(sourceToken.Address,
				sourceToken.Decimals, destinationToken.Address,
				destinationToken.Decimals, quote.InputAmount,
				RpcClient.WalletAddress, cancellationToken);
			destinationAmount = ValidatePriceRoute(route, sourceToken,
				destinationToken, quote.InputAmount, Chain);
			router = GetRouteTarget(route);
			await RpcClient.VerifyContractAsync(router,
				"Augustus v6.2 router", cancellationToken);
			_ = await EnsureApprovalAsync(sourceToken, router,
				quote.InputAmount, cancellationToken);
		}

		var built = await HttpClient.BuildTransactionAsync(route,
			sourceToken.Address, sourceToken.Decimals,
			destinationToken.Address, destinationToken.Decimals,
			quote.InputAmount, RpcClient.WalletAddress, SlippageTolerance,
			cancellationToken);
		var transaction = ToTransaction(built, route);
		...
		OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"A broadcast Velora transaction cannot be replaced through the " +
				"Market API.");
	}
		...
		OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"Velora Market API has no cancellable order book. Pending EVM " +
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"Velora Market API has no open-order group to cancel.");
	}
		...
		OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (statusMsg.OrderId is not null)
			throw new NotSupportedException(
				"Velora swaps use EVM transaction hashes, not numeric order " +
				"identifiers.");
		if (!statusMsg.UserId.IsEmpty())
			throw new NotSupportedException(
				"Velora Market API has no exchange-side user identifier.");
		if (statusMsg.SecurityIds.Length > 0)
			throw new NotSupportedException(
				"Use the primary security filter for Velora order status.");
	...
	private async ValueTask<VeloraQuote> GetOrderQuoteAsync(
		VeloraMarket market, Sides side, BigInteger requestedBaseAmount,
		CancellationToken cancellationToken)
	{
		if (side == Sides.Sell)
		...
		if (adjustedInput <= 0)
			throw new InvalidDataException(
				"Velora returned an invalid market-buy input estimate.");
	...
	private async ValueTask ValidateBalanceAsync(VeloraToken sourceToken,
	...
	private VeloraTransaction ToTransaction(VeloraTransactionData data,
		JObject route)
	{
		...
		if (!from.EqualsIgnoreCase(RpcClient.WalletAddress))
			throw new InvalidDataException(
				"Velora transaction sender does not match the configured " +
					"wallet.");
		if (data.ChainId is int chainId && chainId != (int)Chain)
			throw new InvalidDataException(
				"Velora transaction belongs to a different network.");
		var target = GetRouteTarget(route);
		if (!data.To.NormalizeAddress().EqualsIgnoreCase(target))
			throw new InvalidDataException(
				"Velora transaction target does not match its price route.");
		var value = data.Value.ParseInteger();
		if (value != BigInteger.Zero)
			throw new InvalidDataException(
				"A wrapped-token Velora swap must not transfer native value.");
		var gas = data.Gas.IsEmpty()
			? BigInteger.Zero
			: data.Gas.ParseInteger();
		if (gas < 0)
			throw new InvalidDataException(
		...
		{
			data.GasPrice,
			data.MaximumFeePerGas,
			data.MaximumPriorityFeePerGas,
		})
		{
			if (!fee.IsEmpty() && fee.ParseInteger() < 0)
				throw new InvalidDataException(
	...
	private async ValueTask<bool> EnsureApprovalAsync(VeloraToken token,
		string spender, BigInteger amount,
		CancellationToken cancellationToken)
	{
		if (amount <= 0)
			throw new InvalidOperationException(
				"Velora approval amount must be positive.");
		...
		if (!IsAutoApprove)
			throw new InvalidOperationException(
				$"Token '{token.Symbol}' allowance for the Velora router is " +
	...
	private async ValueTask BroadcastAndConfirmAsync(
		VeloraTransaction transaction, string operation,
		CancellationToken cancellationToken)
	{
		var hash = await RpcClient.SendTransactionAsync(transaction,
			cancellationToken);
		...
		if (!IsSuccessful(receipt))
			throw new InvalidOperationException(
				$"Velora {operation} transaction '{hash}' reverted.");
	}
	...
	private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
		VeloraRpcReceipt receipt, CancellationToken cancellationToken)
	{
	...
	private async ValueTask<
		(VeloraToken Token, BigInteger Amount)[]> LoadBalancesAsync(
		CancellationToken cancellationToken)
	{
		VeloraToken[] tokens;
		using (_sync.EnterScope())
	...
	private async ValueTask SendPortfolioSnapshotAsync(long target,
		bool isForced,
		(VeloraToken Token, BigInteger Amount)[] balances,
		CancellationToken cancellationToken)
	{
	...
	private static decimal? GetCommission(VeloraRpcReceipt receipt)
	{
	...
	private static decimal GetSwapPrice(VeloraMarket market, Sides side,
		BigInteger sourceAmount, BigInteger destinationAmount)
	{
		...
		if (volume <= 0 || quote <= 0)
			throw new InvalidDataException(
				"Velora returned non-positive swap amounts.");
		return quote / volume;
	}

	private VeloraSwapExecution ReadSwapExecution(TrackedSwap swap,
		VeloraRpcReceipt receipt)
	{
		...
		foreach (var log in receipt.Logs ?? [])
		{
			if (log?.IsRemoved != false || log.Address.IsEmpty() ||
				...
				!topics[0].EqualsIgnoreCase(VeloraExtensions.TransferTopic))
				continue;
			string tokenAddress;
			try
			{
			...
			if (!isSource && !isDestination)
				continue;
			var from = VeloraExtensions.ReadTopicAddress(topics[1]);
			var to = VeloraExtensions.ReadTopicAddress(topics[2]);
			var amount = log.Data.ParseInteger();
			if (amount < 0)
				throw new InvalidDataException(
					"A Velora Transfer event contains a negative amount.");
			if (isSource)
			{
				if (from.EqualsIgnoreCase(RpcClient.WalletAddress))
					sourceAmount += amount;
		...
		if (sourceAmount != swap.SourceAmount)
			throw new InvalidDataException(
				$"Velora transaction '{swap.TransactionHash}' spent an " +
	...
	private static bool IsSuccessful(VeloraRpcReceipt receipt)
