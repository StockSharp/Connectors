...
sealed class VeloraRpcClient : BaseLogReceiver
{
	...
	public VeloraRpcClient(string endpoint, VeloraChains chain,
	...
	public override string Name => "Velora_JSON-RPC";

	public VeloraChains Chain { get; }
	...
	public bool IsWalletConfigured =>
		!WalletAddress.EqualsIgnoreCase(VeloraExtensions.ProbeAddress);
	...
	{
		var chainId = (await SendAsync<VeloraRpcEmptyParameters, string>(
			"eth_chainId", new(), true, cancellationToken)).ParseInteger();
		if (chainId != new BigInteger((int)Chain))
			throw new InvalidOperationException(
				$"JSON-RPC is connected to chain {chainId}, but Velora " +
				$"chain {(int)Chain} is configured.");
	}

	public async ValueTask<VeloraToken> GetTokenAsync(string address,
	...
	public async ValueTask<BigInteger> GetBalanceAsync(VeloraToken token,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
			return (await SendAsync<VeloraRpcAddressTagParameters, string>(
				"eth_getBalance", new()
				{
					Address = WalletAddress,
					BlockTag = "latest",
				}, true, cancellationToken)).ParseInteger();
		var data = VeloraExtensions.EncodeStaticCall(
			"balanceOf(address)",
			VeloraExtensions.AbiAddress(WalletAddress));
	...
	public async ValueTask<BigInteger> GetAllowanceAsync(
		VeloraToken token, string spender,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		var data = VeloraExtensions.EncodeStaticCall(
			...
			VeloraExtensions.AbiAddress(WalletAddress),
			VeloraExtensions.AbiAddress(spender));
		return (await CallContractAsync(token.Address, data,
			cancellationToken)).ParseInteger();
	}

	public VeloraTransaction CreateApprovalTransaction(
		VeloraToken token, string spender, BigInteger amount)
	{
		ArgumentNullException.ThrowIfNull(token);
		if (token.Address.IsNativeToken())
		...
		return new()
		{
			...
			Data = VeloraExtensions.EncodeStaticCall(
				"approve(address,uint256)",
				VeloraExtensions.AbiAddress(spender),
				VeloraExtensions.AbiWord(amount)),
			Value = BigInteger.Zero,
		};
	}

	public async ValueTask<BigInteger> EstimateGasAsync(
		VeloraTransaction transaction,
		CancellationToken cancellationToken)
	{
		ValidateTransaction(transaction);
		var call = ToRpcCall(transaction);
		return (await SendAsync<VeloraRpcCallOnlyParameters, string>(
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			var nonce = (await SendAsync<VeloraRpcAddressTagParameters,
			...
			else
			{
				var gasPrice = (await SendAsync<
					VeloraRpcEmptyParameters, string>("eth_gasPrice",
			...
			var hash = await SendAsync<VeloraRpcValueParameters, string>(
	...
	public async ValueTask<VeloraRpcReceipt> GetReceiptAsync(string hash,
		CancellationToken cancellationToken)
	{
		hash = hash.NormalizeHash();
		var receipt = await SendAsync<VeloraRpcValueParameters,
			VeloraRpcReceipt>("eth_getTransactionReceipt", new()
			{
				Value = hash,
	...
	public async ValueTask<VeloraRpcReceipt> WaitForReceiptAsync(
	...
	public async ValueTask<VeloraRpcBlock> GetBlockAsync(
		BigInteger blockNumber, CancellationToken cancellationToken)
	{
		if (blockNumber < 0)
			throw new ArgumentOutOfRangeException(nameof(blockNumber));
		var block = await SendAsync<VeloraRpcTagBooleanParameters,
			VeloraRpcBlock>("eth_getBlockByNumber", new()
			{
	...
	public async ValueTask VerifyContractAsync(string address, string name,
		CancellationToken cancellationToken)
	{
		var code = await SendAsync<VeloraRpcAddressTagParameters, string>(
			"eth_getCode", new()
			{
				Address = address.NormalizeAddress(),
				BlockTag = "latest",
		...
		if (code.IsEmpty() || code.EqualsIgnoreCase("0x") ||
			...
			throw new InvalidDataException(
				$"Velora {name} '{address}' is not a contract on " +
				$"chain {(int)Chain}.");
	}
		...
		TryGetEip1559FeesAsync(CancellationToken cancellationToken)
	{
		try
		{
			var block = await SendAsync<
				VeloraRpcTagBooleanParameters, VeloraRpcBlock>(
			...
			var priority = (await SendAsync<
				VeloraRpcEmptyParameters, string>(
	...
	private async ValueTask<string> CallContractAsync(string address,
		...
		=> await SendAsync<VeloraRpcCallParameters, string>("eth_call",
			new()
			{
				Call = new()
				{
		...
		where TParameters : VeloraRpcParameters
	{
		...
		var payload = JsonConvert.SerializeObject(
			new VeloraRpcRequest<TParameters>
			{
				Id = requestId,
				Method = method,
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			if (!response.IsSuccessStatusCode)
				throw new InvalidOperationException(
					...
					Truncate(body));
			VeloraRpcResponse<TResult> rpc;
			try
			{
				rpc = JsonConvert.DeserializeObject<
					VeloraRpcResponse<TResult>>(body, _jsonSettings);
			}
	...
	private void ValidateTransaction(VeloraTransaction transaction)
	{
	...
	private VeloraRpcCall ToRpcCall(VeloraTransaction transaction)
		=> new()
		{
			From = WalletAddress,
