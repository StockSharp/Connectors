...
static class VeloraExtensions
{
	...
	public static string GetDefaultRpcEndpoint(this VeloraChains chain)
		=> chain switch
		{
			VeloraChains.Ethereum =>
				"https://ethereum-rpc.publicnode.com",
			VeloraChains.Optimism => "https://mainnet.optimism.io",
			VeloraChains.Bnb => "https://bsc-dataseed.binance.org",
			VeloraChains.Gnosis => "https://rpc.gnosischain.com",
			VeloraChains.Polygon =>
				"https://polygon-bor-rpc.publicnode.com",
			VeloraChains.Base => "https://mainnet.base.org",
			VeloraChains.Arbitrum => "https://arb1.arbitrum.io/rpc",
			VeloraChains.Avalanche =>
				"https://api.avax.network/ext/bc/C/rpc",
			_ => throw new ArgumentOutOfRangeException(nameof(chain), chain,
				"Unsupported Velora chain."),
		};

	public static string GetDefaultMarkets(this VeloraChains chain)
		=> chain switch
		{
			VeloraChains.Ethereum =>
	...
	public static string GetNativeSymbol(this VeloraChains chain)
		=> chain switch
		{
			VeloraChains.Ethereum or VeloraChains.Optimism or
				VeloraChains.Base or VeloraChains.Arbitrum => "ETH",
			VeloraChains.Bnb => "BNB",
			VeloraChains.Gnosis => "XDAI",
			VeloraChains.Polygon => "POL",
			VeloraChains.Avalanche => "AVAX",
			_ => throw new ArgumentOutOfRangeException(nameof(chain), chain,
				"Unsupported Velora chain."),
		};
	...
	public static string ToVeloraAddress(this string address)
		=> address.IsNativeToken()
			? ApiNativeTokenAddress
			: address.NormalizeAddress();

	public static bool MatchesVeloraAddress(this string address,
		string expected)
	{
		address = address.NormalizeAddress();
		expected = expected.NormalizeAddress();
		if (expected.IsNativeToken())
			return address.EqualsIgnoreCase(ApiNativeTokenAddress);
		return address.EqualsIgnoreCase(expected);
	}
	...
	public static SecurityId ToStockSharp(this VeloraMarket market)
		=> new()
		{
