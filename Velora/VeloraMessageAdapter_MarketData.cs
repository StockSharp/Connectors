...
public partial class VeloraMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"Velora executable quotes do not expose historical Level1 " +
				"events.");
	...
	private SecurityMessage CreateSecurity(VeloraMarket market,
	...
	private async ValueTask SendLevel1Async(VeloraMarket market,
	...
	private async ValueTask<(decimal Bid, decimal BidVolume, decimal Ask,
		decimal AskVolume)> LoadLevel1Async(VeloraMarket market,
		CancellationToken cancellationToken)
	{
		...
		if (bid <= 0 || ask <= 0 || askBaseVolume <= 0)
			throw new InvalidDataException(
				"Velora returned a non-positive executable quote.");
	...
	private async ValueTask PollMarketAsync(
		CancellationToken cancellationToken)
	{
		(VeloraMarket Market, long[] Targets)[] groups;
		using (_sync.EnterScope())
