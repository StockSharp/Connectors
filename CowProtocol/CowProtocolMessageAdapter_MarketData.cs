...
public partial class CowProtocolMessageAdapter
{
        ...
        MarketDataMessage mdMsg, CancellationToken cancellationToken)
    {
        ...
        if (mdMsg.From is not null)
            throw new NotSupportedException(
                "CoW Protocol quote probes do not expose historical Level1 " +
                "events.");
        ...
        MarketDataMessage mdMsg, CancellationToken cancellationToken)
    {
        ...
        if (!AllTimeFrames.Contains(timeFrame))
            throw new NotSupportedException(
                $"CoW Protocol does not support the {timeFrame} candle interval.");
        var now = DateTime.UtcNow;
        var to = (mdMsg.To ?? now).ToUniversalTime().Min(now);
        var maximum = GetSubscriptionMaximum(mdMsg.Count);
    ...
    private SecurityMessage CreateSecurity(CowProtocolMarket market,
        long originalTransactionId)
        => new SecurityMessage
        {
            SecurityId = market.ToStockSharp(),
            Name = $"{market.BaseToken.Symbol}/{market.QuoteToken.Symbol} CoW",
    ...
    private async ValueTask SendLevel1Async(CowProtocolMarket market,
    ...
    private async ValueTask<(decimal Bid, decimal Ask)> LoadLevel1Async(
        CowProtocolMarket market, CancellationToken cancellationToken)
    {
        ...
        var bidQuote = await GetQuoteAsync(market,
            CowProtocolTradeTypes.ExactInput, units, cancellationToken);
        var askQuote = await GetQuoteAsync(market,
            CowProtocolTradeTypes.ExactOutput, units, cancellationToken);
    ...
    private ValueTask SendLevel1Async(CowProtocolMarket market, long target,
    ...
    private async ValueTask<CowProtocolTrade[]> LoadTradesAsync(
        CowProtocolMarket market, DateTime from, DateTime to, int maximum,
    ...
    private async ValueTask<CowProtocolTrade[]> LoadTradesByBlocksAsync(
        CowProtocolMarket market, BigInteger fromBlock, BigInteger toBlock,
    ...
    private async ValueTask<CowProtocolTrade> ToTradeAsync(
        CowProtocolMarket market, CowProtocolRpcLog log,
        CancellationToken cancellationToken)
    {
        if (log is null || log.IsRemoved || log.Address.IsEmpty() ||
            log.BlockNumber.IsEmpty() || log.TransactionHash.IsEmpty() ||
            log.LogIndex.IsEmpty() ||
            log.Topics is not { Length: >= 2 } topics ||
            !topics[0].EqualsIgnoreCase(CowProtocolExtensions.TradeTopic) ||
            !log.Address.NormalizeAddress().EqualsIgnoreCase(
        ...
        try
        {
            sellToken = CowProtocolExtensions.ReadAbiAddress(log.Data, 0);
            buyToken = CowProtocolExtensions.ReadAbiAddress(log.Data, 1);
            sellAmount = CowProtocolExtensions.ReadAbiWord(log.Data, 2);
            buyAmount = CowProtocolExtensions.ReadAbiWord(log.Data, 3);
            feeAmount = CowProtocolExtensions.ReadAbiWord(log.Data, 4);
            orderUid = CowProtocolExtensions.ReadAbiBytes(log.Data, 5)
                .ToHex(true).NormalizeOrderUid();
        }
        ...
        if (sellAmount <= 0 || buyAmount <= 0 || feeAmount < 0 ||
            feeAmount >= sellAmount)
            return null;
        var sellAmountBeforeFees = sellAmount - feeAmount;
        Sides side;
        BigInteger baseAmount;
        BigInteger quoteAmount;
        if (sellToken.EqualsIgnoreCase(market.BaseToken.Address) &&
            buyToken.EqualsIgnoreCase(market.QuoteToken.Address))
        {
            side = Sides.Sell;
            baseAmount = sellAmountBeforeFees;
            quoteAmount = buyAmount;
        }
        else if (sellToken.EqualsIgnoreCase(market.QuoteToken.Address) &&
            buyToken.EqualsIgnoreCase(market.BaseToken.Address))
        {
            side = Sides.Buy;
            baseAmount = buyAmount;
            quoteAmount = sellAmountBeforeFees;
        }
        else
            return null;
        decimal volume;
        ...
        try
        {
            volume = baseAmount.FromBaseUnits(
                market.BaseToken.Decimals);
            quote = quoteAmount.FromBaseUnits(
    ...
    private async ValueTask<bool> SendTradeAsync(CowProtocolMarket market,
        CowProtocolTrade trade, long target,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<CowProtocolCandle[]> LoadCandlesAsync(
        CowProtocolMarket market, TimeSpan timeFrame, DateTime from, DateTime to,
    ...
    private ValueTask SendCandleAsync(CowProtocolMarket market,
        CowProtocolCandle candle, TimeSpan timeFrame, long target,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask PollLevel1Async(
        CancellationToken cancellationToken)
    {
        (CowProtocolMarket Market, long[] Targets)[] groups;
        using (_sync.EnterScope())
