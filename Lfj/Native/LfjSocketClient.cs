...
sealed class LfjSocketClient : BaseLogReceiver
{
    ...
    public LfjSocketClient(string endpoint)
    {
    ...
    public override string Name => "LFJ_WebSocket";

    public event Action<LfjRpcLog> LogReceived;

    public async ValueTask ConnectAsync(IEnumerable<LfjMarket> markets,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(markets);
        if (_socket is not null)
            throw new InvalidOperationException(
                "The LFJ WebSocket is already connected.");
    ...
    protected override void DisposeManaged()
    {
        ...
        _socket?.Dispose();
        _lifetime?.Dispose();
        FailPending(new ObjectDisposedException(nameof(LfjSocketClient)));
        _sendGate.Dispose();
        _socket = null;
        _lifetime = null;
        _receiveTask = null;
    ...
    private async ValueTask SubscribeAsync(LfjMarket market,
        CancellationToken cancellationToken)
    {
        ...
        var payload = JsonConvert.SerializeObject(new LfjSocketRequest
        {
            Id = id,
            Parameters = new()
            {
        ...
        try
        {
            ...
            if (subscription.IsEmpty())
                throw new InvalidDataException(
                    "LFJ returned an empty WebSocket subscription id.");
        }
        catch
        {
            using (_sync.EnterScope())
        ...
        CancellationToken cancellationToken)
    {
        var socket = _socket ?? throw new InvalidOperationException(
            "The LFJ WebSocket is not connected.");
    ...
    private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
    {
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                LfjSocketMessage message;
                try
                {
                    message = JsonConvert.DeserializeObject<
                        LfjSocketMessage>(payload, _jsonSettings);
                }
                catch (JsonException error)
                {
                    throw new InvalidDataException(
                        "LFJ WebSocket returned an unexpected payload.",
                        error);
                }
                if (message is null)
                    throw new InvalidDataException(
                        "LFJ WebSocket returned an empty payload.");
                ...
                if (message.Method.EqualsIgnoreCase("eth_subscription") &&
                    message.Parameters?.Result is LfjRpcLog log)
                {
                    try
                    {
        ...
        {
            this.AddWarningLog(
                "LFJ WebSocket receive loop stopped: {0}",
                error.Message);
            FailPending(error);
        }
    }
        ...
        while (true)
        {
            ...
            if (result.MessageType == WebSocketMessageType.Close)
                throw new WebSocketException(
                    $"LFJ WebSocket closed with status " +
                    $"'{socket.CloseStatus}'.");
            if (result.MessageType != WebSocketMessageType.Text)
                throw new InvalidDataException(
                    "LFJ WebSocket returned a non-text message.");
            if (target.Length + result.Count > _maximumMessageBytes)
                throw new InvalidDataException(
                    "LFJ WebSocket message exceeds 1 MiB.");
