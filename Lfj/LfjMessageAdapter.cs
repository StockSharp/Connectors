...
public partial class LfjMessageAdapter
{
    ...
    {
        public LfjMarket Market { get; init; }
    }
    ...
    private sealed class TrackedSwap
    {
        ...
        public LfjRpcReceipt Receipt { get; set; }
    }
    ...
    private readonly Dictionary<string, LfjMarket> _markets =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, LfjMarket> _marketsByPool =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, LfjToken> _tokens =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    /// <inheritdoc />
    public override string[] AssociatedBoards => [BoardCodes.Lfj];

    /// <inheritdoc />
    protected override bool ValidateSecurityId(SecurityId securityId)
        => securityId.BoardCode.IsEmpty() ||
            securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Lfj) ||
            securityId.IsAssociated(BoardCodes.Lfj);

    private LfjRpcClient RpcClient => _rpcClient ?? throw new
    ...
    private void EnsureTradingReady()
    {
        ...
        if (!RpcClient.IsSigningAvailable)
            throw new InvalidOperationException(
                "An EVM private key is required for LFJ swaps.");
    }

    private LfjMarket GetMarket(SecurityId securityId)
    {
        if (!ValidateSecurityId(securityId))
            throw new InvalidOperationException(
                $"Security board '{securityId.BoardCode}' is not LFJ.");
        ...
        using (_sync.EnterScope())
            return _markets.TryGetValue(code, out var market)
                ? market
                : throw new InvalidOperationException(
                    $"Unknown LFJ market '{code}'.");
    }

    private string GetPortfolioName()
        => $"LFJ_Avalanche_{RpcClient.WalletAddress[2..10]}";
    ...
    private void ValidatePortfolio(string portfolioName)
    {
        ...
        if (!portfolioName.IsEmpty() &&
            ...
            throw new InvalidOperationException(
                $"Unknown LFJ portfolio '{portfolioName}'.");
    }

    private void OnSocketLog(LfjRpcLog log)
    {
        if (log is null)
            return;
        using (_sync.EnterScope())
