...
public partial class CurveMessageAdapter
{
        ...
        OrderRegisterMessage regMsg, CancellationToken cancellationToken)
    {
        ...
        if (regMsg.OrderType is not (null or OrderTypes.Market))
            throw new NotSupportedException(
                "Curve swaps are immediate AMM market orders.");
        if (regMsg.Condition is not null)
            throw new NotSupportedException(
                "Curve pool contracts do not expose conditional orders.");
        ...
        if (volume <= 0)
            throw new InvalidOperationException(
                "Curve swap volume must be positive.");
        var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
        if (baseUnits <= 0)
            throw new InvalidOperationException(
                "Curve swap volume rounds to zero base units.");

        var tradeType = regMsg.Side == Sides.Sell
            ? CurveTradeTypes.ExactInput
            : CurveTradeTypes.ExactOutput;
        ...
        var inputToken = regMsg.Side == Sides.Sell
            ? market.BaseToken
            : market.QuoteToken;
        var approvalAmount = preliminary.InputAmount;
        ...
        var quote = await RpcClient.GetQuoteAsync(market, tradeType,
            baseUnits, cancellationToken);
        var finalApprovalAmount = quote.InputAmount;
        if (!inputToken.Address.IsNativeToken() &&
        ...
        OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "A broadcast Curve transaction cannot be replaced through " +
            "the protocol API.");
    }
        ...
        OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "Curve has no cancellable order book. Pending EVM nonce " +
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "Curve has no open-order group to cancel.");
    }
        ...
        OrderStatusMessage statusMsg, CancellationToken cancellationToken)
    {
        ...
        if (statusMsg.OrderId is not null)
            throw new NotSupportedException(
                "Curve orders use EVM transaction hashes, not numeric " +
                "order identifiers.");
        if (!statusMsg.UserId.IsEmpty())
            throw new NotSupportedException(
                "Curve has no exchange-side user identifier.");
        if (statusMsg.SecurityIds.Length > 0)
            throw new NotSupportedException(
                "Use the primary security filter for Curve order status.");
    ...
    private async ValueTask EnsureApprovalAsync(CurveToken token,
        string spender, BigInteger amount,
        CancellationToken cancellationToken)
    {
        if (amount <= 0)
            throw new InvalidOperationException(
                "Curve approval amount must be positive.");
    ...
    private async ValueTask BroadcastAndConfirmAsync(
        CurveTransaction transaction, string operation,
        CancellationToken cancellationToken)
    {
        var hash = await RpcClient.SendTransactionAsync(transaction,
            cancellationToken);
        ...
        if (!IsSuccessful(receipt))
            throw new InvalidOperationException(
                $"Curve {operation} transaction '{hash}' reverted.");
    }
    ...
    private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
        CurveRpcReceipt receipt, CancellationToken cancellationToken)
    {
        ...
        if (state == OrderStates.Done)
        {
            ...
            {
                this.AddWarningLog(
                    "Curve fill decoding failed for {0}; using the " +
    ...
    private async ValueTask<
        (CurveToken Token, BigInteger Amount)[]> LoadBalancesAsync(
        CancellationToken cancellationToken)
    {
        CurveToken[] tokens;
        using (_sync.EnterScope())
    ...
    private async ValueTask SendPortfolioSnapshotAsync(long target,
        bool isForced,
        (CurveToken Token, BigInteger Amount)[] balances,
        CancellationToken cancellationToken)
    {
    ...
    private static decimal? GetCommission(CurveRpcReceipt receipt)
    {
    ...
    private static decimal GetSwapPrice(CurveMarket market, Sides side,
        decimal volume, BigInteger inputAmount, BigInteger outputAmount)
    {
        ...
        if (price <= 0)
            throw new InvalidDataException(
                "Curve returned a non-positive execution price.");
        return price;
    }

    private static SwapExecution ReadSwapExecution(TrackedSwap swap,
        CurveRpcReceipt receipt)
    {
        ...
        foreach (var log in receipt.Logs ?? [])
        {
            ...
            try
            {
                ...
                var bought = CurveExtensions.ReadAbiWord(log.Data,
                    offset + 2);
                if (sold > int.MaxValue || bought > int.MaxValue)
                    continue;
                var soldIndex = (int)sold;
                var boughtIndex = (int)bought;
                var soldAmount = CurveExtensions.ReadAbiWord(log.Data,
                    offset + 1);
                var boughtAmount = CurveExtensions.ReadAbiWord(log.Data,
                    offset + 3);
                var isExpected = swap.Side == Sides.Sell
                    ? soldIndex == swap.Market.BaseToken.PoolIndex &&
                        boughtIndex == swap.Market.QuoteToken.PoolIndex
                    : soldIndex == swap.Market.QuoteToken.PoolIndex &&
                        boughtIndex == swap.Market.BaseToken.PoolIndex;
                if (!isExpected)
                    continue;
                baseAmount += swap.Side == Sides.Sell
                    ? soldAmount
                    : boughtAmount;
                quoteAmount += swap.Side == Sides.Sell
                    ? boughtAmount
                    : soldAmount;
                isFound = true;
            }
        ...
        if (!isFound)
            throw new InvalidDataException(
                ...
                "TokenExchange event.");
        var volume = baseAmount.FromBaseUnits(
            swap.Market.BaseToken.Decimals);
        var quote = quoteAmount.FromBaseUnits(
            swap.Market.QuoteToken.Decimals);
        if (volume <= 0 || quote <= 0)
            throw new InvalidDataException(
                $"Curve transaction '{swap.TransactionHash}' " +
    ...
    private static bool IsSuccessful(CurveRpcReceipt receipt)
