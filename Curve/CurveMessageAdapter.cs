...
public partial class CurveMessageAdapter
{
    ...
    {
        public CurveMarket Market { get; init; }
    }
    ...
    private sealed class TrackedSwap
    {
        ...
        public CurveRpcReceipt Receipt { get; set; }
    }
    ...
    private readonly Dictionary<string, CurveMarket> _markets =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, List<CurveMarket>> _marketsByPool =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, CurveToken> _tokens =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    private readonly Queue<CurveRpcLog> _realtimeLogs = [];
    private readonly Dictionary<BigInteger, DateTime> _blockTimes = [];
    private readonly Queue<BigInteger> _blockTimeOrder = [];
    private CurveRpcClient _rpcClient;
    private CurveApiClient _apiClient;
    private CurveSocketClient _socketClient;
    private DateTime _nextMarketPoll;
    private DateTime _nextPrivatePoll;

    /// <summary>Initializes the adapter.</summary>
    public CurveMessageAdapter(IdGenerator transactionIdGenerator)
    ...
    /// <inheritdoc />
    public override string[] AssociatedBoards => [BoardCodes.Curve];

    /// <inheritdoc />
    protected override bool ValidateSecurityId(SecurityId securityId)
        => securityId.BoardCode.IsEmpty() ||
            securityId.BoardCode.EqualsIgnoreCase(BoardCodes.Curve) ||
            securityId.IsAssociated(BoardCodes.Curve);

    private CurveRpcClient RpcClient => _rpcClient ?? throw new
        InvalidOperationException(LocalizedStrings.ConnectionNotOk);

    private CurveApiClient ApiClient => _apiClient ?? throw new
        InvalidOperationException(LocalizedStrings.ConnectionNotOk);
    ...
    private void EnsureTradingReady()
    {
        ...
        if (!RpcClient.IsSigningAvailable)
            throw new InvalidOperationException(
                "An EVM private key is required for Curve swaps.");
    }

    private CurveMarket GetMarket(SecurityId securityId)
    {
        if (!ValidateSecurityId(securityId))
            throw new InvalidOperationException(
                $"Security board '{securityId.BoardCode}' is not Curve.");
        ...
        using (_sync.EnterScope())
            return _markets.TryGetValue(code, out var market)
                ? market
                : throw new InvalidOperationException(
                    $"Unknown Curve market '{code}'.");
    }

    private string GetPortfolioName()
        => $"Curve_Ethereum_{RpcClient.WalletAddress[2..10]}";
    ...
    private void ValidatePortfolio(string portfolioName)
    {
        ...
        if (!portfolioName.IsEmpty() &&
            ...
            throw new InvalidOperationException(
                $"Unknown Curve portfolio '{portfolioName}'.");
    }

    private void OnSocketLog(CurveRpcLog log)
    {
        if (log is null)
            return;
        using (_sync.EnterScope())
