...
sealed class CurveSocketClient : BaseLogReceiver
{
	...
	public CurveSocketClient(string endpoint)
	{
	...
	public override string Name => "Curve_WebSocket";

	public bool IsConnected => _socket?.State == WebSocketState.Open &&
		_receiveTask?.IsCompleted == false;

	public event Action<CurveRpcLog> LogReceived;

	public async ValueTask ConnectAsync(IEnumerable<CurveMarket> markets,
		CancellationToken cancellationToken)
	{
		ArgumentNullException.ThrowIfNull(markets);
		if (_socket is not null)
			throw new InvalidOperationException(
				"The Curve WebSocket is already connected.");
		...
		try
		{
			...
			foreach (var poolId in markets
				.GroupBy(static market => market.PoolId,
					StringComparer.OrdinalIgnoreCase)
				.Select(static group => group.Key))
				await SubscribeAsync(poolId, cancellationToken);
		}
		catch
		{
			lifetime.Cancel();
	...
	protected override void DisposeManaged()
	{
		...
		_socket?.Dispose();
		_lifetime?.Dispose();
		FailPending(new ObjectDisposedException(nameof(CurveSocketClient)));
		_sendGate.Dispose();
		_socket = null;
		_lifetime = null;
		_receiveTask = null;
	...
	private async ValueTask SubscribeAsync(string poolId,
		CancellationToken cancellationToken)
	{
		poolId = poolId.NormalizeAddress();
		var id = Interlocked.Increment(ref _requestId);
		var completion = new TaskCompletionSource<string>(
			TaskCreationOptions.RunContinuationsAsynchronously);
		using (_sync.EnterScope())
			_pending.Add(id, completion);
		var payload = JsonConvert.SerializeObject(new CurveSocketRequest
		{
			Id = id,
			Parameters = new()
			{
				...
				{
					Address = poolId,
					Topics = [CurveExtensions.SwapTopics],
				},
			},
		}, _jsonSettings);
		try
		{
			...
			if (subscription.IsEmpty())
				throw new InvalidDataException(
					"Curve returned an empty WebSocket subscription id.");
		}
		catch
		{
			using (_sync.EnterScope())
		...
		CancellationToken cancellationToken)
	{
		var socket = _socket ?? throw new InvalidOperationException(
			"The Curve WebSocket is not connected.");
	...
	private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
	{
		try
		{
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				CurveSocketMessage message;
				try
				{
					message = JsonConvert.DeserializeObject<
						CurveSocketMessage>(payload, _jsonSettings);
				}
				catch (JsonException error)
				{
					throw new InvalidDataException(
						"Curve WebSocket returned an unexpected payload.",
						error);
				}
				if (message is null)
					throw new InvalidDataException(
						"Curve WebSocket returned an empty payload.");
				...
				if (message.Method.EqualsIgnoreCase("eth_subscription") &&
					message.Parameters?.Result is CurveRpcLog log)
				{
					if (log.Topics is not { Length: > 0 } ||
						!CurveExtensions.SwapTopics.Contains(log.Topics[0],
							StringComparer.OrdinalIgnoreCase))
						continue;
					try
					{
		...
		catch (Exception error)
		{
			_socket?.Abort();
			this.AddWarningLog(
				"Curve WebSocket receive loop stopped: {0}",
				error.Message);
			FailPending(error);
		}
	}
		...
		while (true)
		{
			...
			if (result.MessageType == WebSocketMessageType.Close)
				throw new WebSocketException(
					$"Curve WebSocket closed with status " +
					$"'{socket.CloseStatus}'.");
			if (result.MessageType != WebSocketMessageType.Text)
				throw new InvalidDataException(
					"Curve WebSocket returned a non-text message.");
			if (target.Length + result.Count > _maximumMessageBytes)
				throw new InvalidDataException(
					"Curve WebSocket message exceeds 1 MiB.");
