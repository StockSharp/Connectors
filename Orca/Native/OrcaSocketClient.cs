...
sealed class OrcaSocketClient : BaseLogReceiver
{
	...
	public OrcaSocketClient(string endpoint,
	...
	public override string Name => "Orca_WebSocket";
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (var pool in pools)
		{
			...
			await SendAsync(new OrcaSocketRequest<OrcaSocketParameters>
			{
				Id = id,
				Method = "logsSubscribe",
				Parameters = new OrcaSocketLogsParameters
				{
					Filter = new()
					{
						Mentions = [pool],
		...
		try
		{
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				if (result.MessageType != WebSocketMessageType.Text)
					continue;
				var message = JsonConvert.DeserializeObject<OrcaSocketMessage>(
	...
	private async ValueTask HandleAsync(OrcaSocketMessage message)
	{
	...
	private async ValueTask SendAsync<TParameters>(
		OrcaSocketRequest<TParameters> request,
		CancellationToken cancellationToken)
