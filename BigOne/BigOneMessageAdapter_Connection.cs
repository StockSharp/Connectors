...
public partial class BigOneMessageAdapter
{
		...
		ConnectMessage connectMsg, CancellationToken cancellationToken)
	{
		...
		if (configured.Any(static value => value) &&
			...
			throw new InvalidOperationException(
				"BigONE API key and secret must be " +
					"configured together.");
		...
		try
		{
			...
			_wsClient.BalanceReceived += OnWebSocketBalanceAsync;
			_wsClient.PositionReceived +=
				OnWebSocketContractPositionAsync;
			_wsClient.OrderReceived += OnWebSocketOrderAsync;
			_wsClient.PrivateTradeReceived +=
				OnWebSocketPrivateTradeAsync;
			...
			if (markets is not { Length: > 0 })
				throw new InvalidDataException(
					"BigONE returned no spot or contract markets.");
	...
	private void UnsubscribeClientEvents(BigOneWsClient client)
	{
		...
		client.BalanceReceived -= OnWebSocketBalanceAsync;
		client.PositionReceived -=
			OnWebSocketContractPositionAsync;
		client.OrderReceived -= OnWebSocketOrderAsync;
		client.PrivateTradeReceived -=
			OnWebSocketPrivateTradeAsync;
