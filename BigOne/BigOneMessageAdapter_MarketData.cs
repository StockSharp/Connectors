...
public partial class BigOneMessageAdapter
{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"BigOne does not expose historical Level1 events.");
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (mdMsg.From is not null)
			throw new NotSupportedException(
				"BigOne does not expose historical order-book events.");

		var market = GetMarket(mdMsg.SecurityId);
		var depth = (mdMsg.MaxDepth ?? 20).Min(20).Max(1);
		var streamDepth = BigOneRestClient.NormalizeDepth(depth);
		if (mdMsg.IsHistoryOnly())
		{
		...
		MarketDataMessage mdMsg, CancellationToken cancellationToken)
	{
		...
		if (!BigOneExtensions.TimeFrames.Contains(timeFrame))
			throw new NotSupportedException(
				$"BigONE does not support the {timeFrame} " +
					"candle interval.");

		if (market.IsContract &&
			!timeFrame.IsContractPeriodSupported())
			throw new NotSupportedException(
				$"BigONE contracts do not support the {timeFrame} " +
					"candle interval.");
		var resolution = market.IsContract
	...
	private SecurityMessage CreateSecurity(BigOneSymbol market,
	...
	private async ValueTask SendLevel1SnapshotAsync(
		BigOneSymbol market, long transactionId,
		CancellationToken cancellationToken)
	{
		...
		if (ticker is null)
			throw new InvalidDataException(
				$"BigOne returned no ticker for '{market.Pair}'.");
	...
	private Level1ChangeMessage CreateLevel1Message(
		BigOneSymbol market, BigOneTicker ticker, long transactionId)
		=> new Level1ChangeMessage
		{
			SecurityId = market.ToStockSharp(),
			ServerTime = ticker.Timestamp > 0
				? ticker.Timestamp.FromBigOneMilliseconds()
				: CurrentTime,
	...
	private async ValueTask OnWebSocketTickerAsync(
		BigOneTicker ticker, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketOrderBookAsync(
		BigOneOrderBook book, CancellationToken cancellationToken)
	{
	...
	private async ValueTask OnWebSocketTradesAsync(
		BigOneTradePush push, CancellationToken cancellationToken)
	{
	...
	private ValueTask SendDepthAsync(string securityCode,
		BigOneOrderBook book, long transactionId, int depth,
		CancellationToken cancellationToken)
		=> SendOutMessageAsync(new QuoteChangeMessage
		{
			SecurityId = securityCode.ToBigOneSecurityId(),
	...
	private ValueTask SendPublicTradeAsync(string securityCode,
		BigOneTrade trade, string tradeId, long transactionId,
		CancellationToken cancellationToken)
	...
	private ValueTask SendCandleAsync(string securityCode,
		BigOneCandle candle, TimeSpan timeFrame, long transactionId,
		CancellationToken cancellationToken)
	{
