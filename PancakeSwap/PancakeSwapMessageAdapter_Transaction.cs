...
public partial class PancakeSwapMessageAdapter
{
    private static readonly string _v2SwapTopic =
        PancakeSwapExtensions.AbiTopic(
        ...
        OrderRegisterMessage regMsg, CancellationToken cancellationToken)
    {
        ...
        if (regMsg.OrderType is not (null or OrderTypes.Market))
            throw new NotSupportedException(
                "PancakeSwap swaps are immediate AMM market orders.");
        if (regMsg.Condition is not null)
            throw new NotSupportedException(
                "PancakeSwap pool contracts do not expose conditional orders.");
        ...
        if (volume <= 0)
            throw new InvalidOperationException(
                "PancakeSwap swap volume must be positive.");
        var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
        if (baseUnits <= 0)
            throw new InvalidOperationException(
                "PancakeSwap swap volume rounds to zero base units.");

        var tradeType = regMsg.Side == Sides.Sell
            ? PancakeSwapTradeTypes.ExactInput
            : PancakeSwapTradeTypes.ExactOutput;
        ...
        var approvalAmount = tradeType == PancakeSwapTradeTypes.ExactInput
        ...
        OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "A broadcast PancakeSwap transaction cannot be replaced through " +
            "the protocol API.");
    }
        ...
        OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "PancakeSwap has no cancellable order book. Pending EVM nonce " +
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "PancakeSwap has no open-order group to cancel.");
    }
        ...
        OrderStatusMessage statusMsg, CancellationToken cancellationToken)
    {
        ...
        if (statusMsg.OrderId is not null)
            throw new NotSupportedException(
                "PancakeSwap orders use EVM transaction hashes, not numeric " +
                "order identifiers.");
        if (!statusMsg.UserId.IsEmpty())
            throw new NotSupportedException(
                "PancakeSwap has no exchange-side user identifier.");
        if (statusMsg.SecurityIds.Length > 0)
            throw new NotSupportedException(
                "Use the primary security filter for PancakeSwap order status.");
    ...
    private async ValueTask EnsureApprovalAsync(PancakeSwapToken token,
        string spender, BigInteger amount,
        CancellationToken cancellationToken)
    {
        if (amount <= 0)
            throw new InvalidOperationException(
                "PancakeSwap approval amount must be positive.");
    ...
    private async ValueTask BroadcastAndConfirmAsync(
        PancakeSwapTransaction transaction, string operation,
        CancellationToken cancellationToken)
    {
        var hash = await RpcClient.SendTransactionAsync(transaction,
            cancellationToken);
        ...
        if (!IsSuccessful(receipt))
            throw new InvalidOperationException(
                $"PancakeSwap {operation} transaction '{hash}' reverted.");
    }
    ...
    private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
        PancakeSwapRpcReceipt receipt, CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<
        (PancakeSwapToken Token, BigInteger Amount)[]> LoadBalancesAsync(
        CancellationToken cancellationToken)
    {
        PancakeSwapToken[] tokens;
        using (_sync.EnterScope())
    ...
    private async ValueTask SendPortfolioSnapshotAsync(long target,
        bool isForced,
        (PancakeSwapToken Token, BigInteger Amount)[] balances,
        CancellationToken cancellationToken)
    {
    ...
    private static decimal? GetCommission(PancakeSwapRpcReceipt receipt)
    {
    ...
    private static decimal GetSwapPrice(PancakeSwapMarket market, Sides side,
        decimal volume, BigInteger inputAmount, BigInteger outputAmount)
    {
        ...
        if (price <= 0)
            throw new InvalidDataException(
                "PancakeSwap returned a non-positive execution price.");
        return price;
    }

    private static SwapExecution ReadSwapExecution(TrackedSwap swap,
        PancakeSwapRpcReceipt receipt)
    {
        ...
        foreach (var log in receipt.Logs ?? [])
        {
            ...
            if (!address.EqualsIgnoreCase(swap.Market.PoolId))
                continue;
            if (swap.Market.PoolVersion == PancakeSwapPoolVersions.V2)
            {
                amount0 += PancakeSwapExtensions.ReadAbiWord(log.Data, 0) -
                    PancakeSwapExtensions.ReadAbiWord(log.Data, 2);
                amount1 += PancakeSwapExtensions.ReadAbiWord(log.Data, 1) -
                    PancakeSwapExtensions.ReadAbiWord(log.Data, 3);
            }
            else
            {
                amount0 += PancakeSwapExtensions.ReadAbiSignedWord(
                    log.Data, 0);
                amount1 += PancakeSwapExtensions.ReadAbiSignedWord(
                    log.Data, 1);
            }
            isFound = true;
        }
        ...
        if (!isExpectedDirection)
            throw new InvalidDataException(
                $"PancakeSwap transaction '{swap.TransactionHash}' " +
    ...
    private static bool IsSuccessful(PancakeSwapRpcReceipt receipt)
