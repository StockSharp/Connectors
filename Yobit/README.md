# Yobit Connector for StockSharp

This directory contains the **Yobit** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `YobitMessageAdapter` message adapter, exposing Yobit market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Yobit`.

## Configuration

`YobitMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new YobitMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Yobit connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/yobit.html).
