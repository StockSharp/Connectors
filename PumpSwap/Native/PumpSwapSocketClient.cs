...
sealed class PumpSwapSocketClient : BaseLogReceiver
{
	...
	public PumpSwapSocketClient(string endpoint,
	...
	public override string Name => "PumpSwap_WebSocket";
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (var pool in pools)
		{
			...
			await SendAsync(new PumpSwapSocketRequest<
				PumpSwapSocketParameters>
			{
				Id = id,
				Method = "logsSubscribe",
				Parameters = new PumpSwapSocketLogsParameters
				{
					Filter = new()
					{
						Mentions = [pool],
		...
		try
		{
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				var message = JsonConvert.DeserializeObject<
					PumpSwapSocketMessage>(Encoding.UTF8.GetString(
	...
	private async ValueTask HandleAsync(PumpSwapSocketMessage message)
	{
	...
	private async ValueTask SendAsync<TParameters>(
		PumpSwapSocketRequest<TParameters> request,
		CancellationToken cancellationToken)
