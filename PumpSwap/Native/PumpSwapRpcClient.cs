...
sealed class PumpSwapRpcClient : BaseLogReceiver
{
	...
	public PumpSwapRpcClient(string endpoint, PumpSwapClusters cluster,
		string walletAddress, SecureString privateKey)
	{
		...
		if (!walletAddress.IsEmpty())
		{
				...
				!walletAddress.Equals(derivedAddress, StringComparison.Ordinal))
			{
				...
				throw new ArgumentException(
					"The configured PumpSwap wallet does not match the " +
	...
	public override string Name => "PumpSwap_JSON-RPC";
	...
	public async ValueTask VerifyAsync(CancellationToken cancellationToken)
	{
		var health = await SendAsync<string>("getHealth",
			new PumpSwapRpcEmptyParameters(), cancellationToken);
		if (!health.EqualsIgnoreCase("ok"))
			throw new InvalidDataException(
				$"Solana RPC health check returned '{health}'.");
		var program = await GetAccountAsync(PumpSwapExtensions.ProgramAddress,
			cancellationToken);
		if (program is null || !program.IsExecutable ||
			!program.Owner.Equals(PumpSwapExtensions.ProgramAddress,
	...
	public async ValueTask<PumpSwapRpcAccount> GetAccountAsync(string address,
		CancellationToken cancellationToken)
		=> (await SendAsync<PumpSwapRpcContextValue<PumpSwapRpcAccount>>(
			"getAccountInfo", new PumpSwapRpcAddressAccountParameters
			{
				Address = address.NormalizePublicKey(),
				Config = new(),
			}, cancellationToken)).Value;

	public async ValueTask<PumpSwapRpcAccount[]> GetAccountsAsync(
		IEnumerable<string> addresses, CancellationToken cancellationToken)
	{
		...
		return (await SendAsync<
			PumpSwapRpcContextValue<PumpSwapRpcAccount[]>>(
			"getMultipleAccounts", new PumpSwapRpcAddressesAccountParameters
			{
	...
	{
		EnsureWallet();
		return (await SendAsync<PumpSwapRpcContextValue<ulong>>("getBalance",
			new PumpSwapRpcAddressCommitmentParameters
			{
	...
	public ValueTask<PumpSwapRpcSignatureInfo[]> GetSignaturesAsync(
		string address, string before, int limit,
		CancellationToken cancellationToken)
	{
		if (limit is < 1 or > 1000)
			throw new ArgumentOutOfRangeException(nameof(limit));
		return SendAsync<PumpSwapRpcSignatureInfo[]>(
			"getSignaturesForAddress", new PumpSwapRpcSignaturesParameters
			{
				Address = address.NormalizePublicKey(),
				Config = new()
				{
	...
	public ValueTask<PumpSwapRpcTransaction> GetTransactionAsync(
		string signature, CancellationToken cancellationToken)
		=> SendAsync<PumpSwapRpcTransaction>("getTransaction",
			new PumpSwapRpcTransactionParameters
			{
		...
		CancellationToken cancellationToken)
	{
		var accounts = (writableAccounts ?? []).Select(static address =>
			...
			.Take(128).ToArray();
		var fees = await SendAsync<PumpSwapRpcRecentFee[]>(
			"getRecentPrioritizationFees", new PumpSwapRpcRecentFeesParameters
			{
				Addresses = accounts,
			}, cancellationToken) ?? [];
		...
		CancellationToken cancellationToken)
	{
		...
		var latest = await SendAsync<
			PumpSwapRpcContextValue<PumpSwapRpcLatestBlockhash>>(
			"getLatestBlockhash", new PumpSwapRpcLatestBlockhashParameters
			{
				Config = new(),
			}, cancellationToken);
		...
		return await SendAsync<string>("sendTransaction",
			new PumpSwapRpcSendTransactionParameters
			{
	...
	public async ValueTask<PumpSwapTransactionReceipt> GetReceiptAsync(
		string signature, CancellationToken cancellationToken)
	{
	...
	public async ValueTask<PumpSwapTransactionReceipt> WaitForReceiptAsync(
		string signature, TimeSpan timeout,
		CancellationToken cancellationToken)
	{
		...
		do
		{
			var receipt = await GetReceiptAsync(signature, cancellationToken);
	...
	private async ValueTask<TResult> SendAsync<TResult>(string method,
		PumpSwapRpcParameters parameters,
		CancellationToken cancellationToken)
	{
		...
		for (var attempt = 0; ; attempt++)
		{
			...
			var request = new PumpSwapRpcRequest<PumpSwapRpcParameters>
			{
			...
			if (response.Content.Headers.ContentLength is long length &&
				...
				throw new InvalidDataException(
					$"Solana RPC response for '{method}' exceeds the " +
					"configured safety limit.");
			var body = await ReadBodyAsync(response.Content, method,
				cancellationToken);
			if (!response.IsSuccessStatusCode)
				throw new PumpSwapApiException(response.StatusCode,
					$"Solana RPC '{method}' failed: {Limit(body, 1024)}");
			PumpSwapRpcResponse<TResult> envelope;
			try
			{
				envelope = JsonConvert.DeserializeObject<
					PumpSwapRpcResponse<TResult>>(body, _serializerSettings);
			}
	...
	private void EnsureSigning()
	{
		...
		if (!IsSigningAvailable)
			throw new InvalidOperationException(
				"A Solana private key is required to sign PumpSwap swaps.");
	}
	...
	private static string NormalizeEndpoint(string endpoint)
	{
		...
		if (!Uri.TryCreate(endpoint, UriKind.Absolute, out var uri) ||
			...
			throw new ArgumentException(
				"Solana RPC endpoint must use HTTP or HTTPS.",
				nameof(endpoint));
