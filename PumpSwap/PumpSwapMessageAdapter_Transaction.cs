...
public partial class PumpSwapMessageAdapter
{
		...
		OrderRegisterMessage regMsg, CancellationToken cancellationToken)
	{
		...
		if (regMsg.OrderType is not (null or OrderTypes.Market))
			throw new NotSupportedException(
				"PumpSwap swaps are immediate AMM market orders.");
		if (regMsg.Condition is not null)
			throw new NotSupportedException(
				"PumpSwap does not expose conditional orders.");
		...
		if (!regMsg.UserOrderId.IsEmpty())
			throw new NotSupportedException(
				"A PumpSwap order identifier is its Solana transaction " +
				"signature; no client-order ID is stored on-chain.");
		var volume = regMsg.Volume.Abs();
		if (volume <= 0)
			throw new InvalidOperationException(
				"PumpSwap swap volume must be positive.");
		var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
		if (baseUnits <= 0)
			throw new InvalidOperationException(
				"PumpSwap swap volume rounds to zero base units.");
		...
		var tradeType = regMsg.Side == Sides.Buy
			? PumpSwapTradeTypes.Buy
			: PumpSwapTradeTypes.Sell;
		var quote = market.GetQuote(tradeType, baseUnits,
		...
		var instructions = PumpSwapInstructionBuilder.Build(market,
			regMsg.Side, quote, RpcClient.WalletAddress, globalConfig,
		...
		OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"A broadcast Solana transaction cannot be replaced through " +
			"PumpSwap.");
	}
		...
		OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"PumpSwap has no cancellable order book.");
	}
		...
		CancellationToken cancellationToken)
	{
		...
		_ = cancellationToken;
		throw new NotSupportedException(
			"PumpSwap has no open-order group to cancel.");
	}
		...
		OrderStatusMessage statusMsg, CancellationToken cancellationToken)
	{
		...
		if (statusMsg.OrderId is not null)
			throw new NotSupportedException(
				"PumpSwap orders use Solana transaction signatures, not " +
				"numeric identifiers.");
		if (!statusMsg.UserId.IsEmpty())
			throw new NotSupportedException(
				"PumpSwap has no exchange-side user identifier.");
		if (statusMsg.SecurityIds.Length > 0)
			throw new NotSupportedException(
				"Use the primary security filter for PumpSwap order status.");
	...
	private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
		PumpSwapTransactionReceipt receipt,
		CancellationToken cancellationToken)
	{
		...
		for (var offset = 0; offset < tokens.Length; offset += 100)
		{
			...
			var addresses = chunk.Select(token =>
				PumpSwapExtensions.AssociatedTokenAddress(
					RpcClient.WalletAddress, token.Mint,
			...
			for (var index = 0; index < chunk.Length; index++)
			{
				...
				var amount = index < accounts.Length && accounts[index] is not null
					? PumpSwapExtensions.DecodeTokenAmount(accounts[index],
						token.Mint)
					: 0;
	...
	private static decimal? GetCommission(PumpSwapTransactionReceipt receipt)
		=> receipt is null ? null : receipt.Fee / 1_000_000_000m;

	private static SwapExecution ReadSwapExecution(TrackedSwap swap,
		PumpSwapTransactionReceipt receipt)
	{
		var expectedType = swap.Side == Sides.Buy
			? PumpSwapTradeTypes.Buy
			: PumpSwapTradeTypes.Sell;
		var events = PumpSwapExtensions.DecodeEvents(swap.Signature,
			receipt.LogMessages).Where(pumpEvent =>
				pumpEvent.PoolAddress.Equals(swap.Market.PoolAddress,
					StringComparison.Ordinal) &&
				pumpEvent.TradeType == expectedType).ToArray();
		if (events.Length == 0)
			throw new InvalidDataException(
				$"Successful PumpSwap transaction '{swap.Signature}' contains " +
				"no matching trade event.");
		var baseAmount = events.Aggregate(BigInteger.Zero,
			static (sum, pumpEvent) => sum + pumpEvent.BaseAmount);
		var quoteAmount = events.Aggregate(BigInteger.Zero,
			static (sum, pumpEvent) => sum + pumpEvent.QuoteAmount);
		var volume = baseAmount.FromBaseUnits(swap.Market.BaseToken.Decimals);
		var quote = quoteAmount.FromBaseUnits(
			swap.Market.QuoteToken.Decimals);
		if (volume <= 0 || quote <= 0)
			throw new InvalidDataException(
				$"PumpSwap transaction '{swap.Signature}' contains " +
