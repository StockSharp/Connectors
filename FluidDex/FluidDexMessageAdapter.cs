...
public partial class FluidDexMessageAdapter
{
    ...
    {
        public FluidDexMarket Market { get; init; }
    }
    ...
    private sealed class TrackedSwap
    {
        ...
        public FluidDexRpcReceipt Receipt { get; set; }
    }
    ...
    private readonly Dictionary<string, FluidDexMarket> _markets =
        new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, FluidDexMarket> _marketsByPool =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    private readonly Queue<FluidDexRpcLog> _realtimeLogs = [];
    private readonly Dictionary<BigInteger, DateTime> _blockTimes = [];
    private readonly Queue<BigInteger> _blockTimeOrder = [];
    private FluidDexRpcClient _rpcClient;
    private FluidDexSocketClient _socketClient;
    private DateTime _nextMarketPoll;
    private DateTime _nextPrivatePoll;

    /// <summary>Initializes the adapter.</summary>
    public FluidDexMessageAdapter(IdGenerator transactionIdGenerator)
    ...
    /// <inheritdoc />
    public override string[] AssociatedBoards => [BoardCodes.FluidDex];

    /// <inheritdoc />
    protected override bool ValidateSecurityId(SecurityId securityId)
        => securityId.BoardCode.IsEmpty() ||
            securityId.BoardCode.EqualsIgnoreCase(BoardCodes.FluidDex) ||
            securityId.IsAssociated(BoardCodes.FluidDex);

    private FluidDexRpcClient RpcClient => _rpcClient ?? throw new
    ...
    private void EnsureTradingReady()
    {
        ...
        if (!RpcClient.IsSigningAvailable)
            throw new InvalidOperationException(
                "An EVM private key is required for Fluid DEX swaps.");
    }

    private FluidDexMarket GetMarket(SecurityId securityId)
    {
        if (!ValidateSecurityId(securityId))
            throw new InvalidOperationException(
                $"Security board '{securityId.BoardCode}' is not Fluid DEX.");
        ...
        using (_sync.EnterScope())
            return _markets.TryGetValue(code, out var market)
                ? market
                : throw new InvalidOperationException(
                    $"Unknown Fluid DEX market '{code}'.");
    }

    private string GetPortfolioName()
        => $"FluidDex_{Chain}_{RpcClient.WalletAddress[2..10]}";
    ...
    private void ValidatePortfolio(string portfolioName)
    {
        ...
        if (!portfolioName.IsEmpty() &&
            ...
            throw new InvalidOperationException(
                $"Unknown Fluid DEX portfolio '{portfolioName}'.");
    }

    private void OnSocketLog(FluidDexRpcLog log)
    {
        if (log is null)
            return;
        using (_sync.EnterScope())
