...
public partial class FluidDexMessageAdapter
{
        ...
        OrderRegisterMessage regMsg, CancellationToken cancellationToken)
    {
        ...
        if (regMsg.OrderType is not (null or OrderTypes.Market))
            throw new NotSupportedException(
                "FluidDex swaps are immediate AMM market orders.");
        if (regMsg.Condition is not null)
            throw new NotSupportedException(
                "FluidDex pool contracts do not expose conditional orders.");
        ...
        if (volume <= 0)
            throw new InvalidOperationException(
                "FluidDex swap volume must be positive.");
        var baseUnits = volume.ToBaseUnits(market.BaseToken.Decimals);
        if (baseUnits <= 0)
            throw new InvalidOperationException(
                "FluidDex swap volume rounds to zero base units.");

        var tradeType = regMsg.Side == Sides.Sell
            ? FluidDexTradeTypes.ExactInput
            : FluidDexTradeTypes.ExactOutput;
        ...
        var approvalAmount = tradeType == FluidDexTradeTypes.ExactInput
        ...
        OrderReplaceMessage replaceMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "A broadcast FluidDex transaction cannot be replaced through " +
            "the protocol API.");
    }
        ...
        OrderCancelMessage cancelMsg, CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "FluidDex has no cancellable order book. Pending EVM nonce " +
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = cancellationToken;
        throw new NotSupportedException(
            "FluidDex has no open-order group to cancel.");
    }
        ...
        OrderStatusMessage statusMsg, CancellationToken cancellationToken)
    {
        ...
        if (statusMsg.OrderId is not null)
            throw new NotSupportedException(
                "FluidDex orders use EVM transaction hashes, not numeric " +
                "order identifiers.");
        if (!statusMsg.UserId.IsEmpty())
            throw new NotSupportedException(
                "FluidDex has no exchange-side user identifier.");
        if (statusMsg.SecurityIds.Length > 0)
            throw new NotSupportedException(
                "Use the primary security filter for FluidDex order status.");
    ...
    private async ValueTask EnsureApprovalAsync(FluidDexToken token,
        string spender, BigInteger amount,
        CancellationToken cancellationToken)
    {
        if (amount <= 0)
            throw new InvalidOperationException(
                "FluidDex approval amount must be positive.");
    ...
    private async ValueTask BroadcastAndConfirmAsync(
        FluidDexTransaction transaction, string operation,
        CancellationToken cancellationToken)
    {
        var hash = await RpcClient.SendTransactionAsync(transaction,
            cancellationToken);
        ...
        if (!IsSuccessful(receipt))
            throw new InvalidOperationException(
                $"FluidDex {operation} transaction '{hash}' reverted.");
    }
    ...
    private async ValueTask ApplyReceiptAsync(TrackedSwap swap,
        FluidDexRpcReceipt receipt, CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<
        (FluidDexToken Token, BigInteger Amount)[]> LoadBalancesAsync(
        CancellationToken cancellationToken)
    {
        FluidDexToken[] tokens;
        using (_sync.EnterScope())
    ...
    private async ValueTask SendPortfolioSnapshotAsync(long target,
        bool isForced,
        (FluidDexToken Token, BigInteger Amount)[] balances,
        CancellationToken cancellationToken)
    {
    ...
    private static decimal? GetCommission(FluidDexRpcReceipt receipt)
    {
    ...
    private static decimal GetSwapPrice(FluidDexMarket market, Sides side,
        decimal volume, BigInteger inputAmount, BigInteger outputAmount)
    {
        ...
        if (price <= 0)
            throw new InvalidDataException(
                "Fluid DEX returned a non-positive execution price.");
        return price;
    }

    private static SwapExecution ReadSwapExecution(TrackedSwap swap,
        FluidDexRpcReceipt receipt)
    {
        ...
        foreach (var log in receipt.Logs ?? [])
        {
            ...
            if (direction < 0 || direction > 1 || amountIn <= 0 ||
                amountOut <= 0)
                throw new InvalidDataException(
                    "Fluid DEX Swap event contains invalid amounts.");
            if (direction == BigInteger.One)
            {
                amount0 += amountIn;
                amount1 -= amountOut;
            }
        ...
        if (!isExpectedDirection)
            throw new InvalidDataException(
                $"Fluid DEX transaction '{swap.TransactionHash}' " +
    ...
    private static bool IsSuccessful(FluidDexRpcReceipt receipt)
