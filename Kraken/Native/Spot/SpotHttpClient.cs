...
class SpotHttpClient : BaseLogReceiver
{
	...
	public SpotHttpClient(SecureString key, SecureString secret, string restEndpoint)
	{
		_key = key;
	...
	public override string Name => nameof(Kraken) + "_" + nameof(SpotHttpClient);

	public Task<Dictionary<string, AssetPair>> GetAssetPairs(string info, string pairs, CancellationToken cancellationToken)
	{
		...
		return MakeRequest<Dictionary<string, AssetPair>>(CreateUrl("public/AssetPairs"), request, cancellationToken);
	}

	public async Task<(Dictionary<string, Ohlc[]> Data, long Last)> GetOhlc(string pair, int interval, long? since, CancellationToken cancellationToken)
	{
		if (pair.IsEmpty())
		...
		var jo = await MakeRequest<JObject>(CreateUrl("public/OHLC"), request, cancellationToken);
	...
	public async Task<(Dictionary<string, Trade[]> Data, long Last)> GetRecentTrades(string pair, long? since, CancellationToken cancellationToken)
	{
		var request = CreateRequest(Method.Get);
		...
		var jo = await MakeRequest<JObject>(CreateUrl("public/Trades"), request, cancellationToken);
	...
	public Task<Dictionary<string, decimal>> GetAccountBalance(CancellationToken cancellationToken)
	{
		...
		return MakeRequest<Dictionary<string, decimal>>(uri, ApplySecret(request, uri), cancellationToken);
	}

	public Task<OpenOrders> GetOpenOrders(bool includeTrades, string userRef, CancellationToken cancellationToken)
	{
		...
		return MakeRequest<OpenOrders>(uri, ApplySecret(request, uri), cancellationToken);
	}

	public Task<ClosedOrders> GetClosedOrders(bool includeTrades, long? userRef, long? start, long? end, int? offset, string closeTime, CancellationToken cancellationToken)
	{
		...
		return MakeRequest<ClosedOrders>(uri, ApplySecret(request, uri), cancellationToken);
	}

	public Task<Dictionary<string, OrderInfo>> GetOrdersInfo(IEnumerable<string> transactionIds, bool includeTrades, long? userRef, CancellationToken cancellationToken)
	{
		...
		return MakeRequest<Dictionary<string, OrderInfo>>(uri, ApplySecret(request, uri), cancellationToken);
	}

	public Task<Dictionary<string, PositionInfo>> GetOpenPositions(IEnumerable<string> transactionIds, bool doCalculations, CancellationToken cancellationToken)
	{
		if (transactionIds == null)
			throw new ArgumentNullException(nameof(transactionIds));
		...
		return MakeRequest<Dictionary<string, PositionInfo>>(uri, ApplySecret(request, uri), cancellationToken);
	}

	public async Task<string> AddOrder(string pair, string type, string orderType, decimal volume, decimal? price, decimal? price2, string leverage, string orderFlags, string startTime, double? expireTime, long? userRef, bool validate, CancellationToken cancellationToken)
	{
		...
		dynamic response = await MakeRequest<object>(uri, ApplySecret(request, uri), cancellationToken);

		return (string)response.txid[0];
	}

	public Task CancelOrder(string transactionId, CancellationToken cancellationToken)
	{
		...
		return MakeRequest<object>(uri, ApplySecret(request, uri), cancellationToken);
	}
