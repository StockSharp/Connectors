...
class FuturesPusherClient : BaseLogReceiver
{
	...
	public override string Name => nameof(Kraken) + "_" + nameof(FuturesPusherClient);
		...
		if (obj is JObject)
		{
			...
			switch (evt)
			{
				...
				case "subscriptionStatus":
				{
					...
					{
						case "subscribed":
							_channelIds[(int)chanId] = (dt, reqIdLong, symbol, isSubscribe);
	...
	public ValueTask SubscribeTicker(long transactionId, string currency, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.Level1, true));
		return Process(transactionId, Commands.Subscribe, Subscriptions.Ticker, currency, default, default, cancellationToken);
	}

	public ValueTask UnSubscribeTicker(long transactionId, string currency, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.Level1, false));
		return Process(transactionId, Commands.Unsubscribe, Subscriptions.Ticker, currency, default, default, cancellationToken);
	}

	public ValueTask SubscribeTrades(long transactionId, string currency, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.Ticks, true));
		return Process(transactionId, Commands.Subscribe, Subscriptions.Trades, currency, default, default, cancellationToken);
	}

	public ValueTask UnSubscribeTrades(long transactionId, string currency, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.Ticks, false));
		return Process(transactionId, Commands.Unsubscribe, Subscriptions.Trades, currency, default, default, cancellationToken);
	}

	public ValueTask SubscribeOrderBook(long transactionId, string currency, int? depth, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.MarketDepth, true));
		return Process(transactionId, Commands.Subscribe, Subscriptions.OrderBook, currency, default, depth, cancellationToken);
	}

	public ValueTask UnSubscribeOrderBook(long transactionId, string currency, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, DataType.MarketDepth, false));
		return Process(transactionId, Commands.Unsubscribe, Subscriptions.OrderBook, currency, default, default, cancellationToken);
	}

	public ValueTask SubscribeCandles(long transactionId, string currency, int interval, DataType dt, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, dt, true));
		return Process(transactionId, Commands.Subscribe, Subscriptions.Candles, currency, interval, default, cancellationToken);
	}

	public ValueTask UnSubscribeCandles(long transactionId, string currency, int interval, DataType dt, CancellationToken cancellationToken)
	{
		_requests.Add(transactionId, (currency, dt, false));
		return Process(transactionId, Commands.Unsubscribe, Subscriptions.Candles, currency, interval, default, cancellationToken);
	}
