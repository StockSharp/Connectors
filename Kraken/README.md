# Kraken Connector for StockSharp

This directory contains the **Kraken** connector for the [StockSharp](https://github.com/StockSharp/StockSharp) trading platform. It implements the `KrakenMessageAdapter` message adapter, exposing Kraken market data and trading operations through the StockSharp message model. The source can be used as a reference for building your own connector or included directly in a StockSharp-based application.
...
- Trading board code: `Kraken`.

## Configuration

`KrakenMessageAdapter` is configured through the following properties:
...
```csharp
var adapter = new KrakenMessageAdapter(new IncrementalIdGenerator())
{
...
See the [Kraken connector documentation](https://doc.stocksharp.com/en/topics/api/connectors/crypto_exchanges/kraken.html).
