...
public partial class PhemexMessageAdapter
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			if (RestClient.IsCredentialsAvailable)
			{
				...
				{
					_spotUserClient = CreateUserClient(PrivateWsEndpoint, PhemexSections.Spot);
					await _spotUserClient.ConnectAsync(cancellationToken);
				}
				if (IsSectionEnabled(PhemexSections.Futures))
				{
					_futuresUserClient = CreateUserClient(PrivateWsEndpoint, PhemexSections.Futures);
	...
	private PhemexWsClient CreateMarketClient()
	{
		var client = new PhemexWsClient(PublicWsEndpoint, null, false, null, null,
			RestClient, ReConnectionSettings.WorkingTime) { Parent = this };
	...
	private PhemexWsClient CreateUserClient(string endpoint, PhemexSections section)
	{
		var client = new PhemexWsClient(endpoint, section, true, Key, Secret,
			RestClient, ReConnectionSettings.WorkingTime) { Parent = this };
	...
	private PhemexWsClient[] GetClients()
