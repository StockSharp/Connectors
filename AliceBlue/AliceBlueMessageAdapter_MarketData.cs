...
public partial class AliceBlueMessageAdapter
{
	...
	private readonly SynchronizedDictionary<string, AliceBlueInstrument> _instruments = new(StringComparer.OrdinalIgnoreCase);
	private readonly SynchronizedDictionary<string, AliceBlueMarketUpdate> _marketStates = new(StringComparer.OrdinalIgnoreCase);
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (var instrument in await _restClient.GetInstruments(cancellationToken))
		{
			...
			var lotSize = Positive(instrument.LotSize) ?? Positive(instrument.BoardLotQuantity);
			var security = new SecurityMessage
			{
				OriginalTransactionId = lookupMsg.TransactionId,
				SecurityId = securityId,
				SecurityType = instrument.ToSecurityType(),
				Name = instrument.FormattedName.IsEmpty(instrument.Symbol),
				ShortName = instrument.TradingSymbol.IsEmpty(instrument.Symbol),
				Class = instrument.InstrumentType.IsEmpty(instrument.GroupName),
				Currency = CurrencyTypes.INR,
				PriceStep = Positive(instrument.TickSize),
				VolumeStep = lotSize,
				Multiplier = lotSize,
				ExpiryDate = instrument.ExpiryTime.ToExpiry(),
		...
		CancellationToken cancellationToken)
	{
		...
		if (depth is < 1 or > 5)
			throw new ArgumentOutOfRangeException(nameof(mdMsg.MaxDepth), depth,
				"Alice Blue provides five market-depth levels.");
		...
		CancellationToken cancellationToken)
	{
		...
		if (mdMsg.IsSubscribe)
		{
			...
			if (wasEmpty || wasDepth != isDepth)
				await _marketClient.Subscribe(instrumentKey, isDepth, cancellationToken);
		...
		if (hadDepth != hasDepth)
			await _marketClient.Subscribe(instrumentKey, hasDepth, cancellationToken);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!mdMsg.IsSubscribe)
			return;
		if (!mdMsg.IsHistoryOnly())
			throw new NotSupportedException("Alice Blue provides historical candles only; realtime candle subscriptions are not available.");
		...
		IEnumerable<AliceBlueCandle> ordered = candles
			.Where(candle => candle?.Time.ToAliceBlueTime() != null)
			.OrderBy(candle => candle.Time.ToAliceBlueTime());
		...
		{
			if (candle.Time.ToAliceBlueTime() is not DateTime openTime)
				continue;
	...
	private async Task<AliceBlueInstrument> GetInstrument(string instrumentKey,
		CancellationToken cancellationToken)
	{
		...
		instrument = await _restClient.GetInstrument(instrumentKey, cancellationToken)
			?? throw new InvalidOperationException($"Alice Blue instrument '{instrumentKey}' was not found in the official contract master.");
		_instruments[instrumentKey] = instrument;
		return instrument;
	}

	private async ValueTask OnMarketDataReceived(AliceBlueMarketUpdate update,
		CancellationToken cancellationToken)
	{
